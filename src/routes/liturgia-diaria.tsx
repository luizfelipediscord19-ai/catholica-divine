...
function Bloco({ kicker, itens }: { kicker: string; itens: LeituraLiturgica[] }) {
  ...
  return (
    <section className="border-t border-gold/15 pt-8">
      <p className="text-[10px] tracking-[0.35em] uppercase text-gold mb-3">{kicker}</p>
      {itens.map((l, i) => (
        <article key={`${l.referencia}-${i}`} className="mb-8 last:mb-0">
          ...
          {l.referencia ? (
            <p className="mt-1 text-[11px] tracking-[0.2em] uppercase text-muted-foreground">{l.referencia}</p>
          ) : null}
          {l.refrao ? (
