    ...
    <div>
      ...
      <div className="mb-12">
        ...
        <div className="mt-4 flex flex-wrap gap-2" aria-label="Filtrar o glossário por categoria">
          <button type="button" onClick={() => setCategoria("todas")} aria-pressed={categoria === "todas"} className={`chip ${categoria === "todas" ? "chip-gold" : ""}`}>Todas</button>
          {(Object.entries(ROTULO_CATEGORIA_GLOSSARIO) as [CategoriaGlossario, string][]).map(([chave, rotulo]) => (
            <button key={chave} type="button" onClick={() => setCategoria(chave)} aria-pressed={categoria === chave} className={`chip ${categoria === chave ? "chip-gold" : ""}`}>{rotulo}</button>
          ))}
        </div>
        <p className="mt-4 text-step--2 text-muted-foreground" aria-live="polite">{filtrados.length} verbetes encontrados</p>
      </div>
