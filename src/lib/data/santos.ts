import { SANTOS_ADICIONAIS } from "./santos-extras";

export type Santo = {
  slug: string;
  nome: string;
  titulo: string;
  data: string; // dd/mm
  seculo: string;
  padroeiro: string;
  resumo: string;
  biografia: string;
  virtudes: string[];
  frase: string;
  imagem?: string;
};

// Biografias aprofundadas para todos os santos da lista.
// Slugs gerados a partir do nome exibido (mesma normalização do slugify),
// para casar com SANTOS_LISTA. O findRico também aceita variações sem prefixo.
const SANTOS_NUCLEO: Santo[] = [
  {
    slug: "sao-pedro",
    nome: "São Pedro",
    titulo: "Príncipe dos Apóstolos e primeiro Papa",
    data: "29/06",
    seculo: "I",
    padroeiro: "Pescadores, Papas, Roma",
    resumo: "Simão de Betsaida, pescador da Galileia, escolhido por Cristo como pedra da Igreja.",
    biografia:
      "Simão, filho de Jonas, nasceu em Betsaida e exercia a pesca em Cafarnaum com seu irmão André quando Jesus o chamou: 'Vinde após mim e farei de vós pescadores de homens'. Cristo lhe deu o nome novo de Cefas — Pedro — e sobre ele edificou a Igreja, confiando-lhe as chaves do Reino. Apesar da tríplice negação na noite da Paixão, recebeu, à beira do lago de Tiberíades, a tríplice missão de apascentar o rebanho. Em Pentecostes pregou o primeiro grande discurso cristão, converteu três mil pessoas e tornou-se cabeça visível da comunidade nascente. Presidiu o Concílio de Jerusalém, fundou a sé de Antioquia e, por fim, governou a Igreja de Roma, onde deixou duas epístolas canônicas. Foi crucificado de cabeça para baixo no circo de Nero, por volta do ano 64-67, por não se julgar digno de morrer como o Mestre. Seu túmulo está sob o altar da basílica vaticana que leva seu nome.",
    virtudes: ["Fé ardente", "Humildade após a queda", "Coragem apostólica", "Amor a Cristo"],
    frase: "Senhor, tu sabes tudo, tu sabes que eu te amo.",
  },
  {
    slug: "sao-paulo",
    nome: "São Paulo",
    titulo: "Apóstolo dos Gentios",
    data: "29/06",
    seculo: "I",
    padroeiro: "Missionários, teólogos, imprensa católica",
    resumo:
      "Saulo de Tarso, fariseu perseguidor convertido pela visão do Ressuscitado a caminho de Damasco.",
    biografia:
      "Nascido em Tarso da Cilícia por volta do ano 8, judeu da tribo de Benjamim e cidadão romano de nascimento, formou-se aos pés do rabino Gamaliel em Jerusalém. Zelosíssimo fariseu, perseguiu os cristãos e assistiu ao martírio de Estêvão. A caminho de Damasco, por volta do ano 34, foi derrubado pela luz de Cristo que lhe disse: 'Saulo, Saulo, por que me persegues?'. Batizado por Ananias, retirou-se na Arábia e iniciou a maior aventura missionária da história: três grandes viagens fundando comunidades em todo o Mediterrâneo, da Síria à Grécia, depois a Roma e, segundo a tradição, até as Espanhas. Escreveu treze epístolas que estruturam a teologia cristã: a justificação pela fé, a primazia da caridade, a Igreja como Corpo de Cristo. Preso, julgado e libertado, foi finalmente decapitado em Roma sob Nero, na via Ostiense, por volta do ano 67. Seus restos repousam na basílica de São Paulo Fora dos Muros.",
    virtudes: [
      "Zelo apostólico",
      "Caridade sobrenatural",
      "Constância nas tribulações",
      "Profundidade teológica",
    ],
    frase: "Já não sou eu que vivo, é Cristo que vive em mim.",
  },
  {
    slug: "santo-andre",
    nome: "Santo André",
    titulo: "Apóstolo, irmão de Pedro",
    data: "30/11",
    seculo: "I",
    padroeiro: "Grécia, Rússia, Escócia, pescadores",
    resumo: "Primeiro discípulo chamado por Cristo, conduziu Pedro ao Senhor.",
    biografia:
      "Pescador de Betsaida, irmão de Simão Pedro, foi inicialmente discípulo de João Batista. Quando o Precursor apontou Jesus dizendo 'Eis o Cordeiro de Deus', André seguiu o Mestre e correu a anunciar ao irmão: 'Encontramos o Messias'. Esse gesto inaugural lhe valeu na tradição grega o título de Protócletos, 'o primeiro chamado'. Acompanhou Jesus em todo o ministério público; foi ele quem apresentou ao Senhor o menino dos cinco pães e dois peixes antes da multiplicação. Após Pentecostes, evangelizou a Cítia, o Ponto e a Acaia, fundando comunidades cristãs nos confins do mundo grego. Em Patras, condenado pelo procônsul Egeias por converter sua esposa, foi martirizado por crucificação; segundo a tradição, recolhida nos Atos apócrifos de André, a cruz tinha forma de X — chamada desde então 'cruz de Santo André' — e ele, pendurado vivo por dois dias, continuou pregando à multidão até expirar, em torno do ano 60. É padroeiro nacional da Escócia, da Rússia e venerado com singular afeto pela Igreja ortodoxa de Constantinopla.",
    virtudes: ["Prontidão em seguir Cristo", "Espírito missionário", "Constância no martírio"],
    frase: "Salve, ó cruz, há tanto tempo desejada!",
  },
  {
    slug: "sao-tiago-maior",
    nome: "São Tiago Maior",
    titulo: "Apóstolo, filho de Zebedeu",
    data: "25/07",
    seculo: "I",
    padroeiro: "Espanha, peregrinos, cavaleiros",
    resumo: "Filho do trovão, primeiro apóstolo mártir, padroeiro de Compostela.",
    biografia:
      "Filho de Zebedeu e Salomé, irmão mais velho de João Evangelista, pescava no mar da Galileia quando Jesus o chamou junto com o irmão. Cristo lhes deu o sobrenome de Boanerges, 'filhos do trovão', pelo temperamento veemente. Pertenceu ao círculo íntimo dos três que assistiram à Transfiguração no Tabor, à ressurreição da filha de Jairo e à agonia do Getsêmani. A tradição hispânica, fixada desde o século VII, afirma que após Pentecostes evangelizou a Hispânia, deixando ali sementes da fé antes de retornar a Jerusalém. Foi decapitado por ordem de Herodes Agripa I por volta do ano 44, sendo o primeiro dos Doze a derramar o sangue por Cristo, como narra o livro dos Atos. Seus discípulos teriam trasladado o corpo por mar até a Galiza; o túmulo, identificado segundo essa tradição no século IX em Compostela, tornou-se o terceiro maior centro de peregrinação da cristandade depois de Jerusalém e Roma.",
    virtudes: ["Zelo ardoroso", "Fidelidade até o sangue", "Espírito missionário"],
    frase: "Podemos beber o cálice que tu vais beber.",
  },
  {
    slug: "sao-joao-evangelista",
    nome: "São João Evangelista",
    titulo: "Apóstolo, Evangelista, vidente de Patmos",
    data: "27/12",
    seculo: "I",
    padroeiro: "Teólogos, escritores, virgens",
    resumo: "O discípulo amado, águia entre os evangelistas, contemplador do Verbo.",
    biografia:
      "Filho de Zebedeu e Salomé, irmão menor de Tiago Maior, foi discípulo de João Batista antes de seguir Jesus. Único apóstolo permanecido junto à cruz, recebeu de Cristo a guarda de Maria: 'Eis aí tua mãe'. Após Pentecostes, viveu em Jerusalém junto à Virgem; depois fixou-se em Éfeso, onde governou as Igrejas da Ásia Menor. Sob Domiciano foi exilado na ilha grega de Patmos, onde recebeu as visões do Apocalipse. Voltando a Éfeso já ancião, é tradicionalmente tido como autor do quarto Evangelho — o mais teológico, que abre com o hino ao Verbo eterno — e de três epístolas centradas no mandamento novo do amor, embora a crítica bíblica moderna discuta os detalhes dessa autoria. A tradição relata que, já idoso e carregado pelos discípulos, repetia incansável: 'Filhinhos, amai-vos uns aos outros'. Único apóstolo a morrer de morte natural, por volta do ano 100, em idade muito avançada. Sua teologia da Encarnação, da luz e da caridade lhe valeu o símbolo da águia que voa direto ao sol divino.",
    virtudes: ["Virgindade", "Contemplação", "Caridade fraterna", "Fidelidade ao Mestre"],
    frase: "Deus é amor; quem permanece no amor permanece em Deus.",
  },
  {
    slug: "sao-felipe",
    nome: "São Felipe",
    titulo: "Apóstolo",
    data: "03/05",
    seculo: "I",
    padroeiro: "Pasteleiros, Uruguai",
    resumo: "Apóstolo de Betsaida, evangelizador da Ásia Menor.",
    biografia:
      "Originário de Betsaida, mesma cidade de Pedro e André, foi pessoalmente chamado por Cristo com as palavras 'Segue-me' e imediatamente conduziu Natanael ao Mestre. Foi ele quem, antes da multiplicação dos pães, calculou que duzentos denários de pão não bastariam, e quem, na noite da Última Ceia, pediu: 'Senhor, mostra-nos o Pai'. Ouviu então a célebre resposta: 'Quem me viu, viu o Pai'. Após Pentecostes evangelizou a Frígia, na atual Turquia, e uma tradição antiga, ligada ao testemunho de Papias e às escavações de Hierápolis, associa-o àquela cidade, onde a hagiografia narra a conversão de numerosos pagãos afastados do culto a uma serpente. Segundo a tradição hagiográfica, teria morrido mártir em idade avançada, por crucificação ou apedrejamento, por volta do ano 80 — as fontes divergem quanto à forma. Suas relíquias, com as de Tiago Menor, repousam na basílica romana dos Santos Apóstolos. Não deve ser confundido com Filipe, o diácono, dos Atos dos Apóstolos.",
    virtudes: ["Simplicidade", "Busca sincera da verdade", "Coragem missionária"],
    frase: "Senhor, mostra-nos o Pai e isso nos basta.",
  },
  {
    slug: "sao-bartolomeu",
    nome: "São Bartolomeu",
    titulo: "Apóstolo",
    data: "24/08",
    seculo: "I",
    padroeiro: "Curtidores, açougueiros, Armênia",
    resumo: "Identificado com Natanael, israelita sem dolo, mártir esfolado vivo.",
    biografia:
      "Bartolomeu — 'filho de Talmai' — é tradicionalmente identificado com Natanael de Caná da Galileia, a quem Felipe levou a Jesus. Cético quanto à origem do Mestre ('Pode vir algo de bom de Nazaré?'), foi conquistado quando Cristo lhe revelou ter-lhe visto sob a figueira, e proclamou: 'Tu és o Filho de Deus, o Rei de Israel'. Jesus o elogiou como verdadeiro israelita em que não há falsidade. Após Pentecostes, segundo Eusébio de Cesareia, levou o Evangelho até a Índia, deixando ali um exemplar do Evangelho de Mateus em hebraico. Pregou ainda na Mesopotâmia, Pérsia e, sobretudo, na Armênia, onde, segundo a tradição hagiográfica, converteu o rei Polímio. Por instigação do irmão deste, teria sido preso e, segundo essa mesma tradição, esfolado vivo e decapitado em Albanópolis, por volta do ano 70 — dados que a Igreja transmite como tradição antiga e não como registro histórico documentado. A imagem do santo segurando a própria pele, esculpida por Marco d'Agrate no Duomo de Milão, é célebre exemplo de iconografia martirial.",
    virtudes: ["Retidão de coração", "Estudo das Escrituras", "Constância no martírio"],
    frase: "Rabi, tu és o Filho de Deus, tu és o Rei de Israel.",
  },
  {
    slug: "sao-tome",
    nome: "São Tomé",
    titulo: "Apóstolo",
    data: "03/07",
    seculo: "I",
    padroeiro: "Índia, arquitetos, incrédulos que buscam a fé",
    resumo: "O que duvidou e depois confessou: 'Meu Senhor e meu Deus'.",
    biografia:
      "Chamado Dídimo, 'gêmeo', foi um dos Doze. Quando Jesus decidiu subir à Judeia para ressuscitar Lázaro, apesar do perigo, Tomé exortou os companheiros: 'Vamos também nós, para morrer com ele'. Na Última Ceia confessou a ignorância do caminho, e ouviu a resposta sublime: 'Eu sou o caminho, a verdade e a vida'. Ausente da primeira aparição do Ressuscitado, recusou crer sem ver e tocar; oito dias depois, posto diante das chagas, prostrou-se na maior confissão de fé do Evangelho: 'Meu Senhor e meu Deus'. Uma antiga e importante tradição cristã — atestada em fontes siríacas e vivida até hoje pela comunidade dos 'cristãos de São Tomé', no Malabar — associa o Apóstolo à evangelização dos partos, persas e medos e, sobretudo, da Índia, e ao seu martírio por lança em Mailapur, perto da atual Chennai, por volta do ano 72. A tradição da missão indiana é muito antiga e amplamente aceita, embora os detalhes da pregação e do martírio provenham de relatos hagiográficos, não de documentação histórica direta. Suas relíquias foram veneradas em Edessa e, depois, em Ortona, na Itália.",
    virtudes: ["Sinceridade", "Coragem na adversidade", "Adoração reparadora"],
    frase: "Meu Senhor e meu Deus!",
  },
  {
    slug: "sao-mateus",
    nome: "São Mateus",
    titulo: "Apóstolo e Evangelista",
    data: "21/09",
    seculo: "I",
    padroeiro: "Banqueiros, contadores, fiscais",
    resumo: "Publicano de Cafarnaum chamado da mesa do imposto à mesa do Reino.",
    biografia:
      "Levi, filho de Alfeu, cobrava impostos para Roma na alfândega de Cafarnaum — ofício odiado e classificado entre os pecadores públicos. Jesus, passando, lhe disse apenas: 'Segue-me'. Mateus deixou tudo no instante, organizou um banquete em sua casa convidando colegas publicanos, e provocou o murmúrio dos fariseus, ouvindo a célebre resposta do Mestre: 'Não são os sãos que precisam de médico, mas os enfermos'. Foi um dos Doze e a tradição eclesial mais antiga lhe atribui, após Pentecostes, a autoria do primeiro Evangelho, possivelmente redigido originalmente em aramaico — ordenado segundo cinco grandes discursos e voltado a leitores judeu-cristãos para demonstrar que Jesus é o Messias prometido, cumprimento de toda a Lei e dos Profetas. Pregou na Judeia; sobre a missão posterior as tradições divergem — umas o levam à Etiópia, outras à Pérsia — e o martírio ao pé do altar, enquanto celebrava, pertence à tradição hagiográfica, sem confirmação histórica. Seu símbolo é o homem alado, porque inicia o Evangelho pela genealogia humana de Cristo.",
    virtudes: ["Conversão radical", "Desapego dos bens", "Gratidão pela misericórdia"],
    frase: "Misericórdia quero, e não sacrifício.",
  },
  {
    slug: "sao-tiago-menor",
    nome: "São Tiago Menor",
    titulo: "Apóstolo e primeiro bispo de Jerusalém",
    data: "03/05",
    seculo: "I",
    padroeiro: "Moribundos, chapeleiros",
    resumo: "Parente do Senhor, autor de uma epístola, mártir em Jerusalém.",
    biografia:
      "Filho de Alfeu (Cléofas) e de Maria, chamada na tradição 'irmã' da Virgem — ou seja, parente próxima — é dito 'Menor' por ser mais novo ou mais baixo que o homônimo filho de Zebedeu. Apóstolo entre os Doze, tornou-se após Pentecostes o primeiro bispo de Jerusalém, presidindo a comunidade-mãe da cristandade por cerca de trinta anos. No Concílio de Jerusalém, em torno do ano 49, conduziu com Pedro a decisão que dispensava os gentios da circuncisão, formulando a sentença normativa que abriu a Igreja à universalidade. É tradicionalmente tido como autor de uma epístola católica famosa pelo equilíbrio entre fé e obras: 'A fé sem obras é morta'. Reverenciado até por muitos judeus pela austeridade de vida, foi chamado 'o Justo' e tinha os joelhos calejados pela oração. Seu martírio em Jerusalém, por volta do ano 62, é atestado por Flávio Josefo e por Eusébio de Cesareia, que cita Hegésipo; segundo esse relato tradicional, foi precipitado do pináculo do Templo e, ainda vivo, apedrejado e morto a golpes de bastão — os detalhes provêm da narrativa de Hegésipo, enquanto o fato do martírio é historicamente bem atestado.",
    virtudes: ["Justiça", "Oração contínua", "Coerência entre fé e vida"],
    frase: "Mostra-me a tua fé sem as obras, e eu, pelas obras, te mostrarei a minha fé.",
  },
  {
    slug: "sao-judas-tadeu",
    nome: "São Judas Tadeu",
    titulo: "Apóstolo",
    data: "28/10",
    seculo: "I",
    padroeiro: "Causas difíceis e desesperadas",
    resumo: "O apóstolo a quem se recorre quando tudo parece perdido.",
    biografia:
      "Filho de Cléofas e Maria, irmão de Tiago Menor e parente do Senhor segundo a carne, é chamado Tadeu — 'o de peito largo, magnânimo' — para distingui-lo de Judas Iscariotes. Na Última Ceia perguntou a Jesus por que se manifestaria aos discípulos e não ao mundo, recebendo a promessa de que o Pai e o Filho viriam fazer morada em quem os amasse. Após Pentecostes pregou na Judeia, na Samaria, na Mesopotâmia e, sobretudo, na Pérsia e na Armênia, em companhia de Simão Zelote. É tradicionalmente identificado como autor de uma breve mas vigorosa epístola que combate falsos mestres e exorta à perseverança na fé recebida 'de uma vez por todas'. Segundo a tradição hagiográfica, foi martirizado na Pérsia por volta do ano 70, morto a clavadas ou golpes de machado — daí o instrumento que costuma carregar nas imagens —, sem que as fontes históricas permitam confirmar local e forma. A devoção popular como advogado das causas difíceis é tradicionalmente ligada a escritos atribuídos a Santa Brígida e a São Bernardo, difundiu-se sobretudo a partir do século XVIII e firmou-se largamente nas Américas.",
    virtudes: ["Esperança contra toda esperança", "Compaixão pelos aflitos", "Coragem no anúncio"],
    frase: "Edificai-vos sobre a vossa santíssima fé.",
  },
  {
    slug: "sao-simao-zelote",
    nome: "São Simão Zelote",
    titulo: "Apóstolo",
    data: "28/10",
    seculo: "I",
    padroeiro: "Curtidores, lenhadores",
    resumo: "O cananeu zeloso pela Lei convertido em zeloso pelo Reino.",
    biografia:
      "Chamado 'Zelote' por pertencer, antes do encontro com Jesus, ao movimento dos zelotas — judeus radicais que aguardavam um Messias libertador armado contra Roma — Simão converteu seu zelo pela Lei em zelo pelo Evangelho. Distingue-se de Simão Pedro e é mencionado entre os Doze em todas as listas apostólicas. Sobre sua missão após Pentecostes as fontes históricas silenciam: é a tradição hagiográfica ocidental que o associa a Judas Tadeu na evangelização do Egito, da Mesopotâmia e da Pérsia, e que narra o martírio em Suanir, por volta do ano 70, na forma de ser serrado ao meio — daí a serra que figura como seu atributo iconográfico. Outras tradições divergem quanto ao lugar e à forma do martírio, de modo que nenhuma delas pode ser apresentada como fato historicamente comprovado. A veneração conjunta com Judas Tadeu, cuja festa partilha no calendário romano, e as relíquias honradas num altar da basílica de São Pedro são, essas sim, dados litúrgicos e documentados.",
    virtudes: ["Zelo pela glória de Deus", "Espírito missionário", "Coragem no martírio"],
    frase: "O zelo pela tua casa me consome.",
  },
  {
    slug: "sao-matias",
    nome: "São Matias",
    titulo: "Apóstolo",
    data: "14/05",
    seculo: "I",
    padroeiro: "Alcoólatras em recuperação, carpinteiros",
    resumo: "Eleito por sorte para completar o número dos Doze após a queda de Judas.",
    biografia:
      "Discípulo desde o batismo de João, acompanhara Jesus durante todo o ministério público, sem pertencer aos Doze. Após a Ascensão, Pedro propôs que se completasse o colégio apostólico para preservar o número simbólico das doze tribos. Entre dois candidatos — José Barsabás e Matias — orou a comunidade e, lançada a sorte segundo o costume bíblico, foi escolhido Matias, que recebeu então o Espírito Santo em Pentecostes. Pregou primeiramente na Judeia, depois, segundo as tradições, na Capadócia e nas margens do mar Cáspio, entre os bárbaros etíopes ou colquidianos, com extraordinária paciência diante de povos hostis. As tradições sobre seu martírio divergem: segundo a versão grega, foi apedrejado e depois decapitado; segundo a tradição latina, morreu crucificado — sem que se possa afirmar historicamente qual delas é exata. Suas relíquias teriam sido levadas por Santa Helena para Tréveris, na atual Alemanha, onde se conservam até hoje na abadia beneditina de São Matias, único túmulo apostólico ao norte dos Alpes.",
    virtudes: ["Humildade", "Fidelidade discreta", "Constância no anúncio"],
    frase: "Que outro receba o seu ministério.",
  },
  {
    slug: "sao-marcos-evangelista",
    nome: "São Marcos Evangelista",
    titulo: "Evangelista, discípulo de Pedro",
    data: "25/04",
    seculo: "I",
    padroeiro: "Veneza, secretários, advogados",
    resumo:
      "O 'intérprete' de Pedro, autor do Evangelho mais antigo e fundador da Igreja de Alexandria.",
    biografia:
      "João Marcos era judeu de Jerusalém; sua mãe Maria abriu a casa à comunidade primitiva — foi lá que Pedro acudiu ao sair milagrosamente do cárcere. Acompanhou Paulo e Barnabé na primeira viagem missionária, da qual se separou em Panfília; tornou-se depois colaborador fidelíssimo de Pedro em Roma, onde o chama 'meu filho'. Recolhendo a catequese viva do apóstolo, é tradicionalmente considerado o redator, por volta dos anos 60, do segundo Evangelho — o mais breve e o mais antigo —, vibrante de detalhes concretos e centrado no mistério de Cristo que se desvela na cruz como Filho de Deus. Segundo a tradição eclesiástica antiga, recolhida por Eusébio de Cesareia, foi enviado a Alexandria, onde teria fundado uma das maiores Igrejas da antiguidade, ligada à célebre escola catequética que daria à Igreja Clemente e Orígenes; a mesma tradição situa seu martírio em 68, arrastado por cordas pelas ruas até a morte. Suas relíquias foram levadas em 828 para Veneza, da qual é principal padroeiro; seu símbolo é o leão alado, porque seu Evangelho começa com a voz que clama no deserto.",
    virtudes: ["Fidelidade", "Discrição", "Espírito missionário"],
    frase: "Princípio do Evangelho de Jesus Cristo, Filho de Deus.",
  },
  {
    slug: "sao-lucas-evangelista",
    nome: "São Lucas Evangelista",
    titulo: "Evangelista, médico, companheiro de Paulo",
    data: "18/10",
    seculo: "I",
    padroeiro: "Médicos, pintores, cirurgiões, artistas",
    resumo: "Gentio convertido, historiador delicado do Evangelho da Misericórdia.",
    biografia:
      "Sírio de Antioquia, médico de profissão e provavelmente convertido por Paulo, é o único evangelista gentio. Acompanhou o Apóstolo na segunda e na terceira viagens missionárias e permaneceu com ele nos cativeiros de Cesareia e Roma — 'Lucas, o caríssimo médico, te saúda', escreve Paulo. É tradicionalmente identificado como autor do terceiro Evangelho, fruto de cuidadosa pesquisa junto às testemunhas oculares, e do livro dos Atos dos Apóstolos, ambos dedicados a Teófilo. Sua obra é o Evangelho da misericórdia (parábolas do bom samaritano, do filho pródigo, do fariseu e do publicano), das mulheres, dos pobres e da oração; nele resplandece o rosto materno de Maria, cuja infância de Jesus ele conserva como mais ninguém — daí a tradição que lhe atribui ter pintado a primeira imagem da Virgem. Após o martírio de Paulo, evangelizou a Acaia, a Beócia e a Bitínia. Morreu em idade avançada na Boécia, segundo alguns mártir, segundo outros sereno na velhice. Seu símbolo é o touro alado, animal de sacrifício, porque seu Evangelho abre no templo, com a oferenda de Zacarias.",
    virtudes: ["Compaixão", "Rigor histórico", "Devoção a Maria"],
    frase: "Hoje vos nasceu, na cidade de Davi, um Salvador, que é o Cristo Senhor.",
  },
  {
    slug: "sao-estevao",
    nome: "São Estêvão",
    titulo: "Protomártir, primeiro diácono",
    data: "26/12",
    seculo: "I",
    padroeiro: "Diáconos, pedreiros, perseguidos",
    resumo: "O primeiro cristão a derramar o sangue por Cristo, perdoando os algozes.",
    biografia:
      "'Cheio de fé e do Espírito Santo', foi escolhido entre os sete primeiros diáconos para o serviço das mesas na Igreja de Jerusalém, libertando os apóstolos para a pregação. Operava grandes prodígios e disputava nas sinagogas dos helenistas com tal sabedoria que ninguém lhe podia resistir. Acusado falsamente de blasfemar contra Moisés e o Templo, foi arrastado ao Sinédrio, onde pronunciou um discurso magnífico, percorrendo toda a história da salvação para mostrar que Cristo é cumprimento das promessas e que o povo havia matado os profetas e o Justo. Diante da fúria dos juízes, contemplou os céus abertos e viu o Filho do Homem à direita de Deus. Arrastado para fora dos muros, foi apedrejado por volta do ano 35 — Saulo de Tarso guardava as vestes dos executores. De joelhos, repetindo as palavras do Mestre, perdoou aos algozes: 'Senhor, não lhes imputeis este pecado'. Sua morte preparou a conversão de Paulo e tornou-o o primeiro mártir da Igreja, o protomártir.",
    virtudes: ["Coragem profética", "Misericórdia para com os inimigos", "Visão sobrenatural"],
    frase: "Senhor Jesus, recebe o meu espírito.",
  },
  {
    slug: "sao-jose",
    nome: "São José",
    titulo: "Esposo da Virgem Maria, padroeiro da Igreja universal",
    data: "19/03",
    seculo: "I",
    padroeiro: "Igreja universal, trabalhadores, pais de família, boa morte",
    resumo: "O homem justo a quem o Pai confiou o Filho e a Mãe.",
    biografia:
      "Carpinteiro de Nazaré, descendente da casa de Davi, foi escolhido por Deus para ser o pai virginal e legal de Jesus e o esposo da Virgem Maria. Sua justiça resplandece no silêncio: avisado em sonho de que o Filho concebido em Maria era do Espírito Santo, acolheu-a sem hesitação, deu o nome ao Menino, conduziu-os a Belém para o censo, fugiu de noite com eles para o Egito ante a fúria de Herodes e os trouxe de volta a Nazaré. Ali trabalhou com as mãos para sustentar a Sagrada Família e ensinou ao Filho de Deus o ofício humilde da carpintaria. Está presente no episódio do menino perdido e reencontrado no Templo, depois desaparece dos Evangelhos: a tradição o tem por morto antes da vida pública de Jesus, nos braços de Maria e do próprio Filho — daí ser invocado como padroeiro da boa morte. Pio IX o declarou padroeiro da Igreja universal em 1870; São João Paulo II lhe dedicou a exortação 'Redemptoris Custos', e o Papa Francisco proclamou o Ano de São José em 2020.",
    virtudes: [
      "Justiça",
      "Silêncio orante",
      "Obediência aos sonhos de Deus",
      "Trabalho santificado",
    ],
    frase: "Fez tudo como o Anjo do Senhor lhe ordenara.",
  },
  {
    slug: "sao-joao-batista",
    nome: "São João Batista",
    titulo: "Precursor do Messias",
    data: "24/06",
    seculo: "I",
    padroeiro: "Batistério, conversão dos pecadores, autoestrada",
    resumo: "A voz que clama no deserto, último profeta da Antiga Aliança.",
    biografia:
      "Filho do sacerdote Zacarias e de Isabel, parenta de Maria, foi concebido milagrosamente na velhice dos pais e saltou de alegria no seio materno à voz da Virgem que trazia o Verbo encarnado. Ainda jovem retirou-se ao deserto da Judeia, vestido de pelo de camelo, alimentando-se de gafanhotos e mel silvestre. Por volta do ano 27 começou a pregar nas margens do Jordão um batismo de penitência em vista do Messias iminente; multidões acorriam, e os fariseus eram chamados 'raça de víboras'. Batizou o próprio Cristo, sobre quem viu descer o Espírito em forma de pomba, e o apontou como 'Cordeiro de Deus que tira o pecado do mundo'. Defendeu publicamente a indissolubilidade do matrimônio diante de Herodes Antipas, que vivia com Herodíades, esposa do irmão. Preso na fortaleza de Maqueronte, foi decapitado a pedido de Salomé, por volta do ano 29. Jesus testemunhou dele: 'Entre os nascidos de mulher, ninguém maior que João Batista'.",
    virtudes: ["Austeridade", "Humildade ('Ele deve crescer, eu diminuir')", "Coragem profética"],
    frase: "Eis o Cordeiro de Deus, que tira o pecado do mundo.",
  },
  {
    slug: "santa-ana",
    nome: "Santa Ana",
    titulo: "Mãe da Virgem Maria",
    data: "26/07",
    seculo: "I a.C.",
    padroeiro: "Avós, mães, mulheres em trabalho de parto, mineiros",
    resumo: "A avó do Salvador, modelo de fecundidade alcançada pela oração.",
    biografia:
      "Conhecida não pelas Escrituras canônicas, mas pela tradição cristã e pelo Protoevangelho de Tiago — texto apócrifo do século II —, Ana — cujo nome significa 'graça' — era esposa de Joaquim, descendente da tribo de Judá. Casados há muitos anos sem filhos, suportaram com paciência a humilhação pública da esterilidade, considerada então sinal de reprovação divina. Joaquim retirou-se ao deserto em jejum por quarenta dias, enquanto Ana, em casa, pediu a Deus em lágrimas o dom de um filho, prometendo consagrá-lo ao Templo. Um anjo lhes anunciou que teriam uma filha cujo nome seria celebrado em todas as gerações; nasceu Maria, a Imaculada. Quando a Virgem tinha três anos, os pais a conduziram ao Templo para a apresentação, cumprindo o voto. A devoção a Sant'Ana floresceu cedo no Oriente e propagou-se ao Ocidente a partir do século VI; sua basílica em Jerusalém, junto à piscina probática, marca, segundo a tradição, o lugar de nascimento de Maria. É amada como padroeira das avós e celestial protetora das mães que esperam um filho.",
    virtudes: ["Perseverança na oração", "Esperança paciente", "Maternidade santa"],
    frase: "O Senhor visitou o seu povo.",
  },
  {
    slug: "sao-joaquim",
    nome: "São Joaquim",
    titulo: "Pai da Virgem Maria",
    data: "26/07",
    seculo: "I a.C.",
    padroeiro: "Avôs, pais de família",
    resumo: "Modelo de fidelidade conjugal e de paternidade santificada pela espera.",
    biografia:
      "Esposo de Ana e pai da Virgem Maria, conhecido, como ela, sobretudo pelo apócrifo Protoevangelho de Tiago e pela tradição cristã posterior — não pelas Escrituras canônicas —, era, segundo essa tradição, descendente de Davi e habitante de Nazaré ou Jerusalém. Homem rico e piedoso, dividia seus bens em três partes: uma para o Templo, outra para os pobres e a terceira para a casa. Sua esterilidade prolongada foi causa de zombaria pública; a recusa de sua oferenda no Templo o levou ao deserto, onde jejuou e suplicou durante quarenta dias. Um anjo lhe anunciou a futura concepção da filha, e ele voltou para encontrar Ana, conforme o sinal celeste, junto à Porta Dourada de Jerusalém — cena tantas vezes representada na arte cristã, sobretudo por Giotto na capela Scrovegni. Educou Maria nas Sagradas Escrituras e a apresentou ao Templo segundo o voto materno. A tradição o coloca já falecido quando da Anunciação. A devoção a Joaquim, antiquíssima no Oriente, foi inserida no calendário romano por Júlio II no século XVI e unida à festa de Santa Ana por Paulo VI em 1969.",
    virtudes: ["Justiça", "Esperança na promessa", "Generosidade com os pobres"],
    frase: "Bendito seja o Deus de Israel, que visitou e libertou o seu povo.",
  },
  {
    slug: "santa-maria-madalena",
    nome: "Santa Maria Madalena",
    titulo: "Discípula amada, 'apóstola dos apóstolos'",
    data: "22/07",
    seculo: "I",
    padroeiro: "Penitentes, perfumistas, mulheres convertidas",
    resumo: "Primeira testemunha do Ressuscitado, enviada a anunciar a Páscoa aos Doze.",
    biografia:
      "Originária da próspera Magdala, à beira do mar da Galileia, foi libertada por Jesus de sete demônios e tornou-se uma das mulheres que o seguiam com seus bens. Acompanhou o Mestre até o pé da cruz junto à Virgem e a outra Maria, presenciou a sepultura e correu de madrugada ao túmulo no primeiro dia da semana para ungir o corpo. Encontrou a pedra removida e, em lágrimas, recebeu a aparição do Ressuscitado, que a chamou pelo nome: 'Maria!'. Recebeu então a missão de ir anunciar aos apóstolos a Ressurreição — o que lhe valeu nos Padres o título de 'apóstola dos apóstolos'. A tradição ocidental, a partir de Gregório Magno, a identificou com a pecadora que ungiu os pés de Jesus em casa de Simão e com Maria de Betânia, identificação não admitida pelo Oriente nem hoje sustentada pela exegese. A tradição provençal afirma que evangelizou a região de Marselha e terminou os dias em penitência na gruta de Sainte-Baume. Em 2016, o Papa Francisco elevou sua memória a festa litúrgica.",
    virtudes: ["Amor ardente", "Conversão profunda", "Fidelidade até a cruz"],
    frase: "Vi o Senhor!",
  },
  {
    slug: "santa-marta",
    nome: "Santa Marta",
    titulo: "Hospedeira do Senhor",
    data: "29/07",
    seculo: "I",
    padroeiro: "Cozinheiras, donas de casa, hoteleiros",
    resumo: "Irmã de Lázaro e de Maria de Betânia, modelo do serviço atento.",
    biografia:
      "Habitava em Betânia, a poucos quilômetros de Jerusalém, com os irmãos Lázaro e Maria. Várias vezes acolheu Jesus em sua casa, servindo-o com diligência. No episódio mais célebre, queixou-se de que a irmã apenas escutava o Mestre sentada aos seus pés; ouviu então a delicada repreensão: 'Marta, Marta, andas inquieta com muitas coisas, mas uma só é necessária'. Tornou-se, porém, símbolo da fé operosa: diante do túmulo de Lázaro, fez a maior confissão de fé feminina do Evangelho — 'Sim, Senhor, eu creio que tu és o Cristo, o Filho de Deus, que devia vir ao mundo' — antes mesmo da ressurreição do irmão. Esteve presente no banquete em que Maria ungiu os pés de Jesus, seis dias antes da Páscoa. Segundo a tradição provençal, após Pentecostes evangelizou com os irmãos a região do Ródano, em torno de Tarascon, onde teria amansado um dragão e morrido em paz, em idade avançada. Em 2021 sua memória foi unida na liturgia à de Maria e Lázaro.",
    virtudes: ["Hospitalidade", "Fé corajosa", "Serviço diligente"],
    frase: "Sim, Senhor, eu creio que tu és o Cristo, o Filho de Deus.",
  },
  {
    slug: "santo-inacio-de-antioquia",
    nome: "Santo Inácio de Antioquia",
    titulo: "Bispo, mártir, padre apostólico",
    data: "17/10",
    seculo: "I–II",
    padroeiro: "Igreja do Oriente, catequistas",
    resumo:
      "Bispo da Síria devorado pelos leões em Roma c. 107, autor de sete cartas fundadoras da teologia eclesial.",
    biografia:
      "Discípulo direto dos apóstolos — segundo a tradição, de João — sucedeu Evódio como bispo de Antioquia, a terceira maior Igreja da cristandade, governando-a por cerca de quarenta anos. Sob Trajano foi condenado às feras e conduzido sob escolta militar de Antioquia a Roma, atravessando longo trajeto pela Ásia Menor. Durante o percurso escreveu sete cartas memoráveis — às Igrejas de Éfeso, Magnésia, Trales, Roma, Filadélfia, Esmirna e ao bispo Policarpo —, hoje pedras fundamentais do depósito patrístico. Nelas afirma pela primeira vez o nome 'Igreja católica', explicita a estrutura hierárquica em torno do bispo (assistido por presbíteros e diáconos), defende a verdade da Encarnação contra os docetas e proclama a Eucaristia como 'remédio de imortalidade'. À comunidade romana suplica que não interceda por sua libertação: 'Sou trigo de Deus; que eu seja moído pelos dentes das feras para tornar-me pão puro de Cristo'. Foi devorado no Coliseu por volta de 107.",
    virtudes: ["Amor a Cristo até o sangue", "Senso eclesial", "Coragem episcopal"],
    frase: "Procurai apenas o meu amante, que é Cristo.",
  },
  {
    slug: "sao-policarpo",
    nome: "São Policarpo",
    titulo: "Bispo de Esmirna, mártir",
    data: "23/02",
    seculo: "I–II",
    padroeiro: "Contra otites, contra disenteria",
    resumo: "Discípulo de São João, elo vivo entre os apóstolos e a Igreja patrística.",
    biografia:
      "Nascido por volta de 69, foi discípulo direto do apóstolo João, que o ordenou bispo de Esmirna, na Ásia Menor. Combateu energicamente as primeiras heresias gnósticas, particularmente as de Márcion e Valentino, e ensinou Santo Ireneu de Lião, formando assim um continuum entre o testemunho apostólico e a grande teologia do século II. Visitou em Roma o papa Aniceto, com quem dialogou sobre a data da Páscoa sem chegar a acordo, mas selando comunhão eucarística — modelo de unidade na diversidade. Escreveu uma Carta aos Filipenses, ainda conservada. Aos 86 anos, sob Marco Aurélio, foi preso durante uma perseguição em Esmirna. Levado ao estádio, recusou-se a renegar Cristo com a célebre resposta: 'Há oitenta e seis anos sirvo o meu Rei, e ele nunca me fez mal algum: como poderia agora blasfemar do meu Salvador?'. Foi queimado vivo na fogueira em 155 ou 156; as chamas formaram um arco em torno dele, e finalmente foi traspassado pela espada. O 'Martírio de Policarpo' é o primeiro relato autêntico de martírio fora do Novo Testamento.",
    virtudes: ["Fidelidade ao depósito da fé", "Caridade pastoral", "Coragem serena"],
    frase: "Há oitenta e seis anos sirvo a Cristo, e ele nunca me fez mal algum.",
  },
  {
    slug: "sao-justino",
    nome: "São Justino",
    titulo: "Filósofo e mártir, primeiro apologista",
    data: "01/06",
    seculo: "II",
    padroeiro: "Filósofos, apologistas",
    resumo: "Buscou a verdade nas escolas pagãs e encontrou-a em Cristo, Verbo eterno.",
    biografia:
      "Nascido em Flávia Neápolis (Siquém), na Samaria, por volta de 100, de família pagã grega. Percorreu sucessivamente as escolas estoica, peripatética, pitagórica e platônica em busca da verdade, sem encontrar saciedade. Conheceu o cristianismo por meio de um ancião à beira-mar que lhe falou dos profetas e de Cristo, 'a única filosofia segura e proveitosa'. Convertido por volta de 130, manteve a túnica do filósofo como sinal de que o cristianismo é a 'verdadeira filosofia'. Abriu escola em Roma e escreveu duas Apologias dirigidas a Antonino Pio e ao Senado em defesa dos cristãos, e o Diálogo com Trifão, longa discussão com um rabino sobre o cumprimento das Escrituras em Cristo. Suas obras conservam a mais antiga descrição litúrgica da Eucaristia. Denunciado pelo filósofo cínico Crescente, foi decapitado em Roma sob Marco Aurélio, por volta de 165, com seis companheiros — sua paixão autêntica é um dos clássicos da literatura martirial.",
    virtudes: ["Amor à verdade", "Diálogo com a cultura", "Fidelidade racional à fé"],
    frase: "Tudo o que de verdadeiro foi dito por qualquer pessoa pertence a nós, cristãos.",
  },
  {
    slug: "sao-cipriano-de-cartago",
    nome: "São Cipriano de Cartago",
    titulo: "Bispo, mártir, doutor da eclesiologia",
    data: "16/09",
    seculo: "III",
    padroeiro: "África, advogados",
    resumo:
      "Bispo africano da unidade da Igreja, primeiro grande mártir episcopal do Ocidente latino.",
    biografia:
      "Tascio Cecílio Cipriano nasceu em Cartago por volta de 210, em família pagã abastada; foi advogado e mestre de retórica antes de se converter por volta de 246, sob influência do presbítero Cecílio. Eleito bispo de Cartago apenas dois anos depois, governou com pulso firme em meio às perseguições de Décio e Valeriano. Refugiou-se durante a primeira para conduzir a Igreja por cartas, e enfrentou o cisma dos 'lapsos' — fiéis que tinham apostatado sob tortura — com firmeza e misericórdia, fixando o uso da penitência canônica. Combateu o rigorismo de Novaciano e formulou clássicas teses sobre o batismo e a unidade eclesial em obras como 'De unitate Ecclesiae', onde lê-se a célebre sentença 'Salus extra Ecclesiam non est'. Escreveu também tratados sobre o Pai-Nosso, o vestuário das virgens, a mortalidade e cerca de 80 cartas. Sob Valeriano foi exilado e, recusando-se a sacrificar aos deuses, decapitado em 14 de setembro de 258, na vinha de Sexto, diante de seu povo — primeiro bispo mártir da África.",
    virtudes: ["Senso de Igreja", "Coragem pastoral", "Misericórdia equilibrada com firmeza"],
    frase: "Não pode ter Deus por Pai quem não tem a Igreja por mãe.",
  },
  {
    slug: "sao-lourenco",
    nome: "São Lourenço",
    titulo: "Diácono e mártir de Roma",
    data: "10/08",
    seculo: "III",
    padroeiro: "Cozinheiros, bibliotecários, pobres",
    resumo: "Apresentou os pobres como o verdadeiro tesouro da Igreja e morreu na grelha.",
    biografia:
      "Diácono de Roma e arquidiácono do papa Sisto II, era o responsável pela administração dos bens eclesiásticos e pela assistência aos pobres da Urbe. Em agosto de 258, durante a perseguição de Valeriano, viu o próprio papa ser decapitado e ouviu-lhe a profecia: 'Tu me seguirás em três dias'. Intimado pelo prefeito de Roma a entregar os tesouros da Igreja, Lourenço pediu três dias para reuni-los; distribuiu então tudo o que havia aos necessitados e, no terceiro dia, apresentou ao magistrado a multidão dos pobres, viúvas e enfermos: 'Eis aqui os tesouros da Igreja'. Enfurecido, o prefeito o condenou a um suplício prolongado: foi estendido sobre uma grelha de ferro aquecida. A tradição lhe atribui o gracejo de heroica serenidade ao algoz: 'Já estou assado deste lado, manda virar e come'. Morreu em 10 de agosto de 258. Sua basílica romana extramuros, edificada por Constantino sobre o túmulo, é uma das sete grandes basílicas estacionais.",
    virtudes: ["Amor preferencial pelos pobres", "Alegria no martírio", "Fidelidade ao Papa"],
    frase: "Os pobres são os verdadeiros tesouros da Igreja.",
  },
  {
    slug: "santa-cecilia",
    nome: "Santa Cecília",
    titulo: "Virgem e mártir, padroeira da música sacra",
    data: "22/11",
    seculo: "III",
    padroeiro: "Músicos, cantores, organistas, luthiers",
    resumo: "Jovem patrícia romana que cantava ao Senhor em seu coração ainda durante o martírio.",
    biografia:
      "Nascida em ilustre família romana no final do século II, consagrou, segundo a Paixão de Santa Cecília — texto hagiográfico dos sécs. V–VI de onde provém tudo o que dela se narra —, desde a infância sua virgindade a Cristo. Obrigada pelos pais a se casar com o jovem patrício pagão Valeriano, na noite do casamento revelou-lhe seu voto e o anjo que guardava sua pureza. Valeriano pediu para vê-lo e, ao ser remetido à fonte da Via Ápia, foi batizado pelo papa Urbano I; convertido com o irmão Tibúrcio, ambos foram martirizados pouco depois por sepultar os corpos dos mártires. Cecília prosseguiu a obra catequética convertendo mais de quatrocentas pessoas, que recebeu o batismo de Urbano. Presa e condenada a sufocar nos vapores de seu próprio balneário, saiu ilesa; o carrasco enviado para decapitá-la desferiu três golpes sem conseguir matá-la, e ela viveu ainda três dias, distribuindo aos pobres seus bens. Foi sepultada nas catacumbas de São Calisto; seu corpo, exumado em 1599 incorrupto e na posição em que jazia, inspirou a célebre escultura de Stefano Maderno. A tradição musical a partir do século XV firmou-a como padroeira dos músicos.",
    virtudes: ["Virgindade consagrada", "Apostolado familiar", "Constância no martírio"],
    frase: "Que meu coração e meu corpo permaneçam imaculados.",
  },
  {
    slug: "santa-agueda",
    nome: "Santa Águeda",
    titulo: "Virgem e mártir",
    data: "05/02",
    seculo: "III",
    padroeiro: "Mulheres com câncer de mama, enfermeiras, Sicília",
    resumo: "Jovem siciliana que preferiu a tortura a renegar sua virgindade consagrada a Cristo.",
    biografia:
      "Nascida em Catânia, na Sicília, por volta de 231, em ilustre família cristã, consagrou desde menina a virgindade a Cristo. Em plena perseguição de Décio, o procônsul Quinciano, atraído por sua beleza e pelos bens da família, tentou seduzi-la e, recusado, entregou-a à matrona corruptora Afrodísia, sem êxito. Encarcerada e submetida a sucessivas torturas, teve por fim os seios cortados — daí ser frequentemente representada com eles sobre uma bandeja. À noite, na prisão, foi visitada e curada por um ancião venerável, identificado com São Pedro. Submetida a novas torturas — rolada sobre cacos e brasas —, expirou em prece por volta de 251. No ano seguinte, segundo a tradição, seu véu, levado em procissão sobre o Etna em erupção, deteve a lava antes que destruísse Catânia, devoção que se reedita anualmente nas festas da padroeira. É invocada contra os incêndios e, modernamente, com particular afeto pelas mulheres acometidas de câncer de mama.",
    virtudes: ["Virgindade", "Fortaleza no sofrimento", "Confiança no socorro divino"],
    frase: "Jesus Cristo, Senhor de todas as coisas, vês meu coração e conheces meu desejo.",
  },
  {
    slug: "santa-luzia",
    nome: "Santa Luzia",
    titulo: "Virgem e mártir",
    data: "13/12",
    seculo: "III–IV",
    padroeiro: "Cegos, doentes dos olhos, oftalmologistas",
    resumo: "A jovem de Siracusa que iluminou as catacumbas e converteu sua perseguição em luz.",
    biografia:
      "Nasceu em Siracusa, na Sicília, por volta de 283, em rica família cristã. Aos cinco anos perdeu o pai; consagrou em segredo a virgindade a Cristo. Levou a mãe Eutícia, gravemente enferma, ao túmulo de Santa Águeda em Catânia, onde obteve milagrosa cura. Em ação de graças, decidiu distribuir o dote aos pobres e renunciar ao casamento prometido a um jovem pagão. Despeitado, o noivo a denunciou ao prefeito Pascásio durante a perseguição de Diocleciano. Condenada à prostituição, não pôde ser removida nem por uma junta de bois — tornou-se inamovível pelo poder do Espírito Santo. Submetida a torturas, com óleo fervente e fogo, foi por fim traspassada pela espada na garganta em 13 de dezembro de 304. Seu nome — Lucia, 'luz' — e a festa próxima ao solstício de inverno fizeram dela, sobretudo na Europa do Norte, a santa da luz; na Suécia, meninas vestidas de branco a celebram com coroa de velas. Tradições posteriores narram que arrancou os próprios olhos para desencorajar o noivo, origem da iconografia que a mostra com os olhos numa bandeja.",
    virtudes: ["Virgindade", "Caridade com os pobres", "Coragem serena"],
    frase: "Não há mais luz que aquela do Senhor.",
  },
  {
    slug: "santa-ines",
    nome: "Santa Inês",
    titulo: "Virgem e mártir",
    data: "21/01",
    seculo: "III–IV",
    padroeiro: "Jovens, noivas, virgens",
    resumo: "Menina de doze anos que enfrentou Roma para guardar sua virgindade.",
    biografia:
      "Nasceu em Roma, em ilustre família cristã, por volta de 291. Aos doze anos, em plena perseguição de Diocleciano, vários jovens nobres a pretenderam em casamento; ela respondeu que já tinha por esposo o Filho de Deus, mais nobre que qualquer outro. Denunciada como cristã pelo despeito dos pretendentes, foi conduzida diante do prefeito Semprônio, que ameaçou submetê-la a Vesta ou a um lupanar. Conduzida nua pela via, foi milagrosamente coberta pelos cabelos que cresceram instantaneamente, e cercada de luz angélica; um jovem que ousou olhá-la com desejo foi fulminado e, depois, ressuscitado por sua prece. Condenada à fogueira, as chamas se inclinaram sem tocá-la; foi finalmente degolada por volta de 304. Sepultada nas catacumbas da Via Nomentana, onde Constantina edificou-lhe basílica, recebe culto antiquíssimo: já São Damaso lhe dedicou um epigrama, e Santo Ambrósio, São Jerônimo e Prudêncio cantaram seus louvores. Todos os anos, em 21 de janeiro, dois cordeiros (em latim 'agnus', donde provém seu nome) são bentos em sua basílica para fornecer a lã dos pálios arquiepiscopais.",
    virtudes: ["Virgindade consagrada", "Coragem juvenil", "Fidelidade absoluta"],
    frase: "Cristo é meu esposo, escolhi-o e só a ele servirei.",
  },
  {
    slug: "santo-atanasio",
    nome: "Santo Atanásio",
    titulo: "Bispo de Alexandria, Doutor da Igreja",
    data: "02/05",
    seculo: "IV",
    padroeiro: "Teólogos, defensores da fé",
    resumo:
      "'Atanásio contra o mundo': defensor inflexível da divindade do Verbo contra o arianismo.",
    biografia:
      "Nasceu em Alexandria por volta de 295, formou-se na célebre escola catequética e foi diácono e secretário do bispo Alexandre no Concílio de Niceia (325), no qual brilhou como teólogo da consubstancialidade do Filho com o Pai. Sucedeu Alexandre na sé alexandrina em 328 com apenas 33 anos. Por mais de quarenta anos governou a maior Igreja do Oriente em meio à tormenta ariana, sofrendo cinco exílios sob quatro imperadores e somando dezessete anos longe da sé. Refugiou-se entre os monges do deserto egípcio, aos quais se liga a 'Vida de Santo Antão', tradicionalmente tida como sua — obra que difundiu o monasticismo no Ocidente e converteu, entre outros, Santo Agostinho. Suas obras 'De Incarnatione', 'Contra os Arianos' e os 'Discursos' fixaram para sempre a fé na divindade plena de Cristo: 'Deus se fez homem para que o homem se tornasse Deus'. Morreu em paz em sua cidade em 2 de maio de 373, depois de ver triunfar a fé nicena que tantas vezes lhe parecera perdida. Foi proclamado Doutor da Igreja em 1568.",
    virtudes: ["Constância heroica", "Profundidade teológica", "Comunhão com os monges"],
    frase: "O Verbo se fez homem para que nós fôssemos deificados.",
  },
  {
    slug: "sao-basilio-magno",
    nome: "São Basílio Magno",
    titulo: "Bispo, Doutor da Igreja, pai do monasticismo oriental",
    data: "02/01",
    seculo: "IV",
    padroeiro: "Capadócia, monges orientais, hospitais",
    resumo:
      "Pastor, teólogo e legislador monástico, fundador da grande caridade institucional cristã.",
    biografia:
      "Nasceu em Cesareia da Capadócia em 329, em família de santos: irmão de São Gregório de Nissa, de Santa Macrina e de São Pedro de Sebaste. Estudou em Constantinopla e Atenas, onde travou amizade entranhável com Gregório Nazianzeno. Renunciou à brilhante carreira civil, percorreu os mosteiros do Egito e da Síria e fundou comunidades cenobíticas no Ponto, redigindo as 'Regras Maior e Menor' que ainda governam o monasticismo bizantino. Ordenado sacerdote e, em 370, bispo de Cesareia, governou heroicamente sua Igreja em meio à perseguição ariana do imperador Valente, a quem replicou com firmeza memorável: 'Nunca vi um bispo'. Criou nos arredores da cidade a 'Basiliade', vasto complexo de hospital, asilo e leprosário, primeiro grande exemplo de assistência cristã organizada. Escreveu o 'De Spiritu Sancto', as 'Homilias sobre o Hexamerão', e uma das mais belas liturgias eucarísticas do Oriente. Morreu em 379, exausto pelas vigílias e jejuns, e é venerado como Cappadocio insigne junto aos dois Gregórios.",
    virtudes: ["Sabedoria pastoral", "Caridade institucional", "Austeridade monástica"],
    frase: "O pão que guardas é dos famintos; o manto guardado, dos nus.",
  },
  {
    slug: "sao-gregorio-nazianzeno",
    nome: "São Gregório Nazianzeno",
    titulo: "Bispo, 'o Teólogo', Doutor da Igreja",
    data: "02/01",
    seculo: "IV",
    padroeiro: "Poetas cristãos, oradores sacros",
    resumo:
      "Cantor da Trindade, o único, com São João Evangelista, chamado pela liturgia oriental 'o Teólogo'.",
    biografia:
      "Nasceu em Arianzo, na Capadócia, por volta de 329, filho do bispo Gregório, o Velho. Estudou retórica e filosofia em Cesareia, Alexandria e Atenas, onde se ligou de amizade indissolúvel a Basílio. De temperamento contemplativo, atraído pelo silêncio do mosteiro, foi sucessivamente arrancado à solidão pela ordenação sacerdotal, pelo episcopado de Sasima (que jamais quis ocupar) e, em 379, pela missão de pregar à pequena comunidade católica de Constantinopla, capital invadida pelo arianismo. Em poucos meses, com os cinco célebres 'Discursos Teológicos', reconverteu a cidade e foi aclamado bispo. Presidiu o Concílio de Constantinopla (381), que completou o Credo de Niceia proclamando a divindade do Espírito Santo. Renunciou pouco depois em prol da paz, voltou a Nazianzo e Arianzo, e ali consumiu os últimos anos em oração, poesia (deixou cerca de 17 mil versos) e cartas, falecendo em 389 ou 390. Sua eloquência e teologia trinitária permanecem cume da patrística grega.",
    virtudes: ["Contemplação", "Eloquência", "Humildade que renuncia ao poder"],
    frase: "Não posso adorar o Pai sem o Filho, nem o Filho sem o Espírito.",
  },
  {
    slug: "sao-gregorio-de-nissa",
    nome: "São Gregório de Nissa",
    titulo: "Bispo, místico, Doutor da Igreja",
    data: "10/01",
    seculo: "IV",
    padroeiro: "Místicos, teólogos",
    resumo:
      "Irmão mais novo de Basílio, fino metafísico da infinitude divina e do progresso na vida espiritual.",
    biografia:
      "Nasceu em Cesareia da Capadócia por volta de 335, num lar excepcional: irmão de Basílio, de Macrina e de Pedro de Sebaste. Inicialmente retórico e casado, foi conduzido à vida eclesiástica por insistência do irmão, que o sagrou bispo de Nissa em 371 para defender a fé nicena na pequena diocese. Pouco hábil para a administração, brilhou, porém, como teólogo e místico de primeira grandeza. Participou ativamente do Concílio de Constantinopla (381), que o reconheceu coluna da ortodoxia. Suas obras — 'Vida de Moisés', 'Homilias sobre o Cântico dos Cânticos', 'A Grande Catequese', 'Sobre a alma e a ressurreição' (diálogo com a irmã Macrina às portas da morte) — desenvolveram a doutrina da ascensão sem fim da alma para Deus ('epektasis'), a teologia apofática da incompreensibilidade divina e a antropologia da imagem. Falecendo por volta de 395, completa, com Basílio e Gregório Nazianzeno, a tríade dos Padres Capadócios e foi inscrito por Roma entre os Doutores da Igreja.",
    virtudes: ["Profundidade contemplativa", "Liberdade interior", "Audácia metafísica"],
    frase: "Buscar a Deus é vê-lo; ver a Deus é ainda buscá-lo.",
  },
  {
    slug: "sao-joao-crisostomo",
    nome: "São João Crisóstomo",
    titulo: "Bispo de Constantinopla, Doutor da Igreja",
    data: "13/09",
    seculo: "IV–V",
    padroeiro: "Pregadores, oradores sacros",
    resumo: "'Boca de ouro' que pregou a verdade aos pobres e aos imperadores e morreu no exílio.",
    biografia:
      "Nasceu em Antioquia por volta de 349, em família cristã; perdeu o pai cedo e foi educado pela mãe Antusa. Estudou retórica com o pagão Libânio e teologia com Diodoro de Tarso. Após anos de vida monástica nos montes da Síria, que arruinou sua saúde, voltou à cidade, foi ordenado diácono e sacerdote, e passou doze anos pregando na catedral de Antioquia — entre os Padres, é quem nos legou mais homilias, com comentários integrais a Mateus, João e Paulo. Em 397 foi conduzido a Constantinopla, contra sua vontade, como arcebispo. Reformou austeramente o clero, denunciou o luxo da corte, defendeu os pobres e desafiou a imperatriz Eudóxia. Por duas vezes deposto pelo Sínodo do Carvalho, exilado primeiro na Bitínia e depois nos confins do Ponto, morreu durante a marcha forçada em Comana, em 14 de setembro de 407, pronunciando as últimas palavras: 'Glória a Deus por tudo!'. Suas relíquias foram solenemente devolvidas pelo Vaticano à sé ortodoxa de Constantinopla em 2004.",
    virtudes: ["Coragem profética", "Caridade pelos pobres", "Eloquência sagrada"],
    frase: "Glória a Deus por tudo!",
  },
  {
    slug: "santo-ambrosio",
    nome: "Santo Ambrósio",
    titulo: "Bispo de Milão, Doutor da Igreja",
    data: "07/12",
    seculo: "IV",
    padroeiro: "Milão, apicultores, abelheiros",
    resumo:
      "Catecúmeno aclamado bispo pelo povo, mestre de Santo Agostinho e legislador da liturgia ocidental.",
    biografia:
      "Nasceu em Tréveris por volta de 339, filho do prefeito da Gália. Brilhante advogado e governador da Ligúria e Emília, com sede em Milão, foi providencialmente eleito bispo pelos cidadãos em 374, ainda catecúmeno: em uma semana recebeu o batismo e todas as ordens. Distribuiu seus bens aos pobres, mergulhou no estudo das Escrituras e dos Padres gregos e reformou energicamente a Igreja. Compôs hinos litúrgicos imortais — origem do chamado canto ambrosiano —, escreveu tratados como 'De officiis ministrorum', 'De mysteriis' e 'De Spiritu Sancto', e enfrentou imperadores: obrigou o jovem Valentiniano a recusar a restauração do altar da Vitória, e impôs pública penitência a Teodósio o Grande após o massacre de Tessalônica — episódio simbólico das relações entre Igreja e poder no Ocidente. Foi instrumento decisivo da conversão de Agostinho, a quem batizou em 387 na noite pascal em Milão. Morreu em 4 de abril de 397, sábado santo, e seu corpo repousa intacto na basílica milanesa.",
    virtudes: ["Coragem episcopal", "Caridade", "Eloquência"],
    frase: "O imperador está dentro da Igreja, não acima dela.",
  },
  {
    slug: "santo-agostinho",
    nome: "Santo Agostinho",
    titulo: "Bispo de Hipona, Doutor da Graça",
    data: "28/08",
    seculo: "IV–V",
    padroeiro: "Teólogos, tipógrafos, conversões tardias",
    resumo:
      "O coração inquieto que encontrou em Deus o repouso e moldou o pensamento cristão ocidental.",
    biografia:
      "Aurélio Agostinho nasceu em Tagaste, na Numídia, em 354, do pagão Patrício e da cristã Mônica. Brilhante em retórica, levou em Cartago e Roma vida desregrada, teve um filho fora do casamento (Adeodato) e aderiu por nove anos ao maniqueísmo. Insatisfeito, passou ao ceticismo e, em Milão, conheceu Santo Ambrósio, cuja pregação dissolveu seus preconceitos contra as Escrituras. A leitura de Paulo no jardim ('Toma e lê'), após anos de luta moral narrados nas 'Confissões', selou a conversão; foi batizado por Ambrósio na Páscoa de 387. Voltou à África, viveu em comunidade monástica e foi ordenado sacerdote e, em 395, bispo de Hipona. Durante 35 anos governou a Igreja africana e enfrentou maniqueus, donatistas e pelagianos, escrevendo cerca de 113 obras, 200 cartas e 500 sermões. As 'Confissões', a 'Cidade de Deus' e o 'De Trinitate' moldaram para sempre o Ocidente cristão. Morreu em 28 de agosto de 430, durante o cerco vândalo a Hipona, rezando os salmos penitenciais pregados na parede.",
    virtudes: ["Busca incansável da verdade", "Humildade na conversão", "Caridade pastoral"],
    frase:
      "Fizestes-nos para Vós, Senhor, e o nosso coração vive inquieto enquanto não repousa em Vós.",
  },
  {
    slug: "sao-jeronimo",
    nome: "São Jerônimo",
    titulo: "Sacerdote, Doutor da Igreja, tradutor da Vulgata",
    data: "30/09",
    seculo: "IV–V",
    padroeiro: "Biblistas, tradutores, bibliotecários",
    resumo: "Eremita erudito de Belém que entregou ao Ocidente a Bíblia em latim.",
    biografia:
      "Eusébio Jerônimo nasceu em Estridão, na Dalmácia, por volta de 347. Estudou em Roma sob Donato, recebeu o batismo do papa Libério e levou inicialmente vida mundana, sacudida por sonho célebre em que ouviu o reproche: 'Ciceroniano és, não cristão'. Retirou-se ao deserto da Cálcida, onde aprendeu hebraico com um rabino convertido e mortificou a carne entre escorpiões. Ordenado sacerdote em Antioquia, foi a Constantinopla ouvir Gregório Nazianzeno e depois a Roma, onde se tornou secretário do papa Dâmaso I, que lhe encomendou a revisão das traduções latinas da Bíblia. Acompanhado pelas piedosas matronas Paula e Eustóquio, fixou-se em 386 em Belém, num mosteiro junto à gruta da Natividade. Por mais de trinta anos verteu diretamente do hebraico e do grego o conjunto das Escrituras: nasceu a Vulgata, que o Concílio de Trento consagrou como texto oficial da liturgia latina. Escreveu também monumentais comentários bíblicos, vidas de monges e violentas polêmicas contra Joviniano, Vigilâncio e Pelágio. Morreu em Belém em 419 ou 420.",
    virtudes: ["Amor à Escritura", "Penitência", "Erudição a serviço da fé"],
    frase: "Ignorar as Escrituras é ignorar a Cristo.",
  },
  {
    slug: "sao-gregorio-magno",
    nome: "São Gregório Magno",
    titulo: "Papa, Doutor da Igreja",
    data: "03/09",
    seculo: "VI–VII",
    padroeiro: "Cantores, professores, músicos",
    resumo: "Senador romano feito monge e Papa, criou a Idade Média cristã.",
    biografia:
      "Nasceu em Roma por volta de 540, de antiga família patrícia. Foi prefeito da Urbe ainda jovem, mas renunciou a tudo, converteu o palácio familiar do Célio em mosteiro beneditino, distribuiu seus bens aos pobres e fundou outros seis mosteiros na Sicília. Diácono e apocrisiário em Constantinopla, em 590 foi eleito Papa por aclamação numa Roma devastada pela peste, pelas invasões lombardas e pela fome. Reformou a administração eclesiástica, criou o 'patrimônio de São Pedro' como rede de socorro aos pobres, ordenou e fixou o canto litúrgico que recebeu seu nome — o cantochão gregoriano —, e enviou monges, com Santo Agostinho de Cantuária, à evangelização da Inglaterra. Escreveu obras de referência: a 'Regra Pastoral' (manual de bispos durante mil anos), as 'Morais sobre Jó', os 'Diálogos' e numerosas homilias. Assinava-se 'servo dos servos de Deus', título conservado pelos sucessores. Morreu em 12 de março de 604, exausto pela enfermidade, e foi imediatamente venerado como santo. É um dos quatro grandes Doutores latinos.",
    virtudes: ["Humildade pastoral", "Caridade institucional", "Sabedoria de governo"],
    frase: "Servo dos servos de Deus.",
  },
  {
    slug: "santa-monica",
    nome: "Santa Mônica",
    titulo: "Mãe de Santo Agostinho",
    data: "27/08",
    seculo: "IV",
    padroeiro: "Mães, esposas, viúvas, alcoólicos",
    resumo: "Lágrimas perseverantes de uma mãe que arrancaram o filho à perdição.",
    biografia:
      "Nasceu em Tagaste, Numídia, por volta de 332, em família cristã. Casou-se ainda jovem com o pagão Patrício, homem violento e infiel, a quem ganhou ao Evangelho pela paciência, doçura e oração — convertendo-se ele no leito de morte, em 371. Cuidou de três filhos, em particular do brilhante Agostinho, cuja juventude desregrada e adesão ao maniqueísmo lhe valeram quase vinte anos de lágrimas. Pediu a um bispo que o convertesse; este lhe respondeu a célebre frase: 'Vai em paz: é impossível que se perca um filho de tantas lágrimas'. Quando Agostinho fugiu para Roma, seguiu-o até Milão, onde teve a alegria de assistir à conversão e ao batismo do filho por Santo Ambrósio na Páscoa de 387. Pouco depois, voltando à África, fizeram em Óstia uma célebre conversa contemplativa, descrita pelo filho nas 'Confissões'. Adoeceu logo após e morreu em paz aos 56 anos, pedindo apenas que se lembrassem dela no altar do Senhor. É padroeira universal das mães que rezam pelos filhos.",
    virtudes: ["Perseverança na oração", "Mansidão conjugal", "Esperança paciente"],
    frase: "Filho, tudo me deleitava antes neste mundo. Agora nada me retém.",
  },
  {
    slug: "sao-bento-de-nursia",
    nome: "São Bento de Núrsia",
    titulo: "Abade, patriarca do monasticismo ocidental, Patrono da Europa",
    data: "11/07",
    seculo: "V–VI",
    padroeiro: "Europa, monges, estudantes, contra envenenamento",
    resumo: "Pai dos monges do Ocidente, deu à civilização cristã a regra 'ora et labora'.",
    biografia:
      "Nasceu em Núrsia, na Úmbria, por volta de 480, em família patrícia. Estudante em Roma, fugiu da dissolução da cidade para viver como eremita na gruta de Subiaco. Durante três anos foi alimentado pelo monge Romano que lhe descia o pão por uma corda. Espalhada a fama de sua santidade, fundou ali doze pequenos mosteiros e, por fim, transferiu-se para Monte Cassino por volta de 529, onde destruiu o templo pagão e fundou a abadia-mãe que se tornaria o coração espiritual do Ocidente. Compôs ali a 'Regra' — síntese genial entre as tradições orientais (Pacômio, Basílio, Cassiano) e a moderação latina —, governada pelos princípios da estabilidade, da conversão dos costumes e da obediência, e ritmada pela liturgia das horas e pelo trabalho manual. A divisa 'Ora et labora' resume seu espírito. Segundo os Diálogos de São Gregório Magno, principal fonte sobre sua vida, operou inúmeros milagres, profetizou a destruição de Monte Cassino pelos lombardos e morreu de pé, junto ao altar, sustentado pelos irmãos, em 21 de março de 547. Paulo VI o proclamou Patrono Principal da Europa em 1964.",
    virtudes: ["Discrição", "Estabilidade", "Obediência", "Equilíbrio entre oração e trabalho"],
    frase: "Nada se anteponha ao amor de Cristo.",
  },
  {
    slug: "sao-bonifacio",
    nome: "São Bonifácio",
    titulo: "Bispo, mártir, apóstolo da Germânia",
    data: "05/06",
    seculo: "VII–VIII",
    padroeiro: "Alemanha, cervejeiros, alfaiates",
    resumo: "Monge inglês que evangelizou os povos germânicos e fundou a Igreja alemã.",
    biografia:
      "Wynfrith nasceu em Crediton, na Inglaterra, por volta de 675; entrou cedo no mosteiro beneditino de Nutshalling, onde se distinguiu como erudito e mestre. Recusando o cargo de abade, deixou tudo em 716 para evangelizar a Frísia, e em 719 obteve do papa Gregório II o nome novo de Bonifácio e mandato apostólico para os povos germânicos. Durante quase quarenta anos percorreu a Hessia, a Turíngia, a Baviera e a Frísia, batizando multidões, fundando bispados e mosteiros (entre eles Fulda, com seu discípulo Sturm) e organizando uma hierarquia eclesiástica conectada a Roma. O gesto simbólico de seu apostolado foi a derrubada do carvalho sagrado de Donar em Geismar, em 723: diante do espanto pagão, ergueu com a madeira do ídolo uma capela a São Pedro. Sagrado arcebispo de Mogúncia, reformou a Igreja franca em sínodos memoráveis e ungiu rei Pepino o Breve, abrindo o caminho da aliança carolíngio-papal. Aos 80 anos, retornou à missão entre os frísios pagãos e foi assassinado por uma turba em Dokkum, em 5 de junho de 754, segurando sobre a cabeça o livro dos Evangelhos.",
    virtudes: ["Zelo missionário", "Fidelidade a Roma", "Coragem no martírio"],
    frase: "Sejamos firmes na luta e padeçamos com paciência por Cristo.",
  },
  {
    slug: "santo-anselmo",
    nome: "Santo Anselmo",
    titulo: "Arcebispo de Cantuária, Doutor da Igreja, pai da Escolástica",
    data: "21/04",
    seculo: "XI–XII",
    padroeiro: "Filósofos, teólogos",
    resumo:
      "Monge benedictino que pôs a razão a serviço da fé com o lema 'Fides quaerens intellectum'.",
    biografia:
      "Nasceu em Aosta, no Piemonte, em 1033. Após a morte da mãe, deixou a casa paterna e, depois de andanças pela Borgonha e pela França, atraído pela fama de Lanfranco, entrou no mosteiro beneditino de Bec, na Normandia, em 1060. Tornou-se prior, depois abade, e ali compôs as obras filosófico-teológicas que abrem a era escolástica: o 'Monologion', o 'Proslogion' — onde formula o famoso 'argumento ontológico' da existência de Deus —, e o 'Cur Deus homo', tratado decisivo sobre a redenção como satisfação do amor. Em 1093, contra sua vontade, foi sagrado arcebispo de Cantuária; defendeu a liberdade da Igreja inglesa contra Guilherme II e Henrique I, e sofreu dois longos exílios. Participou do Concílio de Bari (1098), onde rebateu os erros gregos sobre o Filioque. Mestre dos discípulos pela ternura paterna, mais que pela severidade, deixou também magníficas orações e meditações. Morreu em Cantuária em 21 de abril de 1109. Foi proclamado Doutor da Igreja por Clemente XI em 1720.",
    virtudes: ["Inteligência crente", "Mansidão paterna", "Liberdade eclesial"],
    frase: "Não busco compreender para crer; creio para compreender.",
  },
  {
    slug: "sao-bernardo-de-claraval",
    nome: "São Bernardo de Claraval",
    titulo: "Abade, Doutor da Igreja, 'Doutor Melífluo'",
    data: "20/08",
    seculo: "XI–XII",
    padroeiro: "Cistercienses, apicultores, alpinistas",
    resumo:
      "Místico e reformador que sacudiu o século XII com a doçura ardente de seu amor a Cristo e a Maria.",
    biografia:
      "Nasceu em 1090 no castelo de Fontaine-lès-Dijon, terceiro de sete filhos da nobreza borgonhesa. Aos 22 anos entrou no recém-fundado mosteiro de Cister, levando consigo trinta companheiros — entre os quais quatro irmãos e o tio. Três anos depois foi enviado a fundar uma nova casa em Claraval (Clairvaux), da qual seria abade até a morte. Sob seu impulso, a Ordem de Cister espalhou-se em 343 mosteiros pela Europa em vida sua. Conselheiro de papas, reis e bispos, conduziu o cisma de Anacleto II em favor de Inocêncio II, fundamentou a regra dos Templários, combateu os erros de Abelardo no Sínodo de Sens, e pregou aos povos a Segunda Cruzada por encargo de Eugênio III, seu antigo discípulo. Seus sermões sobre o Cântico dos Cânticos, os tratados 'De diligendo Deo' e 'De consideratione', e os escritos marianos — especialmente o 'Memorare' e as quatro homilias 'Super missus est' — fizeram dele cantor incomparável do amor a Cristo e a Maria. Morreu em Claraval em 20 de agosto de 1153; canonizado em 1174, foi proclamado Doutor da Igreja em 1830.",
    virtudes: ["Amor a Cristo crucificado", "Devoção mariana", "Liberdade evangélica"],
    frase: "Por que amar a Deus? Porque é Deus. Como amá-lo? Sem medida.",
  },
  {
    slug: "santo-tomas-de-aquino",
    nome: "Santo Tomás de Aquino",
    titulo: "Sacerdote dominicano, Doutor Angélico, Doutor Comum",
    data: "28/01",
    seculo: "XIII",
    padroeiro: "Estudantes, universidades, teólogos, livreiros",
    resumo: "Síntese suprema entre fé e razão; arquiteto da Summa Theologiae.",
    biografia:
      "Nasceu por volta de 1225 no castelo de Roccasecca, perto de Aquino, filho do conde Landulfo. Educado por monges beneditinos em Monte Cassino, estudou em Nápoles, onde a leitura de Aristóteles e o exemplo dos frades pregadores o levaram a entrar na nova Ordem dos Dominicanos, contra a vontade ferrenha da família, que o sequestrou e manteve um ano em casa. Indomável, partiu para Paris e Colônia, onde foi discípulo de Santo Alberto Magno. Mestre na Universidade de Paris e nos studia romanos, escreveu em pouco mais de vinte anos uma obra monumental: comentários a quase todos os livros das Escrituras e de Aristóteles, as 'Questões disputadas', a 'Suma contra os gentios' e, sobretudo, a 'Summa Theologiae' — síntese ordenadíssima da teologia em mais de três mil artigos. A 6 de dezembro de 1273, após arrebatamento eucarístico em Nápoles, declarou: 'Tudo o que escrevi me parece palha em comparação ao que vi'. Partindo para o Concílio de Lyon, adoeceu e morreu na abadia cisterciense de Fossanova, em 7 de março de 1274. Compôs os ofícios e hinos do Corpus Christi.",
    virtudes: ["Inteligência iluminada", "Humildade", "União eucarística"],
    frase: "Adoro te devote, latens Deitas.",
  },
  {
    slug: "sao-boaventura",
    nome: "São Boaventura",
    titulo: "Cardeal, Doutor da Igreja, 'Doutor Seráfico'",
    data: "15/07",
    seculo: "XIII",
    padroeiro: "Franciscanos, teólogos místicos",
    resumo:
      "Místico e organizador, segundo fundador da Ordem Franciscana e amigo de Tomás de Aquino.",
    biografia:
      "Giovanni di Fidanza nasceu em Bagnoregio, em 1217 ou 1221. A tradição liga seu nome à exclamação do próprio São Francisco — 'O buona ventura!' — ao curar a criança gravemente enferma. Entrou nos Frades Menores por volta de 1243, estudou em Paris sob Alexandre de Hales, ensinou na mesma universidade junto a Tomás de Aquino, com quem fez doutorado em 1257. No mesmo ano foi eleito Ministro Geral dos Franciscanos: governou a Ordem treze anos com tal sabedoria em meio às tensões entre 'espirituais' e 'conventuais' que mereceu o título de segundo fundador. Reformou-a com novas Constituições em Narbona e redigiu a 'Legenda Maior' de São Francisco. Suas obras — 'Itinerarium mentis in Deum', 'Breviloquium', comentários às Sentenças, sermões — são síntese magistral de teologia e mística. Recusou várias mitras antes de aceitar, em 1273, o cardinalato e a sé de Albano. Preparou e abriu o Segundo Concílio de Lyon, decisivo na união provisória com os ortodoxos, mas faleceu antes do encerramento, em 15 de julho de 1274. Canonizado em 1482 e proclamado Doutor da Igreja em 1588.",
    virtudes: ["Sabedoria conciliadora", "Humildade", "Amor à pobreza franciscana"],
    frase: "A perfeição da alma consiste em deixar-se penetrar pela luz da Sabedoria divina.",
  },
  {
    slug: "santo-alberto-magno",
    nome: "Santo Alberto Magno",
    titulo: "Bispo, Doutor da Igreja, 'Doutor Universal'",
    data: "15/11",
    seculo: "XIII",
    padroeiro: "Cientistas naturais, estudantes de ciências",
    resumo:
      "Mestre de Tomás de Aquino, pioneiro do diálogo entre fé, filosofia aristotélica e ciências naturais.",
    biografia:
      "Nasceu em Lauingen, na Suábia, por volta de 1200, de família nobre. Estudou em Pádua, onde, fascinado pelo beato Jordão da Saxônia, entrou nos Dominicanos contra a vontade do tio. Lecionou em Hildesheim, Friburgo, Ratisbona, Estrasburgo, Colônia e Paris, tornando-se o primeiro grande mestre alemão da Ordem. Em Colônia teve por discípulo Tomás de Aquino, em quem reconheceu imediatamente o gênio. Sua obra — 38 volumes na edição de Borgnet — comenta sistematicamente toda a filosofia de Aristóteles e estende a investigação a botânica, zoologia, mineralogia, astronomia, química, medicina, geografia, intuindo séculos antes muitas teses da ciência moderna. Foi provincial dos dominicanos alemães, bispo de Ratisbona (1260-1262) — cargo que renunciou para voltar ao ensino —, conselheiro em concílios e pregador da cruzada. Ainda velho, deslocou-se ao Concílio de Lyon (1274) e defendeu em Paris a memória de Tomás. Morreu em Colônia, em 15 de novembro de 1280, e Pio XI o canonizou e proclamou Doutor da Igreja em 1931, declarando-o em 1941 padroeiro dos cultores das ciências naturais.",
    virtudes: ["Saber enciclopédico", "Humildade do mestre", "Curiosidade contemplativa"],
    frase:
      "Investigar a verdade onde ela se encontre, pois tudo o que é verdadeiro vem do Espírito.",
  },
  {
    slug: "santo-domingos-de-gusmao",
    nome: "Santo Domingos de Gusmão",
    titulo: "Sacerdote, fundador da Ordem dos Pregadores",
    data: "08/08",
    seculo: "XII–XIII",
    padroeiro: "Astrônomos, pregadores, falsamente acusados",
    resumo: "O pregador que combateu a heresia com a pregação itinerante e o rosário.",
    biografia:
      "Domingos de Gusmão nasceu em Caleruega, na Castela, por volta de 1170. Sobrinho do prior dos cônegos regulares de Osma, estudou em Palência, onde vendeu seus livros para socorrer famintos: 'Não posso estudar sobre peles mortas enquanto homens vivos morrem de fome'. Cônego em Osma, acompanhou o bispo Diego na missão diplomática à corte da Dinamarca; no caminho, em Toulouse, encontrou a heresia cátara devastando o sul da França e percebeu que era preciso uma pregação itinerante, pobre, fundamentada na palavra e na santidade de vida. Fundou em 1206 o mosteiro feminino de Prouille e, em 1216, com a aprovação de Honório III, a Ordem dos Pregadores: comunidades de frades doutos, mendicantes e itinerantes, dedicados à pregação para a salvação das almas — 'contemplata aliis tradere'. Difundiu a devoção do santo rosário, recebido segundo a tradição das mãos de Nossa Senhora. Percorreu a Itália, a França e a Espanha enviando frades às universidades. Morreu exausto em Bolonha, em 6 de agosto de 1221, deixando uma das maiores famílias religiosas da Igreja.",
    virtudes: ["Pregação itinerante", "Pobreza alegre", "Estudo a serviço das almas"],
    frase: "Falar com Deus ou falar de Deus.",
  },
  {
    slug: "sao-francisco-de-assis",
    nome: "São Francisco de Assis",
    titulo: "Fundador da Ordem dos Frades Menores",
    data: "04/10",
    seculo: "XII–XIII",
    padroeiro: "Ecologia, animais, Itália",
    resumo: "O Pobrezinho de Assis, místico do amor a Cristo crucificado e a toda criatura.",
    biografia:
      "Nasceu em 1181/82 em Assis, na Úmbria, filho do rico mercador Pietro di Bernardone e de dona Pica. De juventude fastosa e mundana, sonhou com a cavalaria, foi à guerra contra Perugia e amargou um ano de cárcere. Doente, ouviu a chamada de Deus e, diante do crucifixo de São Damião, a ordem: 'Vai, Francisco, restaura a minha casa'. Despojou-se publicamente perante o bispo de Assis, devolveu até as vestes ao pai, e abraçou a Senhora Pobreza. Em 1209, com onze companheiros, obteve do papa Inocêncio III a aprovação da regra primitiva e fundou a Ordem dos Frades Menores; em 1212 nascia, com Santa Clara, a Segunda Ordem; depois, a Ordem dos Penitentes (hoje OFS). Viveu de esmola, pregou a paz nas cidades em guerra, foi até o sultão al-Kamil em Damieta (1219) durante a Quinta Cruzada. No Natal de 1223, em Greccio, criou o primeiro presépio vivo; em 1224, no monte Alverne, recebeu, segundo a tradição franciscana e o testemunho de seus companheiros, os estigmas — sendo tido como o primeiro a recebê-los. Quase cego e exausto, compôs o 'Cântico das Criaturas' e morreu cantando, nu sobre a terra, em Porciúncula, em 3 de outubro de 1226.",
    virtudes: [
      "Pobreza radical",
      "Alegria evangélica",
      "Amor à criação",
      "Conformidade com Cristo crucificado",
    ],
    frase: "Senhor, fazei de mim um instrumento da vossa paz.",
  },
  {
    slug: "santa-clara-de-assis",
    nome: "Santa Clara de Assis",
    titulo: "Virgem, fundadora das Clarissas",
    data: "11/08",
    seculo: "XIII",
    padroeiro: "Televisão, oftalmologistas, bordadeiras",
    resumo:
      "Primeira mulher a escrever uma regra para mulheres na Igreja, planta delicada de São Francisco.",
    biografia:
      "Nasceu em Assis em 1193 ou 1194, em família nobre. Aos dezoito anos, fascinada pela pregação de Francisco, fugiu de casa na noite do Domingo de Ramos de 1212, recebeu dele o hábito penitencial em Porciúncula e refugiou-se primeiro entre as beneditinas; estabeleceu-se finalmente em São Damião, onde permaneceu por 42 anos sem nunca sair, governando a comunidade que dela tomaria o nome — as Clarissas. Conquistou do papa Gregório IX o 'privilégio da pobreza': nem em comum a comunidade poderia possuir bens, vivendo unicamente da Providência. Em 1240 e 1241, o mosteiro foi cercado por mercenários sarracenos a serviço de Frederico II; Clara, doente, fez-se levar com o ostensório do Santíssimo até as muralhas, e os atacantes fugiram. Redigiu para suas irmãs a primeira regra feminina escrita por uma mulher, aprovada em 1253 dois dias antes de sua morte. Santíssima do Sacramento e fidelíssima ao espírito de Francisco, faleceu em 11 de agosto de 1253. Canonizada em 1255, é, desde Pio XII (1958), padroeira da televisão por ter visto à distância, doente no leito, a missa da basílica.",
    virtudes: ["Pobreza absoluta", "Adoração eucarística", "Fidelidade ao carisma franciscano"],
    frase: "Olha o teu Esposo, o mais belo dos filhos dos homens.",
  },
  {
    slug: "santo-antonio-de-padua",
    nome: "Santo Antônio de Pádua",
    titulo: "Sacerdote franciscano, Doutor Evangélico",
    data: "13/06",
    seculo: "XII–XIII",
    padroeiro: "Pobres, objetos perdidos, casamenteiro, Portugal",
    resumo: "O 'Martelo dos hereges' e mestre da Palavra, com o Menino Jesus nos braços.",
    biografia:
      "Fernando Martins de Bulhões nasceu em Lisboa por volta de 1195, em família nobre. Entrou aos quinze anos nos cônegos regulares de Santo Agostinho, primeiro em São Vicente de Fora, depois em Santa Cruz de Coimbra, onde foi ordenado sacerdote e mergulhou nas Escrituras. Em 1220, comovido pela chegada dos corpos dos cinco protomártires franciscanos mortos em Marrocos, deixou os cônegos e tomou o hábito dos frades menores em Coimbra, com o nome de Antônio. Partiu para o martírio em Marrocos, mas, gravemente enfermo, foi devolvido a Portugal; arrastado por uma tempestade até a Sicília, chegou a Assis ao Capítulo das Esteiras (1221). Vivia em escondida vida monástica em Forlì quando, improvisando uma pregação em ocasião imprevista, revelou prodigioso dom de orador. São Francisco encarregou-o do ensino da teologia aos frades — primeiro lente franciscano. Pregou em toda a Itália e no sul da França, combatendo cátaros e albigenses. Inúmeros milagres lhe são atribuídos, da mula que se ajoelhou ante a hóstia à pregação aos peixes. Morreu em 13 de junho de 1231, perto de Pádua, com 36 anos. Canonizado em apenas 352 dias, foi proclamado Doutor da Igreja por Pio XII em 1946.",
    virtudes: ["Pregação inspirada", "Caridade com os pobres", "Ciência das Escrituras"],
    frase: "As obras dão peso às palavras.",
  },
  {
    slug: "santa-catarina-de-sena",
    nome: "Santa Catarina de Sena",
    titulo: "Virgem dominicana, Doutora da Igreja, Padroeira da Europa",
    data: "29/04",
    seculo: "XIV",
    padroeiro: "Itália, Europa, enfermeiras, jornalistas",
    resumo:
      "A jovem analfabeta que reconduziu o Papa de Avignon a Roma e ditou obras de altíssima mística.",
    biografia:
      "Caterina Benincasa nasceu em Siena em 25 de março de 1347, vigésima quarta filha de Jacopo, tintureiro. Aos seis anos, voltando da casa de uma irmã, teve a primeira visão de Cristo em glória sobre a igreja de São Domingos. Aos sete consagrou a virgindade; recusou as bodas que a família lhe impunha e, aos dezesseis, recebeu o hábito das Mantelatas dominicanas, terceira ordem leiga. Após três anos de reclusão e oração na 'cela do conhecimento de si', recebeu de Cristo os místicos esponsais e foi enviada à ação. Cercou-se de discípulos — a 'bella brigata' — de toda condição, e tornou-se diplomata, conselheira de papas e de príncipes, pacificadora de cidades. Em 1376 foi a Avignon e persuadiu Gregório XI a retornar a Roma, pondo fim ao exílio de 70 anos. Trabalhou pela unidade do Cisma do Ocidente. Segundo o testemunho de seus confessores, recebeu os estigmas invisíveis em Pisa (1375). Ditou em êxtase o 'Diálogo da Divina Providência' e cerca de 380 cartas, monumentos do italiano espiritual. Morreu em Roma em 29 de abril de 1380, aos 33 anos. Doutora da Igreja (1970), Padroeira da Europa (1999).",
    virtudes: ["Mística ardente", "Coragem profética", "Caridade com os doentes"],
    frase: "Se sois aquilo que deveis ser, incendiareis o mundo inteiro.",
  },
  {
    slug: "santa-brigida-da-suecia",
    nome: "Santa Brígida da Suécia",
    titulo: "Religiosa, fundadora, Padroeira da Europa",
    data: "23/07",
    seculo: "XIV",
    padroeiro: "Europa, Suécia, viúvas, peregrinos",
    resumo:
      "Esposa, mãe de oito filhos, viúva e mística que recebeu da Cruz as 'Revelações' celebérrimas.",
    biografia:
      "Nasceu em 1303, na província sueca de Uppland, filha do governador Birger. Casou-se aos 14 anos com Ulf Gudmarsson e teve oito filhos — entre eles Santa Catarina da Suécia. Levou vida de profunda piedade, peregrinou com o marido a Compostela em 1341; depois da morte deste, em 1344, repartiu seus bens, abraçou austera vida penitente em Alvastra e começou a receber as 'Revelações' celestes, ditadas em sueco e traduzidas ao latim por seus confessores. Por mandato divino, fundou a Ordem do Santíssimo Salvador — as 'brigidinas' — com mosteiro duplo em Vadstena. Partiu para Roma em 1349 e ali viveu vinte e quatro anos, repreendendo papas e cardeais, exigindo o retorno do Pontífice a Roma e a reforma da Igreja, e exercendo intensa caridade entre os pobres e peregrinos. Empreendeu peregrinação à Terra Santa em 1372, onde teve novas visões da Paixão e do Nascimento de Cristo, que mudariam a iconografia ocidental do presépio. Morreu em Roma em 23 de julho de 1373; canonizada em 1391, foi declarada por São João Paulo II Padroeira da Europa em 1999.",
    virtudes: ["Vida conjugal santa", "Caridade", "Coragem profética com os poderosos"],
    frase: "Senhor, dai-me a graça de fazer-vos conhecido em toda a Europa.",
  },
  {
    slug: "santa-joana-darc",
    nome: "Santa Joana d'Arc",
    titulo: "Virgem, mártir, Padroeira da França",
    data: "30/05",
    seculo: "XV",
    padroeiro: "França, soldados, telegrafistas, vítimas de injustiça",
    resumo:
      "A jovem camponesa de Domrémy que, guiada pelas vozes do céu, salvou um reino e morreu na fogueira por fidelidade.",
    biografia:
      "Nasceu em 1412 em Domrémy, na Lorena, em família camponesa profundamente cristã. Em plena Guerra dos Cem Anos, com a França quase inteira nas mãos inglesas, começou aos treze anos a ouvir vozes de São Miguel, Santa Catarina e Santa Margarida que lhe pediam libertar Orléans e conduzir o delfim Carlos a Reims para sua consagração. Vencidas as resistências, recebeu em Chinon armadura e tropas; em 8 de maio de 1429 libertou Orléans em poucos dias; em 17 de julho, fez Carlos VII coroar-se rei na catedral de Reims, com ela ao seu lado. Capturada em Compiègne em 23 de maio de 1430, foi vendida aos ingleses por João de Luxemburgo e julgada em Ruão pelo tribunal eclesiástico parcial do bispo Pedro Cauchon. Após meses de perguntas insidiosas, recusando-se a desmentir suas vozes e sua missão divina, foi condenada como herética e bruxa. Foi queimada viva na praça de Ruão, em 30 de maio de 1431, com 19 anos, segurando a cruz e pronunciando o nome de Jesus. Reabilitada em 1456, canonizada em 1920, é Padroeira secundária da França.",
    virtudes: ["Pureza", "Coragem", "Obediência às vozes divinas", "Fidelidade até o fogo"],
    frase: "Jesus, Jesus, Jesus!",
  },
  {
    slug: "sao-tomas-morus",
    nome: "São Tomás Morus",
    titulo: "Mártir, leigo, Chanceler da Inglaterra",
    data: "22/06",
    seculo: "XV–XVI",
    padroeiro: "Políticos, juristas, advogados, governantes",
    resumo: "Humanista, marido e pai exemplar, preferiu o cadafalso a trair a consciência.",
    biografia:
      "Nasceu em Londres em 7 de fevereiro de 1478, filho do juiz John More. Educado em Oxford e nas escolas jurídicas londrinas, foi advogado, depois subxerife de Londres, embaixador, presidente da Câmara dos Comuns e, em 1529, Lorde Chanceler do Reino — o mais alto cargo civil — sob Henrique VIII. Casado duas vezes (viúvo da primeira esposa, Jane Colt), pai de quatro filhos, fez de sua casa em Chelsea um centro de humanismo cristão, frequentado por Erasmo, que ali concebeu o 'Elogio da Loucura'. Escreveu a 'Utopia' (1516), clássico do pensamento político. Quando Henrique VIII pretendeu anular o matrimônio com Catarina de Aragão e proclamar-se chefe da Igreja na Inglaterra, More renunciou ao cargo (1532) e recusou-se a prestar o juramento de supremacia. Acusado de alta traição, foi preso na Torre de Londres, julgado em farsa e decapitado em 6 de julho de 1535, declarando ao subir o cadafalso: 'Morro como bom servidor do rei, mas primeiro de Deus'. Canonizado em 1935 por Pio XI; padroeiro dos governantes e políticos por declaração de João Paulo II em 2000.",
    virtudes: ["Fidelidade à consciência", "Senso do humor cristão", "Vida familiar santa"],
    frase: "Morro como bom servidor do rei, mas primeiro de Deus.",
  },
  {
    slug: "santa-teresa-de-avila",
    nome: "Santa Teresa de Ávila",
    titulo: "Virgem, fundadora, Doutora da Igreja",
    data: "15/10",
    seculo: "XVI",
    padroeiro: "Espanha, doentes da cabeça, escritores místicos",
    resumo:
      "Reformadora do Carmelo, mestra incomparável da oração mística e da audácia espiritual.",
    biografia:
      "Teresa Sánchez de Cepeda y Ahumada nasceu em Ávila em 28 de março de 1515, em família de origem judaico-conversa. Aos vinte entrou no Carmelo da Encarnação de Ávila, então mosteiro mundano e relaxado. Doente, atravessou anos de mediocridade até uma conversão decisiva diante de um 'Cristo todo chagado' em 1554. Começou então uma vida de oração contemplativa intensíssima, com graças místicas extraordinárias: arrebatamentos, visões, a transverberação do coração descrita pela escultura de Bernini em Roma. Decidiu, ainda monja simples, reformar o Carmelo e voltá-lo à primitiva austeridade. Fundou em 1562 o pequeno mosteiro de São José de Ávila, com regra observantíssima e sem rendas; nasciam os Carmelitas Descalços. Em vinte anos de itinerâncias por toda a Espanha, fundou outros dezesseis mosteiros femininos e, com a colaboração de São João da Cruz, ramos masculinos. Escreveu obras imortais — 'Vida', 'Caminho de Perfeição', 'Castelo Interior', 'Fundações', cartas —, todas redigidas no meio de viagens, doenças, oposições e processos inquisitoriais. Morreu em Alba de Tormes em 4 de outubro de 1582. Primeira mulher Doutora da Igreja (1970).",
    virtudes: ["Oração contínua", "Determinação varonil", "Senso comum santificado", "Alegria"],
    frase: "Nada te perturbe, nada te espante; só Deus basta.",
  },
  {
    slug: "sao-joao-da-cruz",
    nome: "São João da Cruz",
    titulo: "Sacerdote carmelita, Doutor Místico",
    data: "14/12",
    seculo: "XVI",
    padroeiro: "Místicos, poetas espanhóis, contemplativos",
    resumo: "Pequeno frade, gigante místico, cantor da noite escura e da chama viva de amor.",
    biografia:
      "Juan de Yepes nasceu em Fontiveros, na Velha Castela, em 1542, filho de Gonzalo, deserdado por casar-se com Catalina, modesta tecelã. Órfão cedo, conheceu a pobreza extrema. Enfermeiro num hospital e estudante na escola dos jesuítas de Medina, entrou aos 21 nos Carmelitas, estudou em Salamanca e, prestes a passar à Cartuxa em busca de maior austeridade, foi convencido por Teresa de Ávila a colaborar na reforma da Ordem. Em 1568 inaugurou, em Duruelo, o primeiro convento de Carmelitas Descalços. Em 1577 foi preso por seus próprios irmãos, que se opunham à reforma, e mantido nove meses num cárcere subterrâneo em Toledo, em sofrimento extremo. Ali compôs mentalmente versos imortais — 'Cântico Espiritual', 'Noite Escura' — antes de fugir milagrosamente. Foi prior, formador e vigário provincial; redigiu os comentários em prosa às próprias poesias — 'Subida do Monte Carmelo', 'Noite Escura da Alma', 'Cântico', 'Chama Viva' — cume da literatura mística universal. Caído em desgraça nas próprias fileiras nos últimos meses, morreu em Úbeda, na Andaluzia, em 14 de dezembro de 1591. Doutor da Igreja desde 1926.",
    virtudes: ["Pureza de fé", "Desapego absoluto", "Amor crucificado"],
    frase: "No entardecer da vida, seremos julgados pelo amor.",
  },
  {
    slug: "santo-inacio-de-loyola",
    nome: "Santo Inácio de Loyola",
    titulo: "Sacerdote, fundador da Companhia de Jesus",
    data: "31/07",
    seculo: "XVI",
    padroeiro: "Retiros espirituais, militares, exército",
    resumo:
      "Soldado convertido pela leitura dos santos, deu à Igreja os 'Exercícios' e os Jesuítas.",
    biografia:
      "Íñigo López de Loyola nasceu em 1491 no castelo de Loyola, no País Basco, treze filhos de família nobre. Pajem na corte, depois cavaleiro a serviço do vice-rei de Navarra, ferido em ambas as pernas no cerco de Pamplona em 1521. Na longa convalescença, à falta de romances, leu uma 'Vida de Cristo' e o 'Floretum Sanctorum'; comparou os movimentos da alma e descobriu o discernimento dos espíritos. Convertido, peregrinou a Montserrat, depositou as armas ante a Virgem e passou um ano em Manresa, em durísima penitência e oração, onde compôs o primeiro esboço dos 'Exercícios Espirituais'. Estudou em Barcelona, Alcalá, Salamanca e Paris (já com mais de 30 anos), reunindo companheiros — Francisco Xavier, Pedro Fabro, entre outros. Em Montmartre, em 1534, fizeram votos. Em 1540, Paulo III aprovou a Companhia de Jesus; Inácio foi eleito primeiro Prepósito Geral. Durante 15 anos governou da casa de Roma a Ordem que cresceu a mil membros e à evangelização da Índia, do Japão, do Brasil e da Europa. Escreveu as 'Constituições' e o 'Diário Espiritual'. Morreu em Roma em 31 de julho de 1556. Canonizado em 1622.",
    virtudes: ["Discernimento", "Indiferença santa", "Obediência à Igreja", "Magnanimidade"],
    frase: "Ad maiorem Dei gloriam.",
  },
  {
    slug: "sao-francisco-xavier",
    nome: "São Francisco Xavier",
    titulo: "Sacerdote jesuíta, apóstolo do Oriente",
    data: "03/12",
    seculo: "XVI",
    padroeiro: "Missões, missionários, Índia, Japão, turistas",
    resumo: "O 'gigante das missões' que levou Cristo a meio mundo em apenas dez anos.",
    biografia:
      "Francisco de Jasso y Azpilcueta nasceu no castelo de Xavier, em Navarra, em 7 de abril de 1506, filho do chanceler do rei. Estudante de filosofia em Paris, dividia o quarto com Pedro Fabro e foi conquistado por Inácio de Loyola — 'Que aproveita ao homem ganhar o mundo se perde a alma?'. Esteve entre os sete primeiros companheiros que fizeram votos em Montmartre em 1534. Em 1541, partiu de Lisboa como nuncio apostólico para o Oriente, a pedido do rei João III de Portugal. Em treze meses chegou a Goa; percorreu a Índia portuguesa, a Costa da Pescaria, Sri Lanka, Malaca, as ilhas Molucas. Em 1549, com dois companheiros e um intérprete, desembarcou no Japão, primeiro missionário cristão, e em três anos plantou comunidades em Kagoshima, Hirado, Yamaguchi e Kyoto. Voltou a Goa, organizou a missão e preparou-se para entrar na China. Faleceu sozinho, com um chinês cristão a seu lado, na pequena ilha de Sanchão, à vista do litoral chinês, em 3 de dezembro de 1552, aos 46 anos. Diz-se que batizou cerca de 30 mil pessoas. Padroeiro universal das missões católicas com Santa Teresinha desde 1927.",
    virtudes: ["Zelo missionário", "Coragem", "Adaptabilidade cultural", "Confiança em Deus"],
    frase: "Dai-me almas, levai o resto.",
  },
  {
    slug: "sao-filipe-neri",
    nome: "São Filipe Néri",
    titulo: "Sacerdote, fundador do Oratório, 'Apóstolo de Roma'",
    data: "26/05",
    seculo: "XVI",
    padroeiro: "Roma, jovens, humoristas, alegria cristã",
    resumo:
      "O santo da alegria, que reformou a Roma do Renascimento com brincadeiras e confessionário.",
    biografia:
      "Nasceu em Florença em 21 de julho de 1515, em família modesta. Deixou aos 18 anos a casa do tio que lhe propunha herança em San Germano e foi a Roma, onde viveu pobremente como preceptor e estudante de filosofia e teologia, em prolongada vida laical de oração nas catacumbas de São Sebastião. Em 1544, na vigília de Pentecostes, recebeu o 'fogo do Espírito Santo' que lhe dilatou fisicamente o coração — fenômeno verificado após a morte. Apóstolo nas ruas, abordava romanos com 'Quando começaremos a fazer o bem?'. Ordenado sacerdote em 1551 e instalado em San Girolamo della Carità, atraía com seu confessionário multidões; reunia leigos e clérigos em reuniões informais de oração, leitura espiritual, conversa, música — origem do Oratório. Em 1575 obteve a igreja Santa Maria in Vallicella, hoje 'Chiesa Nuova', e fundou a Congregação do Oratório, comunidade de sacerdotes seculares aprovada por Gregório XIII. Conselheiro de papas, cardeais (Carlos Borromeu, Cesar Baronio) e artistas, transformou a vida cristã da cidade pelo humor, a humildade e a alegria. Morreu em Roma em 26 de maio de 1595, aos 79 anos. Canonizado em 1622 com Inácio, Xavier, Teresa e Isidoro.",
    virtudes: ["Alegria cristã", "Humildade extravagante", "Direção espiritual paterna"],
    frase: "Preferiria a alegria à melancolia, qualquer dia.",
  },
  {
    slug: "sao-carlos-borromeu",
    nome: "São Carlos Borromeu",
    titulo: "Cardeal, Arcebispo de Milão, reformador",
    data: "04/11",
    seculo: "XVI",
    padroeiro: "Bispos, catequistas, seminaristas, contra a peste",
    resumo: "Arquiteto da reforma tridentina, modelo de bispo da Contrarreforma.",
    biografia:
      "Carlo Borromeo nasceu em Arona, à beira do Lago Maggiore, em 2 de outubro de 1538, da nobre família dos condes de Arona, sobrinho do papa Pio IV. Estudou direito em Pavia; tonsurado aos 12 anos para usufruir do título da abadia familiar, foi chamado a Roma pelo tio em 1559 e, aos 22 anos, criado cardeal e Secretário de Estado. Foi alma do reabrir e do encerramento do Concílio de Trento (1562-1563), e o principal redator do 'Catecismo Romano' e dos decretos de reforma. Mesmo arcebispo titular de Milão, residia em Roma; mas, ordenado sacerdote em 1563 e sagrado bispo, transferiu-se em 1565 para a sua diocese, onde realizou pessoalmente o programa tridentino: seis sínodos provinciais, onze sínodos diocesanos, fundação de seminários (modelo para toda a Igreja), visitas pastorais minuciosíssimas a todas as paróquias e vales alpinos, reforma do clero, das ordens religiosas e da catequese (criou as 'Escolas da Doutrina Cristã'). Durante a peste de 1576-1578, ficou na cidade, hipotecou os bens para alimentar os doentes e percorreu as ruas a pé com o crucifixo. Vida austeríssima de jejum, oração e disciplina. Morreu de exaustão em Milão em 3 de novembro de 1584, aos 46 anos.",
    virtudes: ["Austeridade pessoal", "Caridade pastoral", "Zelo reformador"],
    frase: "As almas conquistam-se de joelhos.",
  },
  {
    slug: "sao-francisco-de-sales",
    nome: "São Francisco de Sales",
    titulo: "Bispo de Genebra, Doutor da Igreja",
    data: "24/01",
    seculo: "XVI–XVII",
    padroeiro: "Jornalistas, escritores, surdos-mudos",
    resumo: "Apóstolo gentilíssimo do Chablais, mestre da devoção dos leigos.",
    biografia:
      "Nasceu no castelo de Sales, na Saboia, em 21 de agosto de 1567, primogênito de família nobre. Estudou em Paris e em Pádua, formando-se em direito e teologia. Renunciou à carreira de magistrado e ao casamento concertado para se fazer sacerdote em 1593. No mesmo ano, ofereceu-se para a missão do Chablais, região reconquistada pela Saboia ao calvinismo: durante quatro anos enfrentou solidão, frio, atentados e, sobretudo, a indiferença, distribuindo de porta em porta pequenos folhetos manuscritos — origem da imprensa católica e razão pela qual é padroeiro dos jornalistas. Em 1602 foi sagrado bispo de Genebra, residindo em Annecy (Genebra estava em mãos calvinistas). Pregou, catequizou e governou com tal doçura que dizia: 'Conquista-se mais com uma gota de mel do que com um barril de vinagre'. Em 1604 conheceu Joana Francisca de Chantal, com quem fundou em 1610 a Ordem da Visitação. Escreveu obras imortais: 'Introdução à vida devota' (manual de santidade para leigos) e 'Tratado do amor de Deus'. Morreu em Lyon em 28 de dezembro de 1622. Canonizado em 1665, Doutor da Igreja em 1877.",
    virtudes: ["Mansidão", "Paciência apostólica", "Equilíbrio espiritual"],
    frase: "Nada por força, tudo por amor.",
  },
  {
    slug: "santa-margarida-maria-alacoque",
    nome: "Santa Margarida Maria Alacoque",
    titulo: "Virgem visitandina, apóstola do Sagrado Coração",
    data: "16/10",
    seculo: "XVII",
    padroeiro: "Apostolado da Oração, devotos do Sagrado Coração",
    resumo:
      "A religiosa silenciosa de Paray-le-Monial a quem Cristo confiou a devoção a seu Coração.",
    biografia:
      "Nasceu em Lhautecour, na Borgonha, em 22 de julho de 1647. Doente longamente na infância, foi curada por intercessão de Nossa Senhora, a quem prometeu virgindade. Em 1671 entrou na Visitação de Paray-le-Monial. Pouco notada, sofrida por humildades em comunidade, relatou ter recebido entre 1673 e 1675 quatro grandes aparições do Sagrado Coração de Jesus — revelações privadas, que não pertencem ao depósito da fé, que lhe revelou o amor desprezado, instituiu a 'comunhão reparadora' nas primeiras sextas-feiras, pediu a 'Hora Santa' às quintas e a instituição da festa do Sagrado Coração na sexta após a oitava de Corpus Christi: 'Eis o Coração que tanto amou os homens'. Encontrou o apoio decisivo do jesuíta São Cláudio La Colombière, seu confessor, que reconheceu a autenticidade das visões e divulgou-as. Faleceu em Paray-le-Monial em 17 de outubro de 1690, aos 43 anos. Canonizada em 1920, sua mensagem inspirou Leão XIII a consagrar o gênero humano ao Sagrado Coração em 1899 e Pio XI a fixar a doutrina na encíclica 'Miserentissimus Redemptor'.",
    virtudes: ["Reparação", "Obediência", "Amor ardente a Cristo"],
    frase: "Eis o Coração que tanto amou os homens.",
  },
  {
    slug: "sao-vicente-de-paulo",
    nome: "São Vicente de Paulo",
    titulo: "Sacerdote, fundador, 'Pai dos Pobres'",
    data: "27/09",
    seculo: "XVI–XVII",
    padroeiro: "Obras de caridade, hospitais, missões populares",
    resumo: "Sacerdote camponês que organizou a caridade em escala nunca vista na Igreja.",
    biografia:
      "Nasceu em Pouy (hoje Saint-Vincent-de-Paul), na Gasconha, em 24 de abril de 1581, filho de pequenos agricultores. Ordenado sacerdote aos 19 anos, viveu desorientado entre ambições; segundo a tradição, foi capturado por corsários, escravo em Túnis e libertado convertendo o senhor renegado em 1607. De volta à França, capelão da rainha Margarida, tornou-se pároco de Clichy e de Châtillon, onde a miséria do campo o tocou profundamente. Aconselhado por Pierre de Bérulle, descobriu o tesouro do povo pobre. Fundou em 1617 as Confrarias de Caridade, em 1625 a Congregação da Missão (Lazaristas) para evangelizar o campo e formar clero, e em 1633, com Santa Luísa de Marillac, as Filhas da Caridade — primeira congregação feminina sem clausura, 'tendo por mosteiro as ruas das cidades'. Reformou seminários, evangelizou galés, recolheu enjeitados, socorreu provinciais devastadas pela guerra dos Trinta Anos, comprou liberdade de cristãos cativos. Conselheiro do Conselho de Consciência de Ana de Áustria. Morreu em Paris em 27 de setembro de 1660. Canonizado em 1737. Leão XIII o proclamou padroeiro universal das obras de caridade em 1885.",
    virtudes: ["Caridade organizada", "Humildade", "Mansidão pastoral"],
    frase: "Os pobres são nossos mestres.",
  },
  {
    slug: "sao-luis-maria-grignion-de-montfort",
    nome: "São Luís Maria Grignion de Montfort",
    titulo: "Sacerdote, fundador, missionário mariano",
    data: "28/04",
    seculo: "XVII–XVIII",
    padroeiro: "Devotos marianos, missionários populares",
    resumo: "O pregador da consagração total a Jesus por Maria.",
    biografia:
      "Nasceu em Montfort-sur-Meu, na Bretanha, em 31 de janeiro de 1673, primogênito de dezoito filhos. Estudou no colégio jesuíta de Rennes e, a pé, partiu para Paris em 1693, em pobreza extrema, para seguir o seminário sulpiciano. Ordenado em 1700, dedicou-se inicialmente aos pobres do hospital geral de Poitiers, onde conheceu Marie-Louise Trichet, com quem fundaria a congregação das Filhas da Sabedoria. Confrontado com hostilidades, peregrinou a Roma em 1706, recebendo de Clemente XI o título de 'missionário apostólico' e o mandato de pregar pelo norte da França. Por dezesseis anos percorreu Bretanha, Anjou, Poitou, Vendée, pregando missões populares de altíssimo impacto, restaurando calvários, propagando o rosário e a renovação das promessas batismais. Fundou também a Companhia de Maria (Padres Montfortinos). Escreveu obras-chave: 'Tratado da verdadeira devoção a Maria' (perdido por dois séculos e redescoberto em 1842), 'O Segredo de Maria', 'Amor da Eterna Sabedoria', e o magnífico 'Cântico'. Seu lema 'Totus Tuus' seria assumido por João Paulo II. Morreu em Saint-Laurent-sur-Sèvre, em 28 de abril de 1716. Canonizado em 1947.",
    virtudes: ["Devoção mariana", "Pobreza apostólica", "Zelo missionário"],
    frase: "Totus tuus ego sum, et omnia mea tua sunt.",
  },
  {
    slug: "santo-afonso-maria-de-ligorio",
    nome: "Santo Afonso Maria de Ligório",
    titulo: "Bispo, Doutor da Igreja, fundador dos Redentoristas",
    data: "01/08",
    seculo: "XVII–XVIII",
    padroeiro: "Moralistas, confessores",
    resumo: "Jurista convertido em apóstolo dos camponeses napolitanos e doutor da moral cristã.",
    biografia:
      "Nasceu em Marianella, perto de Nápoles, em 27 de setembro de 1696, em ilustre família nobre. Doutor em direito civil e canônico aos 16 anos, exerceu por oito anos com sucesso a advocacia. Após uma humilhante derrota num caso famoso em 1723, percebeu a vaidade do mundo e, diante de uma estátua de Maria, ouviu o chamado de Deus. Renunciou à profissão, recebeu a ordenação sacerdotal em 1726 e dedicou-se às 'capelas vespertinas' para os jovens dos bairros pobres de Nápoles. Em 1732 fundou em Scala a Congregação do Santíssimo Redentor — os Redentoristas — para evangelizar o campo abandonado pelo clero da capital. Apesar de oposições, intrigas e cismas internos, governou e dilatou a Ordem por toda a Itália. Sagrado bispo de Sant'Agata dei Goti em 1762, governou heroicamente até 1775. Escreveu mais de cem livros: a monumental 'Teologia Moral' (manual da Igreja por dois séculos), 'As Glórias de Maria', 'Visitas ao Santíssimo Sacramento', e a cantiga 'Tu scendi dalle stelle'. Quase cego e paralítico, sofreu dores atrozes e arteiras divisões. Morreu em Pagani em 1º de agosto de 1787. Canonizado em 1839, Doutor da Igreja em 1871, patrono dos confessores e moralistas em 1950.",
    virtudes: ["Misericórdia pastoral", "Equilíbrio moral", "Devoção mariana e eucarística"],
    frase: "Quem reza salva-se; quem não reza condena-se.",
  },
  {
    slug: "sao-joao-bosco",
    nome: "São João Bosco",
    titulo: "Sacerdote, educador, fundador dos Salesianos",
    data: "31/01",
    seculo: "XIX",
    padroeiro: "Jovens, educadores, aprendizes, mágicos",
    resumo: "O pai e mestre da juventude, que educou pela razão, religião e bondade.",
    biografia:
      "Giovanni Melchiorre Bosco nasceu em Becchi, no Piemonte, em 16 de agosto de 1815. Aos dois anos perdeu o pai. A mãe, Mamãe Margarida, formou-lhe o caráter. Aos nove anos teve o 'sonho profético' que orientaria sua vida: viu-se cercado de meninos turbulentos e ouviu: 'Não com pancadas, mas com mansidão e caridade conquista-os'. Estudou com sacrifícios extremos, foi ordenado sacerdote em 1841 e, na visita às prisões de Turim, sentiu-se chamado a salvar os adolescentes em risco da grande migração industrial. Em 1846 inaugurou em Valdocco o Oratório festivo permanente; oferecia aos rapazes pão, casa, oficina, escola, alegria, prevenção, religião. Fundou em 1859 a Sociedade Salesiana de São Francisco de Sales; em 1872, com Maria Domingas Mazzarello, as Filhas de Maria Auxiliadora; e em 1875 começou a obra missionária na Patagônia. Seu sistema 'preventivo' — razão, religião, amorevolezza — revolucionou a educação. Escreveu manuais, biografias edificantes (entre elas a do aluno Domingos Sávio) e a 'História eclesiástica'. Dotado de carismas extraordinários, profecias e sonhos, morreu em Turim em 31 de janeiro de 1888. Canonizado em 1934.",
    virtudes: ["Caridade preventiva", "Trabalho", "Alegria salesiana", "Devoção mariana"],
    frase: "Basta que sejais jovens para que eu vos ame.",
  },
  {
    slug: "sao-joao-maria-vianney",
    nome: "São João Maria Vianney",
    titulo: "Sacerdote, Cura d'Ars, padroeiro dos párocos",
    data: "04/08",
    seculo: "XVIII–XIX",
    padroeiro: "Párocos, confessores",
    resumo:
      "O humilde pároco do vilarejo perdido a quem afluía a França inteira em busca de absolvição.",
    biografia:
      "Jean-Marie Vianney nasceu em Dardilly, perto de Lyon, em 8 de maio de 1786, em família camponesa. Cresceu durante a Revolução Francesa, escondendo padres que celebravam em segredo. Vocação tardia e estudos penosos: a falta de aptidão para o latim quase impediu sua ordenação, alcançada em 1815 graças à insistência de seu reitor M. Balley. Coadjutor primeiro, em 1818 foi enviado à pequena Ars, vilarejo de 230 habitantes religiosamente decaído. Em poucos anos transformou a paróquia pela pregação simples, a oração, o jejum extremo (vivia de poucas batatas), as visitas e a luta espiritual contra ataques visíveis e audíveis do demônio, que apelidou de 'le grappin'. Desde 1827 começaram as peregrinações: nos últimos anos ouvia confissões 16 a 18 horas por dia, atendendo a mais de 100 mil pessoas por ano, lendo nos corações com dom de discernimento. Várias vezes tentou fugir para a Trapa, mas voltou. Diminuto, esquálido, exausto, jamais se queixou. Morreu em Ars em 4 de agosto de 1859. Canonizado por Pio XI em 1925, declarado padroeiro principal de todos os párocos em 1929.",
    virtudes: ["Penitência heroica", "Oração", "Direção espiritual penetrante"],
    frase: "O sacerdócio é o amor do Coração de Jesus.",
  },
  {
    slug: "santa-bernadette-soubirous",
    nome: "Santa Bernadette Soubirous",
    titulo: "Virgem, vidente de Lourdes",
    data: "16/04",
    seculo: "XIX",
    padroeiro: "Lourdes, doentes, pobres, pastores",
    resumo: "A menina pobre dos Pirineus que viu na gruta a 'Imaculada Conceição'.",
    biografia:
      "Marie-Bernarde Soubirous nasceu em Lourdes, em 7 de janeiro de 1844, primogênita de família muito pobre. Doente, asmática, analfabeta, ainda não fizera a primeira comunhão aos 14 anos. Em 11 de fevereiro de 1858, recolhendo lenha na gruta de Massabielle, à beira do Gave, viu uma 'Senhora' jovem, vestida de branco com faixa azul, que lhe sorria. Seguiram-se dezoito aparições até 16 de julho. A Senhora pediu penitência e oração pelos pecadores, fez surgir a fonte miraculosa, ordenou construir uma capela e processar em peregrinação. Em 25 de março, festa da Anunciação, finalmente disse: 'Eu sou a Imaculada Conceição', confirmando o dogma proclamado quatro anos antes por Pio IX. Bernadette atravessou interrogatórios policiais, eclesiásticos e médicos sem nunca se contradizer. Em 1866 entrou no Convento das Irmãs da Caridade de Nevers; consumiu-se em silêncio, sofrimentos contínuos e oração escondida. Recusava qualquer privilégio: 'Foi-me dito que devia ser uma boa religiosa, escondida'. Morreu em Nevers em 16 de abril de 1879, aos 35 anos. Seu corpo permanece incorrupto. Canonizada em 1933.",
    virtudes: ["Humildade", "Pobreza", "Penitência", "Simplicidade"],
    frase: "Sou enviada para vos dizer, não para vos fazer crer.",
  },
  {
    slug: "santa-teresinha-do-menino-jesus",
    nome: "Santa Teresinha do Menino Jesus",
    titulo: "Virgem carmelita, Doutora da Igreja",
    data: "01/10",
    seculo: "XIX",
    padroeiro: "Missões, floristas, aviadores, doentes de AIDS",
    resumo: "A 'pequena flor' de Lisieux, mestra do 'caminho da infância espiritual'.",
    biografia:
      "Marie-Françoise-Thérèse Martin nasceu em Alençon, na Normandia, em 2 de janeiro de 1873, caçula de nove filhos (cinco filhas religiosas) de Luís e Zélia Martin, ambos santos canonizados. Perdeu a mãe aos 4 anos. Sofreu profunda crise emocional na infância, curada na noite de Natal de 1886 por uma graça que chamou de 'conversão'. Em peregrinação a Roma em 1887, jogou-se aos pés de Leão XIII implorando entrar no Carmelo aos 15 anos — autorização concedida pelo bispo de Bayeux. Entrou no Carmelo de Lisieux em 9 de abril de 1888. Em nove anos de vida monástica escondida, descobriu o 'caminho pequeno' — não a heroicidade impossível, mas a confiança total e o abandono filial de uma criança que se deixa erguer no colo do Pai. Por ordem das superioras, escreveu sua autobiografia em três cadernos — a 'História de uma Alma' — que, publicada após sua morte, tornou-se um dos livros espirituais mais lidos do mundo. Tuberculosa desde 1896, morreu em Lisieux em 30 de setembro de 1897, dizendo: 'Meu Deus, eu vos amo'. Canonizada em 1925, padroeira universal das missões em 1927; terceira mulher Doutora da Igreja em 1997.",
    virtudes: ["Confiança total", "Pequenez", "Amor escondido", "Sorriso no sofrimento"],
    frase: "Quero passar o meu céu fazendo o bem na terra.",
  },
  {
    slug: "santa-edith-stein",
    nome: "Santa Edith Stein",
    titulo: "Virgem carmelita, mártir, 'Teresa Benedita da Cruz', Padroeira da Europa",
    data: "09/08",
    seculo: "XIX–XX",
    padroeiro: "Europa, vítimas do nazismo, conversões intelectuais",
    resumo:
      "Filósofa judia convertida, carmelita assassinada em Auschwitz pela sua estirpe e sua fé.",
    biografia:
      "Edith Stein nasceu em Breslávia, capital da Silésia, em 12 de outubro de 1891, no Yom Kippur judaico, décima primeira filha de família judia ortodoxa. Adolescente, declarou-se ateia. Brilhante estudante, foi discípula e assistente de Edmund Husserl em Gotinga e Friburgo, tornando-se uma das principais fenomenólogas da escola; sua tese sobre a empatia recebeu o sumo louvor 'summa cum laude'. Em 1921, lendo de uma só sentada a autobiografia de Teresa de Ávila, exclamou: 'Esta é a verdade'. Recebeu o batismo em 1922; lecionou em Speyer e em Münster, mas em 1933 as leis raciais nazistas a expulsaram da cátedra. Em 14 de outubro de 1933, entrou no Carmelo de Colônia, recebendo o nome de Teresa Benedita da Cruz. Escreveu obras de antropologia e mística — entre elas o 'Ser finito e ser eterno' e 'Ciência da Cruz' sobre João da Cruz, concluída pouco antes da morte. Em 1938, frente ao avanço da perseguição, transferiu-se ao Carmelo holandês de Echt. Em 2 de agosto de 1942, em retaliação à carta dos bispos holandeses contra a deportação dos judeus, foi presa com sua irmã Rosa. Morta na câmara de gás em Auschwitz em 9 de agosto de 1942. Canonizada em 1998, declarada Padroeira da Europa em 1999.",
    virtudes: ["Busca da verdade", "Imolação pelo seu povo", "Coragem intelectual"],
    frase: "Quem busca a verdade, mesmo sem saber, busca a Deus.",
  },
  {
    slug: "sao-maximiliano-maria-kolbe",
    nome: "São Maximiliano Maria Kolbe",
    titulo: "Sacerdote franciscano, mártir da caridade em Auschwitz",
    data: "14/08",
    seculo: "XIX–XX",
    padroeiro: "Famílias, drogados, jornalistas, prisioneiros, pró-vida",
    resumo: "Frade polonês que deu a vida pelo pai de família condenado ao bunker da fome.",
    biografia:
      "Raymund Kolbe nasceu em Zduńska Wola, Polônia, em 8 de janeiro de 1894. Aos 12 anos teve, segundo relato que ele mesmo transmitiu, a visão de Maria oferecendo-lhe duas coroas — branca da pureza e vermelha do martírio — e ele escolheu as duas. Entrou nos Franciscanos Conventuais em 1907, recebendo o nome de Maximiliano, depois Maria. Estudou em Roma onde obteve doutorados em filosofia e teologia. Em 1917, em pleno auge maçônico, fundou a 'Milícia da Imaculada' para a conversão dos inimigos da Igreja pela consagração mariana. Voltando à Polônia em 1919, criou em 1927 'Niepokalanów' (Cidade da Imaculada), maior convento franciscano do mundo, com gráfica, rádio e dezenas de revistas — 'O Cavaleiro da Imaculada' alcançou um milhão de tiragem. Entre 1930 e 1936 fundou em Nagasaki, Japão, uma segunda 'Cidade da Imaculada' na encosta protegida do Monte Hikosan — preservada da bomba atômica. Ao voltar à Polônia, foi preso pela Gestapo em fevereiro de 1941 e deportado a Auschwitz em maio. Em 29 de julho, após a fuga de um prisioneiro, dez homens foram condenados ao bunker da fome; o sargento Franciszek Gajowniczek gemeu pela mulher e filhos. Kolbe deu um passo à frente: 'Quero morrer no lugar deste'. Sobreviveu duas semanas a sede e fome cantando hinos; foi morto com injeção de fenol em 14 de agosto de 1941. Canonizado em 1982 como 'mártir da caridade'.",
    virtudes: ["Devoção mariana", "Caridade heroica", "Apostolado dos meios de comunicação"],
    frase: "Permite-me, Imaculada, que viva por ti, contigo, em ti.",
  },
  {
    slug: "sao-padre-pio-de-pietrelcina",
    nome: "São Padre Pio de Pietrelcina",
    titulo: "Sacerdote capuchinho, estigmatizado",
    data: "23/09",
    seculo: "XIX–XX",
    padroeiro: "Adolescentes, voluntários da Defesa Civil, alívio da dor",
    resumo: "Confessor incansável e místico das chagas, padre de San Giovanni Rotondo.",
    biografia:
      "Francesco Forgione nasceu em Pietrelcina, em 25 de maio de 1887, em família camponesa. Entrou aos 15 anos nos Capuchinhos com o nome de Pio. Ordenado em 1910, debilitadíssimo de saúde, foi enviado para o ar puro de San Giovanni Rotondo, na Apúlia, em 1916, onde permaneceria 52 anos. Em 5 de agosto de 1918 recebeu a 'transverberação' do coração; em 20 de setembro de 1918, no coro do convento, recebeu visivelmente os cinco estigmas, primeiros a manchar um sacerdote na história da Igreja, e os conservou sangrando até dois dias antes de morrer. Submetido a campanhas de difamação e a duríssimas restrições impostas em 1923, 1931 e 1933 pelo Santo Ofício (proibido de celebrar publicamente e confessar), aceitou tudo em obediência. Liberado em 1934, dedicou-se incansavelmente à confissão: até 18 horas diárias, lendo nos corações, jejuando, sustentado pela só Eucaristia. Em 1956 inaugurou a 'Casa Sollievo della Sofferenza', moderno hospital construído com as ofertas dos seus filhos espirituais, e fundou os 'Grupos de Oração' difundidos pelo mundo. Bilocação, perfume celeste, profecias e curas o cercaram. Morreu em 23 de setembro de 1968, com os estigmas desaparecidos. Canonizado em 2002.",
    virtudes: ["União com a Paixão", "Obediência heroica", "Caridade no confessionário"],
    frase: "Reza, espera e não te preocupes.",
  },
  {
    slug: "sao-joao-xxiii",
    nome: "São João XXIII",
    titulo: "Papa, 'o Papa Bom'",
    data: "11/10",
    seculo: "XIX–XX",
    padroeiro: "Padres patriarcas, exército italiano",
    resumo: "O Papa camponês de Sotto il Monte que abriu o Concílio Vaticano II.",
    biografia:
      "Angelo Giuseppe Roncalli nasceu em Sotto il Monte, perto de Bérgamo, em 25 de novembro de 1881, terceiro de treze filhos de família camponesa pobre. Ordenado em 1904, foi secretário do bispo de Bérgamo monsenhor Radini-Tedeschi, capelão militar na Grande Guerra, presidente nacional das Obras Missionárias, e a partir de 1925, diplomata: visitador apostólico na Bulgária, delegado na Turquia e Grécia (onde, durante a Segunda Guerra, salvou milhares de judeus expedindo certificados de batismo de trânsito), núncio em Paris (1944-1953) após a delicadíssima depuração do clero francês. Cardeal patriarca de Veneza em 1953. Eleito Papa em 28 de outubro de 1958, aos 77 anos, contra todas as expectativas, escolheu o nome João XXIII, e foi acolhido por todos como pastor de bondade transparente. Em 25 de janeiro de 1959, surpreendeu o mundo anunciando a convocação do Concílio Ecumênico Vaticano II, que abriu em 11 de outubro de 1962 com o discurso 'Gaudet Mater Ecclesia'. Publicou as encíclicas sociais 'Mater et Magistra' (1961) e 'Pacem in terris' (1963), endereçada a 'todos os homens de boa vontade'. Em pleno Concílio, em 3 de junho de 1963, morreu de câncer no estômago, cercado pelo afeto do mundo inteiro. Canonizado em 2014 com João Paulo II.",
    virtudes: ["Mansidão pastoral", "Coragem profética", "Bom humor cristão"],
    frase: "Voltai para casa e dai um abraço aos vossos filhos da parte do Papa.",
  },
  {
    slug: "sao-paulo-vi",
    nome: "São Paulo VI",
    titulo: "Papa, 'o Papa do diálogo'",
    data: "29/05",
    seculo: "XX",
    padroeiro: "Movimentos eclesiais, casados",
    resumo: "Conduziu e encerrou o Concílio Vaticano II e enfrentou a tempestade pós-conciliar.",
    biografia:
      "Giovanni Battista Montini nasceu em Concesio, perto de Bréscia, em 26 de setembro de 1897, filho de um deputado católico. Ordenado em 1920, ingressou na Secretaria de Estado em 1924, onde permaneceu por trinta anos. Foi assistente nacional da FUCI, formando uma geração de leigos católicos. Substituto da Secretaria de Estado sob Pio XII em 1937, depois Pró-Secretário para os Negócios Ordinários, organizou em silêncio o resgate de milhares de perseguidos durante a guerra. Arcebispo de Milão em 1954, criado cardeal por João XXIII em 1958, eleito Papa em 21 de junho de 1963. Conduziu as três sessões finais do Vaticano II (1963-1965) e empenhou-se em sua aplicação. Abraçou o patriarca Atenágoras em Jerusalém em 1964, gesto inédito desde 1054. Foi o primeiro Papa moderno a viajar (Terra Santa, Índia, ONU, Colômbia, Genebra, Uganda, Ásia oriental). Reformou a Cúria, promulgou o novo Missal (1969), publicou encíclicas decisivas — 'Ecclesiam suam', 'Populorum Progressio' (1967) sobre o desenvolvimento dos povos, 'Humanae vitae' (1968), defesa profética da vida que lhe valeu a 'noite' do pontificado, e a exortação 'Evangelii nuntiandi' (1975). Sofreu o assassinato de seu amigo Aldo Moro em 1978. Morreu em Castel Gandolfo em 6 de agosto de 1978. Canonizado em 2018.",
    virtudes: ["Fortaleza meditativa", "Diálogo", "Fidelidade ao Concílio"],
    frase: "A civilização do amor é o futuro da humanidade.",
  },
  {
    slug: "sao-joao-paulo-ii",
    nome: "São João Paulo II",
    titulo: "Papa, 'o Papa peregrino'",
    data: "22/10",
    seculo: "XX–XXI",
    padroeiro: "Famílias, jovens, Jornada Mundial da Juventude",
    resumo:
      "Karol Wojtyła, o Papa que atravessou o século e proclamou ao mundo: 'Não tenhais medo!'",
    biografia:
      "Karol Józef Wojtyła nasceu em Wadowice, Polônia, em 18 de maio de 1920. Perdeu a mãe aos 9 anos, o irmão médico aos 12, o pai aos 21. Estudante de filologia em Cracóvia, durante a ocupação nazista trabalhou em pedreira e fábrica química, foi ator do Teatro Rapsódico clandestino e, em 1942, entrou no seminário também clandestino do Cardeal Sapieha. Ordenado em 1946, doutorou-se em Roma sob Garrigou-Lagrange. Foi professor de ética em Lublin, bispo auxiliar de Cracóvia em 1958, arcebispo em 1964, cardeal em 1967. Participou ativamente do Vaticano II. Eleito Papa em 16 de outubro de 1978, primeiro não italiano em 455 anos. Inaugurou o pontificado com o grito 'Não tenhais medo, abri as portas a Cristo!'. Em 27 anos, realizou 104 viagens internacionais, beatificou 1340 e canonizou 482 servos de Deus, escreveu 14 encíclicas (Redemptor Hominis, Veritatis Splendor, Evangelium Vitae, Fides et Ratio), promulgou o Catecismo (1992) e o novo Código de Direito Canônico (1983), instituiu a Jornada Mundial da Juventude (1985), a festa da Divina Misericórdia (2000) e o Ano Mariano. Sobreviveu ao atentado em 13 de maio de 1981. Contribuiu decisivamente para o fim do comunismo no Leste europeu. Atravessou a doença com testemunho conmovedor. Morreu em 2 de abril de 2005, vigília da Divina Misericórdia. Canonizado em 2014.",
    virtudes: [
      "Coragem profética",
      "Devoção mariana ('Totus Tuus')",
      "Amor à juventude e à família",
    ],
    frase: "Não tenhais medo! Abri, escancarai as portas a Cristo!",
  },
  {
    slug: "santa-madre-teresa-de-calcuta",
    nome: "Santa Madre Teresa de Calcutá",
    titulo: "Virgem, fundadora das Missionárias da Caridade",
    data: "05/09",
    seculo: "XX–XXI",
    padroeiro: "Missionários da Caridade, Calcutá, pobres, moribundos",
    resumo: "Pequena albanesa que viu Cristo nos rostos dos mais pobres entre os pobres.",
    biografia:
      "Agnes Gonxha Bojaxhiu nasceu em Skopje (Macedônia, então Império Otomano), em 26 de agosto de 1910, em família albanesa católica. Aos 18 anos entrou nas Irmãs de Loreto em Dublin; partiu em 1929 para a Índia, fazendo a profissão religiosa com o nome de Maria Teresa. Lecionou por 17 anos no colégio Loreto de Calcutá, do qual foi diretora. Em 10 de setembro de 1946, no trem para retiro em Darjeeling, recebeu 'a chamada dentro da chamada': deixar tudo e seguir Cristo nos mais pobres dos pobres. Após dois anos de espera obediente, obteve em 1948 a exclaustração; em hábito branco de sari, sozinha, começou a recolher moribundos das ruas. Em 1950 fundou as Missionárias da Caridade, com o quarto voto de servir gratuitamente os mais pobres; abriu Nirmal Hriday (a 'Casa do Coração Puro') em 1952, lar para moribundos. A obra multiplicou-se: leprosários, casas para crianças abandonadas, AIDS, e mais de 600 casas em 130 países ao tempo de sua morte, com cerca de 5 mil irmãs e ramos masculinos e contemplativos. Recebeu o Prêmio Nobel da Paz em 1979. Viveu interiormente 'a noite escura' de quase 50 anos, revelada após a morte. Faleceu em Calcutá em 5 de setembro de 1997. Canonizada em 2016 pelo Papa Francisco.",
    virtudes: ["Caridade heroica", "Pobreza", "Fé na noite escura", "Sorriso a Jesus"],
    frase: "Faze qualquer coisa, mas faze-a com amor.",
  },
  {
    slug: "sao-josemaria-escriva",
    nome: "São Josemaría Escrivá",
    titulo: "Sacerdote, fundador do Opus Dei",
    data: "26/06",
    seculo: "XX",
    padroeiro: "Trabalhadores comuns, executivos",
    resumo: "Mensageiro da santificação do trabalho ordinário e da chamada universal à santidade.",
    biografia:
      "Josemaría Escrivá de Balaguer y Albás nasceu em Barbastro, Aragão, em 9 de janeiro de 1902, segundo de seis filhos. Perdeu três irmãzinhas na infância e o pai, arruinado nos negócios, aos quinze anos. Sentiu o chamado em 1917, vendo as pegadas de pés descalços de um carmelita na neve. Ordenado sacerdote em 1925, exerceu ministério em paróquias rurais e estudantes pobres em Madri. Em 2 de outubro de 1928, durante um retiro, recebeu de Deus a visão fundadora do Opus Dei: que homens e mulheres comuns, no exercício do trabalho profissional e em meio às realidades temporais, fossem chamados à plenitude da vida cristã — antecipação do ensinamento universal da chamada à santidade do Concílio Vaticano II. Atravessou as perseguições da Guerra Civil Espanhola (1936-1939) escondido e fugindo a pé pelos Pirineus. Em 1946 transferiu-se para Roma, onde dirigiu o crescimento da Obra: aprovação pontifícia em 1950, ereção em Prelazia Pessoal em 1982 por João Paulo II. Viajou por todo o mundo catequizando multidões em conversas espontâneas. Escreveu obras espirituais — 'Caminho', 'Sulco', 'Forja', 'Amigos de Deus' — lidas por milhões. Morreu repentinamente em Roma em 26 de junho de 1975. Canonizado em 2002 com a presença de 300 mil pessoas na praça de São Pedro.",
    virtudes: ["Santificação do trabalho", "Filiação divina", "Liberdade dos filhos de Deus"],
    frase: "Tudo o que é humano pode ser divino.",
  },
  {
    slug: "santo-oscar-romero",
    nome: "Santo Óscar Romero",
    titulo: "Bispo, mártir, Arcebispo de San Salvador",
    data: "24/03",
    seculo: "XX",
    padroeiro: "Catequistas, El Salvador, América Latina, perseguidos",
    resumo: "A 'voz dos sem voz' salvadorenho, abatido no altar por defender os pobres.",
    biografia:
      "Óscar Arnulfo Romero y Galdámez nasceu em Ciudad Barrios, El Salvador, em 15 de agosto de 1917, segundo de oito filhos. Aprendiz de carpinteiro, entrou aos treze anos no seminário menor; estudou em Roma na Universidade Gregoriana e foi ordenado em 1942. Pároco, secretário diocesano, depois bispo auxiliar e bispo de Santiago de María. Era considerado conservador e prudente quando foi nomeado Arcebispo de San Salvador em 22 de fevereiro de 1977, em meio à intensificação da repressão militar. O assassinato, apenas três semanas depois, do jesuíta Rutilio Grande, seu amigo, foi a conversão pastoral: assumiu publicamente a defesa dos pobres, dos camponeses massacrados, dos sequestrados, criou o Socorro Jurídico, e tornou-se 'a voz dos que não têm voz'. Suas homilias dominicais transmitidas pela rádio diocesana 'YSAX' alcançavam todo o país e parte da América Central, denunciando assassinatos com nomes e datas. Em 23 de março de 1980, depois de pedir, em nome de Deus, 'cesse a repressão!', sabia o risco. No dia seguinte, 24 de março de 1980, foi assassinado a tiros enquanto elevava o cálice na missa da capela do Hospital Divina Providência. Canonizado em 14 de outubro de 2018.",
    virtudes: ["Coragem profética", "Caridade pelos pobres", "Fidelidade até o sangue"],
    frase: "Se me matarem, ressuscitarei no povo salvadorenho.",
  },
  {
    slug: "santa-rita-de-cassia",
    nome: "Santa Rita de Cássia",
    titulo: "Religiosa agostiniana, 'a santa dos impossíveis'",
    data: "22/05",
    seculo: "XIV–XV",
    padroeiro: "Causas impossíveis, esposas em dificuldade, perdão",
    resumo:
      "Esposa, mãe e viúva antes de ser monja; carregou um espinho da coroa de Cristo na fronte.",
    biografia:
      "Margherita Lotti nasceu em Roccaporena, perto de Cássia, na Úmbria, por volta de 1381, filha tardia de pais idosos chamados de 'pacificadores'. Sentiu-se chamada à vida religiosa desde a infância, mas foi obrigada pelos pais a se casar aos doze anos com Paolo Mancini, homem rude e violento envolvido nas vendetas de Cássia. Por dezoito anos de paciência, oração e doçura, ganhou-o à mansidão. Quando, finalmente convertido, ele foi assassinado pelos rivais, Rita perdoou publicamente os assassinos e rezou para que seus dois filhos adolescentes não vingassem o pai — ambos morreram pouco depois, antes de cometer o crime, segundo a tradição em estado de graça. Viúva, pediu a entrada no mosteiro agostiniano de Santa Maria Madalena em Cássia. Recusada três vezes pelas irmãs por temor das vinganças familiares, foi conduzida ao mosteiro pela ação dos seus santos patronos (Agostinho, Nicolau de Tolentino, João Batista). Aos 56 anos, ouvindo um sermão sobre a coroa de espinhos, recebeu na fronte o estigma de um espinho que sangrou por quinze anos. Os últimos quatro anos passou-os na cama; pediu, no inverno, uma rosa e dois figos do jardim de Roccaporena — encontrados em pleno frio. Morreu em 22 de maio de 1457. Canonizada em 1900.",
    virtudes: ["Paciência heroica", "Perdão", "Fortaleza na dor"],
    frase: "Senhor, dai-me a graça de sofrer convosco.",
  },
  {
    slug: "sao-jorge",
    nome: "São Jorge",
    titulo: "Mártir, soldado, 'megalomártir'",
    data: "23/04",
    seculo: "III–IV",
    padroeiro: "Inglaterra, Catalunha, Geórgia, Portugal, soldados, cavaleiros, escoteiros",
    resumo:
      "Soldado romano da Capadócia que rasgou o edito de Diocleciano e venceu o dragão do paganismo.",
    biografia:
      "Pouco se sabe historicamente do soldado Jorge, oficial cristão do exército romano nascido na Capadócia ou na Palestina, por volta de 280, em família ilustre. Tribuno do exército imperial sob Diocleciano, sofreu martírio em Lida (Lod), na Palestina, por volta de 303, ao publicar-se o edito de perseguição aos cristãos: a tradição relata que rasgou publicamente o edito, distribuiu seus bens aos pobres e enfrentou prolongadas torturas — espinhos, roda de espadas, óleo fervente, veneno — antes de ser decapitado, convertendo pelo testemunho até a imperatriz Alexandra. Seu culto difundiu-se rapidamente em todo o Oriente e, a partir das Cruzadas, no Ocidente. A célebre lenda do dragão, narrada na 'Legenda Áurea' (séc. XIII), simboliza a vitória de Cristo sobre o paganismo: Jorge salva a princesa de Silene, na Líbia, transpassando a fera; sua imagem a cavalo, com lança e bandeira branca cruzada de vermelho, tornou-se ícone universal da cavalaria cristã. É padroeiro nacional da Inglaterra (desde Eduardo III), da Geórgia, da Catalunha, de Portugal e de inúmeras cidades. No Brasil, sua devoção é viva sobretudo no Rio de Janeiro.",
    virtudes: ["Coragem", "Fidelidade à fé", "Defesa dos fracos"],
    frase: "Sou cristão, e Cristo é o meu Rei.",
  },
  {
    slug: "sao-cristovao",
    nome: "São Cristóvão",
    titulo: "Mártir, padroeiro dos motoristas e viajantes",
    data: "25/07",
    seculo: "III",
    padroeiro: "Motoristas, viajantes, peregrinos, motociclistas",
    resumo: "O gigante que carregou nos ombros, sem saber, o Menino Jesus e o peso do mundo.",
    biografia:
      "A 'Legenda Áurea' de Jacopo da Varagine (séc. XIII) conserva os traços lendários do mártir cananeu Réprobo: um gigante de força extraordinária que, decidido a servir o senhor mais poderoso do mundo, deixou o rei pelo demônio, e o demônio quando descobriu que ele temia a Cruz de Cristo. Aconselhado por um eremita a servir Cristo carregando viajantes através de um rio caudaloso, foi um dia interpelado por um menino que pediu travessia. À medida que avançava, o menino se tornava mais e mais pesado; ao chegar à outra margem, o gigante, exausto, descobriu que carregara nos ombros o próprio Senhor que sustenta o mundo. Recebeu então o batismo e o nome de Christophoros, 'portador de Cristo'. Pregou em seguida na Lícia, na Ásia Menor, foi preso, sofreu prolongado martírio sob o imperador Décio e foi decapitado por volta de 250. Historicamente, seu culto está atestado desde o século V em Cálcedon e foi popularíssimo em todo o Ocidente medieval, em que a contemplação de sua imagem grandiosa pintada nas fachadas das igrejas era considerada preservativo contra a morte súbita do viajante. Padroeiro universal dos transportes, dos motoristas e dos viajantes.",
    virtudes: ["Força a serviço", "Conversão a Cristo", "Coragem no martírio"],
    frase: "Quem busca o maior dos Senhores, só em Cristo o encontra.",
  },
  {
    slug: "sao-miguel-arcanjo",
    nome: "São Miguel Arcanjo",
    titulo: "Arcanjo, príncipe das milícias celestes",
    data: "29/09",
    seculo: "—",
    padroeiro: "Igreja, soldados, policiais, paraquedistas, doentes graves, exorcistas",
    resumo: "'Quem como Deus?' — o anjo que venceu Lúcifer e defende a Igreja peregrina.",
    biografia:
      "Miguel — em hebraico 'Mi-ka-El', 'Quem é como Deus?' — é o grande arcanjo nomeado nas Escrituras. Daniel o chama 'o grande Príncipe que defende o teu povo'; a Carta de Judas o mostra disputando com o diabo o corpo de Moisés; o Apocalipse o vê combater o dragão na batalha celeste e precipitá-lo, com seus anjos rebeldes, fora do céu. A Tradição lhe atribui quatro ofícios: combater Satanás, salvar das suas mãos as almas dos fiéis sobretudo na hora da morte, proteger o povo de Deus, e conduzir as almas dos defuntos para o juízo divino. É o anjo da Igreja militante, particularmente invocado contra as forças do mal. Seu culto difundiu-se desde o século IV no monte Gárgano, na Apúlia, após uma célebre aparição; depois em Roma sobre o Mausoléu de Adriano (Castel Sant'Angelo) após a peste de 590; e na França, no Mont-Saint-Michel, em 708. Leão XIII, em 1886, após visão durante missa, compôs a oração que se rezou ao final de toda missa rezada até 1965, e que São João Paulo II e Francisco recomendaram retomar nos tempos atuais: 'São Miguel Arcanjo, defendei-nos no combate'. Celebrado com Gabriel e Rafael na liturgia dos Santos Arcanjos.",
    virtudes: ["Adoração", "Combate espiritual", "Proteção"],
    frase: "Quis ut Deus? — Quem como Deus?",
  },
  {
    slug: "santa-faustina-kowalska",
    nome: "Santa Faustina Kowalska",
    titulo: "Virgem, religiosa, apóstola da Divina Misericórdia",
    data: "05/10",
    seculo: "XX",
    padroeiro: "Devotos da Divina Misericórdia, pecadores",
    resumo: "A religiosa polonesa a quem Jesus revelou a imagem da Misericórdia e o terço.",
    biografia:
      "Helena Kowalska nasceu em Głogowiec, Polônia, em 25 de agosto de 1905, terceira de dez filhos camponeses. Mal pôde frequentar a escola; trabalhou como empregada doméstica para ajudar a família. Aos 19 anos, durante uma dança em Łódź, viu Cristo sofredor que lhe perguntou: 'Até quando vou esperar-te?'. Entrou em 1925 na Congregação das Irmãs de Nossa Senhora da Misericórdia em Varsóvia, com o nome de Maria Faustina. Cumpriu tarefas humildes — cozinha, jardim, portaria — em diversos conventos. Em 22 de fevereiro de 1931, em Płock, recebeu a primeira aparição do Senhor da Divina Misericórdia, vestido de branco, com a mão direita erguida em bênção e a esquerda no peito, donde brotavam dois raios — branco (água do batismo) e vermelho (sangue eucarístico) —, com a invocação 'Jesus, eu confio em vós'. Seguiram-se anos de profundas graças místicas, registradas em seu 'Diário' por ordem de seus diretores, sobretudo o beato padre Sopoćko e o jesuíta padre Andrasz. Recebeu o pedido da festa da Divina Misericórdia (primeiro domingo após a Páscoa), da Hora da Misericórdia (15h, hora da agonia) e do Terço da Divina Misericórdia. Sofreu tuberculose e profundas escuridões interiores. Morreu em Cracóvia em 5 de outubro de 1938, aos 33 anos. Canonizada em 2000 por João Paulo II, que instituiu na mesma data a festa universal da Divina Misericórdia.",
    virtudes: ["Confiança total na misericórdia", "Humildade", "Oferenda pelos pecadores"],
    frase: "Jesus, eu confio em vós.",
  },
  {
    slug: "sao-charbel-makhlouf",
    nome: "São Charbel Makhlouf",
    titulo: "Sacerdote maronita, monge, eremita",
    data: "24/07",
    seculo: "XIX",
    padroeiro: "Líbano, Igreja maronita, curas extraordinárias",
    resumo:
      "O eremita silencioso do monte Líbano cujo corpo intacto continua a derramar curas pelo mundo.",
    biografia:
      "Youssef Antoun Makhlouf nasceu em Bekaa Kafra, no norte do Líbano, em 8 de maio de 1828, em humilde família maronita. Perdeu o pai aos três anos. Cresceu pastoreando, em oração contínua, atraído pela vida monástica. Aos 23 anos deixou em segredo a família e entrou no mosteiro de Nossa Senhora de Mayfouk e, depois, no mosteiro de São Marun de Annaya, da Ordem Libanesa Maronita, tomando o nome do mártir Charbel do século II. Estudou em Kfifan, ali tendo por mestre São Nimatullah Hardini. Foi ordenado sacerdote em 1859. Após dezesseis anos de exemplar vida cenobítica, obteve permissão em 1875 para retirar-se à eremida de Sant'Anna, anexa ao mosteiro, onde viveu 23 anos no mais rigoroso silêncio, jejum perpétuo, oração noturna, mortificação extrema e celebração diária da divina liturgia, da qual fazia o centro do dia. Não saía senão para cultivar um pequeno horto. Em 16 de dezembro de 1898, ao celebrar a missa, foi atingido por um derrame; agonizou durante oito dias, repetindo o triságio. Morreu no Natal de 1898. Logo após o sepultamento, luminosidade extraordinária envolveu sua tumba, e seu corpo, exumado várias vezes, foi achado incorrupto e exsudando 'sangue e água'. Canonizado em 1977 por Paulo VI.",
    virtudes: ["Silêncio", "Penitência", "União eucarística", "Hesicasmo cristão"],
    frase: "Pai da Verdade, eis o vosso Filho, vítima imolada que vos agrada.",
  },
  {
    slug: "sao-frei-galvao",
    nome: "São Frei Galvão",
    titulo: "Sacerdote franciscano, primeiro santo brasileiro nato",
    data: "25/10",
    seculo: "XVIII–XIX",
    padroeiro: "Gestantes, parturientes, Brasil",
    resumo: "O frade paulista das pílulas milagrosas, fundador do Recolhimento da Luz.",
    biografia:
      "Antônio de Sant'Anna Galvão nasceu em Guaratinguetá, capitania de São Paulo, em 10 de maio de 1739, décimo filho do capitão-mor português Antônio Galvão de França e da brasileira Isabel Leite. Estudou com os jesuítas em Belém de Cachoeira, na Bahia. Aos 21 anos entrou nos Franciscanos do convento de Macacu, no Rio de Janeiro; ordenado sacerdote em 1762, foi enviado a São Paulo para estudos teológicos e pregação. Confessor e diretor espiritual em São Paulo, ligou-se em 1769 a Sóror Helena Maria do Espírito Santo, monja recolhida da capela de Santa Teresa, que recebia revelações. Após a morte dela, em 1774, assumiu a continuidade da obra e fundou em 1774 o Recolhimento de Nossa Senhora da Conceição da Divina Providência, hoje Mosteiro da Luz, em São Paulo, mãe das Concepcionistas Franciscanas brasileiras. Acompanhou pessoalmente cada detalhe da construção, mendigando esmolas, dirigindo as obras, atendendo confissões em fila. Inventou as 'pílulas de Frei Galvão' — minúsculas pilulinhas de papel arroz com a inscrição mariana 'Post partum, Virgo inviolata permansisti…' —, distribuídas com fé como remédio para enfermidades e partos difíceis, ainda hoje preparadas no mosteiro. Morreu em São Paulo em 23 de dezembro de 1822. Beatificado em 1998, canonizado em 11 de maio de 2007 em São Paulo por Bento XVI: primeiro santo nascido em terras brasileiras.",
    virtudes: ["Caridade contínua", "Pobreza franciscana", "Devoção mariana"],
    frase: "Tudo seja para a maior glória de Deus.",
  },
  {
    slug: "santa-paulina-do-coracao-agonizante",
    nome: "Santa Paulina do Coração Agonizante",
    titulo: "Virgem, primeira santa do Brasil",
    data: "09/07",
    seculo: "XIX–XX",
    padroeiro: "Doentes de diabetes, imigrantes italianos no Brasil",
    resumo:
      "Imigrante italiana de Vígolo Vattaro que fundou no sul do Brasil a Congregação das Pequenas Irmãs.",
    biografia:
      "Amabile Lúcia Visintainer nasceu em Vígolo Vattaro, então pertencente ao Império Austro-Húngaro (hoje Trentino, Itália), em 16 de dezembro de 1865, primogênita de família camponesa católica. Em 1875, emigrou com os pais para o sul do Brasil, instalando-se em Vígolo, núcleo da nova colônia italiana junto a Nova Trento, em Santa Catarina. Profundamente religiosa, dedicou-se desde a juventude ao catecismo das crianças, ao cuidado dos doentes nas casas pobres e à oração contemplativa. Em 12 de julho de 1890, com a companheira Virgínia Rosa Nicolodi e o apoio do pároco padre Luigi Rossi, recolheu numa pequena casa de Vígolo uma anciã enferma e abandonada — gesto considerado fundação da Congregação das Pequenas Irmãs da Imaculada Conceição. Recebeu o hábito em 7 de dezembro de 1895 com o nome de Paulina do Coração Agonizante de Jesus. Eleita superiora-geral, transferiu a casa-mãe para São Paulo em 1903, ampliou a obra para escolas, asilos e hospitais. Em 1909, sob duras incompreensões e calúnias, foi destituída e enviada como simples irmã ao Asilo Santa Catarina, em Bragança Paulista, onde permaneceu humilde, doente e silenciosa por muitos anos. Reabilitada antes da morte, padeceu longa agonia de diabetes e cegueira. Morreu em São Paulo em 9 de julho de 1942. Canonizada em 19 de maio de 2002 por João Paulo II: primeira santa brasileira por adoção.",
    virtudes: ["Caridade pelos doentes", "Humildade na injustiça", "Confiança no Coração de Jesus"],
    frase: "Vontade de Deus, paraíso meu.",
  },
  {
    slug: "santa-dulce-dos-pobres",
    nome: "Santa Dulce dos Pobres",
    titulo:
      "Religiosa, fundadora, 'Anjo Bom da Bahia' — nome de nascimento: Maria Rita de Sousa Brito Lopes Pontes",
    data: "13/08",
    seculo: "XX",
    padroeiro: "Doentes pobres, Bahia, obras sociais",
    resumo:
      "A franciscana de Salvador que abrigou os pobres num galinheiro e ergueu a maior obra social do Nordeste.",
    biografia:
      "Batizada Maria Rita de Sousa Brito Lopes Pontes — nome de nascimento da santa, conhecida na Igreja como Santa Dulce dos Pobres —, nasceu em Salvador, Bahia, em 26 de maio de 1914, filha de família próspera de dentistas. Aos 13 anos, ao testemunhar a miséria dos bairros operários, prometeu dedicar-se aos pobres. Entrou em 1933 na Congregação das Irmãs Missionárias da Imaculada Conceição em São Cristóvão (Sergipe), recebendo o nome de Dulce, em homenagem à mãe falecida. Voltou a Salvador como professora; fundou em 1936 a União Operária São Francisco e o Círculo Operário da Bahia, primeiros sindicatos católicos da Bahia. Em 1939, viu na rua um homem doente abandonado: recolheu-o, recolheu outros, ocupou casas abandonadas e finalmente, em 1949, o galinheiro do Convento de Santo Antônio em Salvador, onde acomodou os primeiros sessenta enfermos sob o lema 'Aqui se cura, mas sobretudo aqui se ama'. Cresceu dali a Obras Sociais Irmã Dulce, hoje complexo hospitalar e assistencial entre os maiores do Brasil. Atendia 16 a 20 horas por dia, vivia em cela paupérrima, pesava no fim da vida apenas 40 quilos por um pulmão atrofiado. Foi visitada em 1980 pelo papa João Paulo II; a frase 'sereis a santa do Brasil', a ele atribuída nessa ocasião, é transmitida pela tradição biográfica e não por documento oficial. Morreu em Salvador em 13 de março de 1992. Canonizada em 13 de outubro de 2019 por Francisco — primeira santa nascida no Brasil.",
    virtudes: ["Caridade incansável", "Pobreza", "Confiança na Providência"],
    frase: "Quem ama, não cansa nunca.",
  },
  {
    slug: "sao-jose-de-anchieta",
    nome: "São José de Anchieta",
    titulo: "Sacerdote jesuíta, 'Apóstolo do Brasil'",
    data: "09/06",
    seculo: "XVI–XVII",
    padroeiro: "Catequistas, professores, missionários no Brasil",
    resumo: "Canário de Tenerife que se tornou poeta, lingüista e pacificador do Brasil colonial.",
    biografia:
      "José de Anchieta nasceu em San Cristóbal de La Laguna, Tenerife (Ilhas Canárias), em 19 de março de 1534, em família basca de origem nobre. Estudou em Coimbra, onde aos 17 anos entrou na Companhia de Jesus, em 1551. Padecendo de tuberculose óssea da coluna, foi enviado em 1553 ao Brasil em busca de clima salutar. Desembarcou aos 19 anos com o provincial Manuel da Nóbrega. Em 1554 esteve entre os fundadores do colégio de São Paulo de Piratininga, em cuja missa de 25 de janeiro inaugural — núcleo da futura cidade — começou seu apostolado entre os índios. Aprendeu rapidamente o tupi e compôs a 'Arte de grammatica da lingoa mais usada na costa do Brasil' (1595), primeira gramática indígena americana. Em 1563, durante negociações de paz com os tamoios aliados aos franceses, ficou refém em Iperoig (atual Ubatuba) por cinco meses, durante os quais — por falta de papel — compôs, segundo a tradição, na areia da praia o poema mariano em latim 'De Beata Virgine Dei Matre Maria', 4172 versos memorizados e copiados depois no convento. Foi pacificador na guerra contra os franceses no Rio, ordenado sacerdote em 1566, e em 1577 provincial dos jesuítas no Brasil. Pregador, dramaturgo, poeta, médico improvisado, fundou ou consolidou as casas de Piratininga, Rio, Espírito Santo, Bahia e Reritiba (Anchieta, ES), onde morreu em 9 de junho de 1597. Beatificado em 1980, canonizado em 2014 por Francisco.",
    virtudes: ["Zelo missionário", "Doçura com os indígenas", "Inteligência inculturada"],
    frase: "Os indígenas são minha glória e a coroa que me espera.",
  },
  {
    slug: "santa-maria-goretti",
    nome: "Santa Maria Goretti",
    titulo: "Virgem e mártir, 'a pequena mártir da pureza'",
    data: "06/07",
    seculo: "XIX–XX",
    padroeiro: "Jovens, vítimas de abuso, paz familiar, perdão",
    resumo: "Camponesa de 11 anos que preferiu morrer a perder a inocência e perdoou o agressor.",
    biografia:
      "Maria Teresa Goretti nasceu em Corinaldo, perto de Ancona, em 16 de outubro de 1890, terceira de seis filhos de família camponesa pobre. Em 1899 a família migrou às pântanos pontinos, na região do Lácio, em busca de trabalho. Em 1900 perdeu o pai de malária. A mãe, sozinha com seis filhos, prosseguiu como meeira em sociedade com a família Serenelli. Maria, profundamente piedosa, fez a primeira comunhão em 29 de maio de 1902 com fervor extraordinário. Em 5 de julho de 1902, o vizinho Alessandro Serenelli, jovem de 19 anos perturbado pela leitura de revistas obscenas, encontrou-a sozinha em casa, tentou possuí-la à força e, ante a resistência ('Não, é pecado, Deus não quer!'), apunhalou-a quatorze vezes. Maria sobreviveu vinte horas em agonia atroz no hospital de Nettuno; antes de morrer perdoou explicitamente o agressor: 'Eu o perdoo, e quero que esteja comigo no paraíso'. Morreu em 6 de julho de 1902, aos 11 anos, com a imagem de Nossa Senhora nos lábios. Serenelli, condenado a trinta anos de prisão, converteu-se em 1910, segundo seu próprio relato, após uma visão de Maria que lhe entregava açucenas; libertado em 1929, pediu publicamente perdão à mãe da menina e tornou-se irmão leigo capuchinho. Maria foi canonizada em 1950 por Pio XII na praça de São Pedro, com a presença da mãe e do próprio agressor convertido — caso único na história.",
    virtudes: ["Pureza", "Perdão", "Fortaleza juvenil"],
    frase: "Não, é pecado; Deus não quer.",
  },
  {
    slug: "ns-aparecida",
    nome: "Nossa Senhora Aparecida",
    titulo: "Padroeira do Brasil",
    data: "12/10",
    seculo: "—",
    padroeiro: "Brasil, pescadores, lavradores",
    resumo:
      "A pequena imagem encontrada nas redes do rio Paraíba que se tornou a Mãe de todo o povo brasileiro.",
    biografia:
      "Em outubro de 1717, três humildes pescadores — Domingos Garcia, Filipe Pedroso e João Alves — saíram para pescar nas águas turvas do rio Paraíba do Sul, em Guaratinguetá, São Paulo, por encomenda do conde de Assumar, em trânsito pela região. Após muitas tentativas sem sucesso, lançaram a rede junto ao porto de Itaguaçu e retiraram o corpo de uma pequena imagem da Imaculada Conceição em terracota escura, com cerca de 39 cm. Numa segunda lançada, recolheram a cabeça correspondente. A partir daquele instante, a pesca tornou-se milagrosamente abundante. A imagem, atribuída ao escultor beneditino Frei Agostinho de Jesus por volta de 1650, foi venerada inicialmente na casa de Filipe Pedroso e seu filho Atanásio Pedroso, que abriu pequena capela em sua honra. Em 1745 foi inaugurada a primeira capela pública e em 1834 a Basílica Velha. A fama dos milagres difundiu-se: a libertação do escravo Zacarias das correntes, em 1850, é o mais célebre. Coroada em 1904 pelo papa Pio X através do bispo de São Paulo, foi proclamada Padroeira oficial do Brasil em 16 de julho de 1930 por Pio XI. A nova Basílica Nacional de Aparecida, segundo maior templo católico do mundo, foi consagrada em 1980 por João Paulo II. Recebe anualmente mais de 13 milhões de peregrinos.",
    virtudes: ["Maternidade espiritual", "Humildade", "Proximidade com o povo"],
    frase: "Sob a vossa proteção nos refugiamos, Santa Mãe de Deus.",
  },
  {
    slug: "bem-aventurado-carlo-acutis",
    nome: "Bem-Aventurado Carlo Acutis",
    titulo: "Leigo adolescente, 'apóstolo da Eucaristia na era digital'",
    data: "12/10",
    seculo: "XX–XXI",
    padroeiro: "Jovens, internautas, programadores, primeira comunhão",
    resumo:
      "Adolescente milanês que catalogou os milagres eucarísticos do mundo num site e morreu santo aos 15 anos.",
    biografia:
      "Carlo Acutis nasceu em Londres em 3 de maio de 1991, único filho de pais italianos — Andrea Acutis e Antonia Salzano. Ainda bebê voltou a Milão. Cresceu em ambiente burguês não particularmente praticante; foi ele quem trouxe a mãe de volta à fé com perguntas e exemplo. Recebeu a primeira comunhão antecipadamente aos 7 anos, depois de longa preparação, e desde então participou da missa diária e da adoração eucarística — 'a minha autoestrada para o céu'. Sentia particular ternura pela Virgem Maria, rezando todos os dias o rosário. Estudante brilhante, dedicava-se à programação informática, criando websites para a paróquia e para uma associação de voluntariado. Em 2002, com 11 anos, começou seu projeto mais conhecido: catalogar e publicar online todos os 187 milagres eucarísticos reconhecidos pela Igreja no mundo, com fotografias, decretos e descrições — a exposição itinerante decorrente já passou por mais de dez mil paróquias em cinco continentes. Generoso com pobres da estação central de Milão, alegre, amigo querido entre colegas, defensor de adolescentes intimidados, futebolístico (torcedor do Milan), comum em tudo, extraordinário no amor a Cristo eucarístico. Em outubro de 2006 foi diagnosticado com leucemia fulminante. Ofereceu seus sofrimentos pelo Papa e pela Igreja. Morreu em Monza em 12 de outubro de 2006, aos 15 anos. Beatificado em Assis em 10 de outubro de 2020. Canonizado em 2025, é o primeiro santo da geração millennial.",
    virtudes: ["Eucaristia", "Castidade adolescente", "Originalidade no seguir Cristo"],
    frase: "Todos nascem originais, mas muitos morrem como fotocópias.",
  },
  {
    slug: "santa-helena",
    nome: "Santa Helena",
    titulo: "Imperatriz, descobridora da Santa Cruz",
    data: "18/08",
    seculo: "III–IV",
    padroeiro: "Arqueólogos, conversões difíceis, mães de filhos convertidos",
    resumo: "Mãe do imperador Constantino, peregrina à Terra Santa e descobridora da Vera Cruz.",
    biografia:
      "Flávia Júlia Helena nasceu por volta de 248, provavelmente em Drepano da Bitínia, de família modesta — segundo Santo Ambrósio, foi hospedeira de estalagem. Uniu-se ao oficial romano Constâncio Cloro, de quem teve Constantino, e foi repudiada quando o marido ascendeu à tetrarquia. Reabilitada pelo filho após 306, recebeu o título de Augusta. Converteu-se ao cristianismo já madura, cerca de 312, e dedicou o resto da vida à caridade: libertou prisioneiros, socorreu pobres e fundou igrejas. Por volta de 326, com quase oitenta anos, empreendeu a célebre peregrinação à Palestina. Mandou demolir o templo pagão erguido sobre o Gólgota e, segundo a tradição transmitida por Rufino, Sócrates e Santo Ambrósio, encontrou ali três cruzes, entre as quais a Vera Cruz teria sido reconhecida, segundo essa mesma tradição, pela cura de uma enferma. Promoveu a construção da Basílica do Santo Sepulcro, da Natividade em Belém e da Ascensão no Monte das Oliveiras. Morreu por volta de 330, provavelmente em Nicomédia, junto ao filho. Suas relíquias são veneradas em Roma, na igreja de Ara Coeli, e a Basílica de Santa Cruz em Jerusalém, em Roma, guarda a terra que ela trouxe do Calvário.",
    virtudes: ["Fé madura", "Caridade com os pobres", "Zelo pelos lugares santos"],
    frase: "Que a Cruz de Cristo seja a glória de todo o império.",
  },
  {
    slug: "sao-nicolau-de-mira",
    nome: "São Nicolau de Mira",
    titulo: "Bispo taumaturgo, padroeiro das crianças",
    data: "06/12",
    seculo: "IV",
    padroeiro: "Crianças, marinheiros, comerciantes, noivas pobres",
    resumo:
      "Bispo de Mira na Lícia, célebre pela generosidade escondida e pelos milagres em favor dos pobres.",
    biografia:
      "Nicolau nasceu em Patara, na Lícia (atual Turquia), por volta de 270, filho único de pais cristãos abastados que morreram numa epidemia. Herdeiro de grande fortuna, resolveu distribuí-la secretamente. O episódio mais famoso, transmitido desde o séc. VI, narra que salvou três jovens da prostituição lançando de noite, pela janela, bolsas de ouro que serviram de dote — origem remota do costume dos presentes natalinos e da figura do Papai Noel. Eleito bispo de Mira ainda jovem, governou a diocese com firmeza pastoral. Sofreu prisão e tortura durante a perseguição de Diocleciano e foi libertado com o Edito de Milão em 313. A tradição o coloca entre os padres do Concílio de Niceia (325), defensor ardoroso da divindade de Cristo contra Ário. Intercedeu junto ao imperador pela vida de três oficiais condenados injustamente e, segundo os relatos, teria acalmado tempestades em favor de marinheiros. Morreu em Mira por volta de 343. Em 1087 mercadores italianos transladaram suas relíquias para Bari, onde a basílica venera o fenômeno tradicionalmente chamado 'manná di San Nicola'. É um dos santos mais venerados do Oriente e do Ocidente.",
    virtudes: ["Generosidade oculta", "Defesa da fé", "Proteção dos indefesos"],
    frase: "Dai sem que a mão esquerda saiba o que faz a direita.",
  },
  {
    slug: "sao-martinho-de-tours",
    nome: "São Martinho de Tours",
    titulo: "Bispo, pai do monaquismo na Gália",
    data: "11/11",
    seculo: "IV",
    padroeiro: "Soldados, alfaiates, mendigos, França",
    resumo:
      "Soldado romano que partiu o manto com um mendigo e tornou-se bispo missionário da Gália.",
    biografia:
      "Martinho nasceu em Sabária, na Panônia (atual Hungria), por volta de 316, filho de um tribuno militar pagão. Ainda menino, em Pavia, foi atraído pelo cristianismo e inscreveu-se como catecúmeno aos dez anos. Obrigado pela lei a servir no exército, foi cavaleiro da guarda imperial em Amiens. Ali ocorreu o episódio que o imortalizou: num inverno rigoroso, cortou o próprio manto militar em dois com a espada e deu metade a um mendigo nu; na noite seguinte viu em sonho Cristo revestido daquele meio manto, dizendo aos anjos: 'Martinho, ainda catecúmeno, cobriu-me com este manto'. Batizado, deixou as armas declarando-se 'soldado de Cristo'. Discípulo de Santo Hilário de Poitiers, fundou em Ligugé o primeiro mosteiro do Ocidente, cerca de 361. Aclamado bispo de Tours pelo povo em 371 — apesar da resistência dele, que se escondeu num galinheiro —, continuou vivendo como monge em Marmoutier. Evangelizou incansavelmente o campo gaulês, destruindo santuários pagãos e fundando paróquias rurais, e defendeu os priscilianistas condenados à morte, opondo-se à pena capital em matéria de fé. Morreu em Candes em 397. Sua capa (cappa) guardada como relíquia deu origem às palavras 'capela' e 'capelão'.",
    virtudes: ["Misericórdia com os pobres", "Renúncia militar", "Zelo missionário"],
    frase: "Senhor, se ainda sou necessário ao teu povo, não recuso o trabalho.",
  },
  {
    slug: "santo-isidoro-de-sevilha",
    nome: "Santo Isidoro de Sevilha",
    titulo: "Bispo e Doutor da Igreja, último Padre latino",
    data: "04/04",
    seculo: "VI–VII",
    padroeiro: "Estudantes, enciclopedistas, internet e informática",
    resumo:
      "Arcebispo hispano-romano que salvou a cultura antiga nas Etimologias e organizou a Igreja visigótica.",
    biografia:
      "Isidoro nasceu em Cartagena por volta de 560, numa família de santos: seus irmãos Leandro, Fulgêncio e Florentina são igualmente venerados. Educado pelo irmão mais velho Leandro, arcebispo de Sevilha, sucedeu-lhe na sé por volta de 600 e governou-a por quase quarenta anos. Viveu no delicado momento em que os visigodos, recém-convertidos do arianismo, precisavam de unidade doutrinal e institucional: presidiu o II Concílio de Sevilha (619) e o IV Concílio de Toledo (633), que impôs a criação de escolas catedralícias em toda diocese, unificou a liturgia hispânica e defendeu o batismo livre dos judeus contra conversões forçadas. Sua obra maior, as Etimologias, em vinte livros, compila em ordem sistemática todo o saber antigo — gramática, medicina, direito, agricultura, cosmologia, teologia — e foi o manual básico da Europa por oito séculos. Escreveu também a História dos godos, o De natura rerum e as Sentenças. Morreu em Sevilha em 4 de abril de 636, tendo distribuído seus bens aos pobres nos últimos dias. Foi proclamado Doutor da Igreja em 1722 e é hoje popularmente invocado como patrono da internet, pela intuição enciclopédica de sua obra.",
    virtudes: ["Amor ao estudo", "Zelo pastoral", "Serviço à unidade da Igreja"],
    frase: "A oração nos purifica; a leitura nos instrui.",
  },
  {
    slug: "sao-beda-veneravel",
    nome: "São Beda Venerável",
    titulo: "Monge e Doutor da Igreja, pai da história inglesa",
    data: "25/05",
    seculo: "VII–VIII",
    padroeiro: "Historiadores, eruditos, escritores",
    resumo:
      "Monge de Jarrow, exegeta e historiador, autor da História Eclesiástica do Povo Inglês.",
    biografia:
      "Beda nasceu por volta de 673 nas terras do mosteiro de Wearmouth-Jarrow, na Nortúmbria. Aos sete anos foi confiado ao abade Bento Biscop e passou toda a vida naquele mosteiro duplo, ordenado diácono aos dezenove e sacerdote aos trinta. Nunca viajou para além de poucas milhas, mas dispôs de uma das melhores bibliotecas do Ocidente, formada com os livros que Biscop trouxera de Roma. Escreveu cerca de sessenta obras: comentários bíblicos que se tornaram padrão na exegese medieval, homilias, hagiografias, tratados de ortografia, métrica e cronologia. Foi ele quem popularizou a datação a partir da Encarnação — o 'anno Domini' — hoje universal. Sua obra-prima, a Historia ecclesiastica gentis Anglorum, concluída em 731, narra com escrupuloso cuidado documental a evangelização da Inglaterra, citando fontes e testemunhas: é o texto fundador da historiografia inglesa. Morreu em 26 de maio de 735, ditando a tradução do Evangelho de João em língua anglo-saxônica; terminou a última frase e expirou cantando o Gloria Patri no chão de sua cela. Recebeu já no séc. IX o título de 'Venerável' e foi declarado Doutor da Igreja por Leão XIII em 1899, o único inglês com esse título.",
    virtudes: ["Fidelidade monástica", "Rigor intelectual", "Humildade do estudioso"],
    frase: "É doce cantar ao Senhor até o último suspiro.",
  },
  {
    slug: "santa-hildegarda-de-bingen",
    nome: "Santa Hildegarda de Bingen",
    titulo: "Abadessa e Doutora da Igreja, a 'Sibila do Reno'",
    data: "17/09",
    seculo: "XII",
    padroeiro: "Músicos, herboristas, cientistas, esperantistas",
    resumo:
      "Abadessa beneditina alemã, mística visionária, compositora, naturalista e conselheira de papas e imperadores.",
    biografia:
      "Hildegarda nasceu em Bermersheim, na Renânia, em 1098, décima filha de família nobre. Aos oito anos foi confiada à reclusa Jutta de Sponheim, junto ao mosteiro beneditino de Disibodenberg, e ali fez profissão. Eleita magistra em 1136, recebeu em 1141 a ordem interior de escrever as visões que a acompanhavam desde a infância: nasceu assim o Scivias, aprovado pelo papa Eugênio III no Sínodo de Trier (1147-48) sob o testemunho de São Bernardo. Fundou o mosteiro de Rupertsberg, perto de Bingen, e depois o de Eibingen. Escreveu ainda o Liber vitae meritorum e o Liber divinorum operum, além de duas obras de história natural e medicina — Physica e Causae et curae — que catalogam plantas, minerais e remédios com notável observação empírica. Compôs cerca de setenta e sete cânticos litúrgicos reunidos na Symphonia armonie celestium revelationum e o drama musical Ordo virtutum, o mais antigo do gênero. Pregou publicamente em quatro viagens pela Alemanha, algo excepcional para uma mulher medieval, e correspondeu-se com Frederico Barbarossa, Bernardo de Claraval e vários papas, sem poupar repreensões. Morreu em Bingen em 17 de setembro de 1179. Bento XVI a canonizou e a proclamou Doutora da Igreja em 2012.",
    virtudes: ["Contemplação", "Coragem profética", "Harmonia entre fé e criação"],
    frase: "O homem é uma pluma sobre o sopro de Deus.",
  },
  {
    slug: "santa-isabel-de-portugal",
    nome: "Santa Isabel de Portugal",
    titulo: "Rainha pacificadora, terciária franciscana",
    data: "04/07",
    seculo: "XIII–XIV",
    padroeiro: "Paz, reconciliação familiar, Coimbra e Portugal",
    resumo: "Rainha de Portugal que reconciliou reis inimigos e serviu os pobres do reino.",
    biografia:
      "Isabel nasceu em Saragoça em 1271, filha de Pedro III de Aragão e sobrinha-neta de Santa Isabel da Hungria, cujo nome recebeu. Aos doze anos casou-se com D. Dinis, rei de Portugal, homem culto mas de vida moral difícil. Suportou as infidelidades do marido com mansidão e criou com afeto os filhos bastardos dele, sem deixar de educar os próprios, o infante Afonso e a infanta Constança. Levava vida de rigorosa disciplina: missa diária, jejum, ofício divino, e destinava as rendas ao socorro dos pobres, à fundação de hospitais, orfanatos e albergues de peregrinos, e à criação de dotes para moças sem recursos. A tradição do 'milagre das rosas' conta que, surpreendida pelo rei ao levar pão aos pobres, viu as moedas ou os pães transformarem-se em rosas em seu regaço. Sua vocação maior foi a paz: interveio pessoalmente entre o marido e o filho Afonso em 1323, cavalgando entre os dois exércitos em Alvalade; mediou conflitos entre Portugal e Castela e entre Fernando IV e Jaime II de Aragão. Viúva em 1325, vestiu o hábito das terciárias franciscanas e retirou-se junto ao mosteiro de Santa Clara de Coimbra, que fundara. Morreu em Estremoz em 1336, ainda numa missão de pacificação. Foi canonizada em 1625 por Urbano VIII.",
    virtudes: ["Mansidão", "Espírito de paz", "Caridade régia"],
    frase: "Bem-aventurados os pacificadores, porque serão chamados filhos de Deus.",
  },
  {
    slug: "santa-isabel-da-hungria",
    nome: "Santa Isabel da Hungria",
    titulo: "Princesa terciária franciscana, mãe dos pobres",
    data: "17/11",
    seculo: "XIII",
    padroeiro: "Obras de caridade, padeiros, viúvas, Terceira Ordem Franciscana",
    resumo:
      "Princesa húngara, landgravina da Turíngia, que despojou-se de tudo para servir leprosos e famintos.",
    biografia:
      "Isabel nasceu em 1207, filha do rei André II da Hungria e de Gertrudes de Merânia. Aos quatro anos foi levada ao castelo de Wartburg, na Turíngia, para ser educada como noiva do futuro landgrave. Casou-se aos catorze com Luís IV, num matrimônio raro na Idade Média por ser feliz e profundamente cristão; tiveram três filhos. Influenciada pelos frades menores recém-chegados à Alemanha e dirigida pelo severo Conrado de Marburgo, abraçou o ideal de pobreza franciscana dentro do próprio palácio: vestia-se com simplicidade, servia à mesa dos pobres, cuidava pessoalmente de leprosos e chegou a instalar um deles no leito real. Durante a fome de 1226 abriu os celeiros do castelo e distribuiu o trigo do reino. Também a ela se atribui o 'milagre das rosas'. Viúva aos vinte anos, quando Luís morreu de peste rumo à cruzada, foi expulsa de Wartburg pela família do marido. Estabeleceu-se em Marburgo, fez profissão como terciária franciscana em 1228 — a primeira na Alemanha — e com o dote construiu um hospital dedicado a São Francisco, onde trabalhou como enfermeira até a morte, aos vinte e quatro anos, em 17 de novembro de 1231. Foi canonizada apenas quatro anos depois, em 1235, por Gregório IX.",
    virtudes: ["Pobreza voluntária", "Serviço aos leprosos", "Alegria no despojamento"],
    frase: "Devemos tornar felizes os homens; é isso que Deus quer de nós.",
  },
  {
    slug: "sao-luis-ix-de-franca",
    nome: "São Luís IX de França",
    titulo: "Rei cruzado, modelo de governante cristão",
    data: "25/08",
    seculo: "XIII",
    padroeiro: "França, terciários franciscanos, governantes, tapeceiros",
    resumo:
      "Rei que uniu justiça, penitência e caridade no exercício do poder, construtor da Sainte-Chapelle.",
    biografia:
      "Luís nasceu em Poissy em 1214 e tornou-se rei aos doze anos, sob a regência da mãe Branca de Castela, que lhe repetia preferir vê-lo morto a sabê-lo em pecado mortal. Casou-se com Margarida de Provença, com quem teve onze filhos. Governou a França de 1226 a 1270 num dos períodos mais prósperos do reino: reformou a justiça criando os inquéritos reais para reparar abusos dos oficiais, aboliu as ordálias e o duelo judiciário, instituiu a presunção de inocência e recebia pessoalmente as queixas dos súditos, célebre pelas audiências sob o carvalho de Vincennes. Fundou hospitais como o Quinze-Vingts para cegos, sustentou a Sorbonne e alimentava diariamente pobres à própria mesa. Adquiriu de Balduíno II o que se venerava como relíquias da Paixão — a coroa de espinhos — e ergueu para guardá-las a Sainte-Chapelle de Paris, joia do gótico radiante, concluída em 1248. Terciário franciscano, vivia austeramente, com jejuns, cilício e confissão frequente. Conduziu duas cruzadas: na sétima (1248-54) foi feito prisioneiro no Egito e resgatado; na oitava morreu de disenteria diante de Túnis, em 25 de agosto de 1270, murmurando 'Jerusalém, Jerusalém'. Foi canonizado em 1297 por Bonifácio VIII.",
    virtudes: ["Justiça", "Penitência", "Caridade régia", "Fidelidade conjugal"],
    frase: "Prefiro que um escocês venha governar meu povo bem, a governá-lo eu mesmo mal.",
  },
  {
    slug: "santa-gema-galgani",
    nome: "Santa Gema Galgani",
    titulo: "Virgem leiga, estigmatizada da Paixão",
    data: "11/04",
    seculo: "XIX–XX",
    padroeiro: "Estudantes, farmacêuticos, tuberculosos, tentados",
    resumo:
      "Jovem leiga de Lucca, mística da Paixão de Cristo, marcada pelos estigmas todas as sextas-feiras.",
    biografia:
      "Gema nasceu em Camigliano, perto de Lucca, na Itália, em 12 de março de 1878, quinta de oito filhos de um farmacêutico. Perdeu a mãe aos sete anos e o pai aos dezenove, quando a família caiu na miséria; passou a servir como doméstica e a cuidar dos irmãos menores. Estudou nas Irmãs Oblatas do Espírito Santo, onde recebeu a primeira comunhão aos nove anos e desde então concebeu ardente devoção à Paixão. Sofreu meningite e tuberculose vertebral, ficando paralítica; teria curado-se subitamente em 1899, segundo os relatos hagiográficos, após uma novena a São Gabriel da Virgem das Dores. Desejou ardentemente ser passionista, mas a saúde precária lhe fechou as portas do claustro; acolhida pela família Giannini, viveu como leiga em profunda vida mística sob a direção do padre Germano Ruoppolo. A partir de junho de 1899 recebeu os estigmas, que se abriam nas quintas-feiras à noite e se fechavam nas sextas à tarde, além de suores de sangue, êxtases e uma coroa de espinhos invisível; relatou também violentos assaltos diabólicos, descritos com sobriedade em suas cartas e no diário. Morreu no Sábado Santo, 11 de abril de 1903, aos vinte e cinco anos, de tuberculose. Foi canonizada por Pio XII em 1940.",
    virtudes: ["Amor à Paixão de Cristo", "Pureza", "Paciência na doença"],
    frase: "Jesus, tudo o que quiseres, como quiseres, quando quiseres.",
  },
  {
    slug: "sao-camilo-de-lellis",
    nome: "São Camilo de Léllis",
    titulo: "Fundador dos Ministros dos Enfermos",
    data: "14/07",
    seculo: "XVI–XVII",
    padroeiro: "Enfermos, hospitais, enfermeiros, agonizantes",
    resumo:
      "Ex-soldado e jogador convertido que revolucionou o cuidado dos doentes na Roma barroca.",
    biografia:
      "Camilo nasceu em Bucchianico, nos Abruzos, em 25 de maio de 1550, filho de mãe já sexagenária. Cresceu rude, alto e violento, seguiu o pai na carreira das armas e combateu contra os turcos em serviço de Veneza e Nápoles. Viciado no jogo, perdeu tudo, até a camisa, e acabou trabalhando como servente numa obra dos capuchinhos em Manfredônia. Uma exortação do guardião do convento, em 2 de fevereiro de 1575, provocou sua conversão radical. Uma chaga incurável na perna, que o acompanharia toda a vida, impediu-o de entrar entre os capuchinhos; recolheu-se então ao hospital romano de São Tiago dos Incuráveis, primeiro como paciente e depois como enfermeiro e administrador. Horrorizado pela negligência dos mercenários que serviam os doentes, decidiu formar uma companhia de homens que cuidassem dos enfermos 'como mães cuidam de seus filhos doentes'. Ordenado sacerdote em 1584 com o apoio de São Filipe Néri, seu confessor, fundou os Ministros dos Enfermos — os camilianos —, que acrescentam aos três votos um quarto: servir os doentes mesmo com perigo da própria vida. Adotou a cruz vermelha no hábito, símbolo depois universal. Introduziu normas revolucionárias de higiene, dieta, isolamento de contagiosos e cuidado espiritual dos agonizantes, e enviou religiosos aos campos de batalha da Hungria e da Croácia em 1595, primeira unidade sanitária militar da história. Morreu em Roma em 14 de julho de 1614, canonizado em 1746.",
    virtudes: ["Compaixão heroica", "Conversão radical", "Serviço aos moribundos"],
    frase: "Mais coração nessas mãos, irmãos.",
  },
  {
    slug: "sao-joao-de-deus",
    nome: "São João de Deus",
    titulo: "Fundador da Ordem Hospitaleira",
    data: "08/03",
    seculo: "XVI",
    padroeiro: "Enfermeiros, hospitais, livreiros, doentes mentais",
    resumo:
      "Português convertido em Granada que criou o hospital moderno e o tratamento humano dos doentes mentais.",
    biografia:
      "João Cidade nasceu em Montemor-o-Novo, Portugal, em 8 de março de 1495. Aos oito anos deixou a casa paterna com um clérigo peregrino e nunca mais viu os pais. Foi pastor na Espanha, soldado nas guerras contra a França e os turcos, trabalhador em Ceuta e vendedor ambulante de livros em Granada. Em 20 de janeiro de 1537, ouvindo um sermão de São João de Ávila sobre a conversão, teve uma crise tão violenta de arrependimento — rasgando as roupas e gritando pelas ruas — que foi internado como louco no Hospital Real, onde sofreu os tratamentos brutais então usuais. Visitado por João de Ávila, que se tornou seu diretor espiritual, saiu curado e resolveu dedicar a vida exatamente àqueles que tinham sido seus companheiros de sofrimento. Alugou uma casa na rua Lucena e começou a recolher doentes, mendigos, prostitutas arrependidas e alienados, mendigando à noite pela cidade com o grito 'Fazei o bem, irmãos, a vós mesmos!'. Sua novidade absoluta foi separar os doentes por patologia, dar leito individual a cada um e tratar os enfermos mentais com bondade em vez de correntes. O arcebispo lhe deu o nome de João de Deus. Morreu em Granada em 8 de março de 1550, de pneumonia contraída ao salvar um jovem das águas do rio Genil. Seus seguidores formaram a Ordem Hospitaleira. Canonizado em 1690, é copadroeiro dos hospitais com São Camilo.",
    virtudes: ["Caridade sem medida", "Humildade", "Dignificação dos doentes mentais"],
    frase: "Fazei o bem, irmãos, a vós mesmos!",
  },
  {
    slug: "santo-estanislau-kostka",
    nome: "Santo Estanislau Kostka",
    titulo: "Jovem jesuíta polonês",
    data: "13/11",
    seculo: "XVI",
    padroeiro: "Noviços, jovens estudantes, Polônia",
    resumo:
      "Nobre polonês que fugiu a pé até Roma para entrar na Companhia de Jesus e morreu aos dezoito anos.",
    biografia:
      "Estanislau nasceu no castelo de Rostkowo, na Mazóvia polonesa, em 28 de outubro de 1550, filho de senador do reino. Aos catorze anos foi enviado com o irmão mais velho Paulo ao colégio dos jesuítas em Viena. Enquanto o irmão levava vida mundana e o maltratava por sua devoção, Estanislau cultivava intensa vida interior e devoção mariana. Em dezembro de 1565 caiu gravemente enfermo na casa de um senhor luterano que não permitia a entrada de sacerdote; segundo seu testemunho, Santa Bárbara lhe trouxe a comunhão e depois a Virgem Maria lhe entregou o Menino nos braços, ordenando que entrasse na Companhia de Jesus. Curado subitamente, e sabendo que o pai jamais consentiria, empreendeu em agosto de 1567 uma fuga extraordinária: disfarçado de mendigo, caminhou cerca de mil e quinhentos quilômetros de Viena a Dillingen e depois a Roma, atravessando a Alemanha a pé. São Pedro Canísio o acolheu e São Francisco de Borja, então geral da Companhia, admitiu-o ao noviciado de Sant'Andrea al Quirinale em 28 de outubro de 1567. Viveu apenas dez meses de vida religiosa, marcados por obediência exata, alegria e êxtases eucarísticos. Predisse a própria morte para a festa da Assunção e faleceu na madrugada de 15 de agosto de 1568, aos dezoito anos incompletos. Canonizado em 1726.",
    virtudes: ["Determinação na vocação", "Pureza", "Devoção mariana"],
    frase: "Nasci para coisas maiores.",
  },
  {
    slug: "sao-luis-gonzaga",
    nome: "São Luís Gonzaga",
    titulo: "Jesuíta, padroeiro da juventude cristã",
    data: "21/06",
    seculo: "XVI",
    padroeiro: "Juventude, estudantes, vítimas de epidemias, AIDS",
    resumo:
      "Marquês herdeiro que renunciou ao principado, entrou entre os jesuítas e morreu servindo pestilentos.",
    biografia:
      "Luís Gonzaga nasceu em Castiglione delle Stiviere, na Lombardia, em 9 de março de 1568, primogênito do marquês Ferrante Gonzaga, e destinado a herdar o principado. Educado nas cortes de Florença, Mântua e Madri, onde foi pajem do infante Diogo, filho de Filipe II, conheceu de perto a corrupção cortesã e fez voto de castidade perpétua aos nove anos. A leitura das cartas dos missionários jesuítas na Índia acendeu-lhe a vocação. Enfrentou três anos de resistência do pai, que chegou a levá-lo pelas cortes italianas para dissuadi-lo; em novembro de 1585 renunciou formalmente ao marquesado em favor do irmão Rodolfo e entrou no noviciado romano de Sant'Andrea. Teve como diretor espiritual São Roberto Belarmino. Estudou filosofia e teologia no Colégio Romano, moderando por obediência as penitências extremas que praticava. Em 1591 uma violenta peste assolou Roma; os jesuítas abriram um hospital e Luís pediu para servir os doentes, carregando pelas ruas os moribundos abandonados, lavando-os e preparando-os para a morte. Contraiu a doença, teve breve remissão e definhou durante três meses de febre. Morreu na noite de 20 para 21 de junho de 1591, aos vinte e três anos, com o nome de Jesus nos lábios. Beatificado em 1605 e canonizado em 1726, foi declarado padroeiro da juventude por Bento XIII.",
    virtudes: ["Castidade angélica", "Renúncia ao mundo", "Caridade heroica com os pestilentos"],
    frase: "De que serve isto para a eternidade?",
  },
  {
    slug: "sao-tarcisio",
    nome: "São Tarcísio",
    titulo: "Acólito mártir da Eucaristia",
    data: "15/08",
    seculo: "III",
    padroeiro: "Coroinhas, acólitos, primeira comunhão",
    resumo:
      "Jovem acólito romano morto a pauladas por não entregar a Eucaristia que levava aos presos.",
    biografia:
      "Tarcísio foi um jovem cristão de Roma, provavelmente acólito, martirizado no século III, verossimilmente durante a perseguição de Valeriano, por volta de 257. O testemunho antigo mais importante é o epitáfio composto pelo papa São Dâmaso I (366-384) para seu túmulo na catacumba de São Calisto, na Via Ápia, comparando-o ao protomártir Estêvão. Segundo esse texto e a tradição romana, o menino levava escondidos junto ao peito os sagrados mistérios — a Eucaristia destinada aos cristãos encarcerados que aguardavam o martírio — quando foi interceptado por um grupo de pagãos. Instado a mostrar o que carregava e a entregar o 'tesouro' dos cristãos, recusou-se firmemente; a multidão o espancou com paus e pedras até a morte. Ao recolherem seu corpo, nada se encontrou nas mãos: a tradição afirma que preferiu perder a vida a expor o Corpo do Senhor à profanação. Seus restos foram sepultados na catacumba calistiana e mais tarde parte deles transferida para a basílica de San Silvestro in Capite. Sua figura foi popularizada no século XIX pelo romance Fabíola, do cardeal Nicholas Wiseman, e desde então tornou-se o modelo por excelência dos coroinhas e dos jovens servidores do altar, celebrado com particular afeto pela Igreja no dia 15 de agosto.",
    virtudes: ["Fidelidade à Eucaristia", "Coragem juvenil", "Fortaleza no martírio"],
    frase: "Antes morrer que entregar o Senhor.",
  },
  {
    slug: "santa-rosa-de-lima",
    nome: "Santa Rosa de Lima",
    titulo: "Terciária dominicana, primeira santa das Américas",
    data: "23/08",
    seculo: "XVI–XVII",
    padroeiro: "América Latina, Peru, Filipinas, floricultores, jardineiros",
    resumo:
      "Mística peruana de penitência extrema e caridade, canonizada como primeira santa do Novo Mundo.",
    biografia:
      "Isabel Flores de Oliva nasceu em Lima, no Vice-Reino do Peru, em 20 de abril de 1586, filha de um arcabuzeiro espanhol e de mãe criolla. Recebeu o apelido de Rosa por causa da beleza do rosto na infância, nome confirmado na crisma pelo arcebispo Santo Toríbio de Mogrovejo. Desde menina fez voto de virgindade e, para desencorajar pretendentes, cortou os cabelos e desfigurou o rosto com pimenta e cal. Trabalhava bordando e cultivando flores para sustentar os pais empobrecidos. Recusou tanto o casamento arranjado quanto a entrada no convento por não poder abandonar a família; tornou-se terciária dominicana em 1606, inspirada em Santa Catarina de Sena, e viveu numa pequena cabana construída no jardim de casa, onde passava horas em oração diante de um oratório. Praticou penitências severíssimas — coroa de espinhos oculta sob o véu, jejuns, vigílias —, sempre sob obediência de seus confessores. Ao mesmo tempo abriu num cômodo da casa uma enfermaria para indígenas doentes, crianças abandonadas e velhos desamparados, considerada germe da assistência social no Peru. Passou longos anos em provações interiores de aridez, alternadas com graças místicas. Morreu em 24 de agosto de 1617, aos trinta e um anos, na casa do contador Gonzalo de la Maza. Foi canonizada em 1671 por Clemente X, a primeira santa nascida nas Américas.",
    virtudes: ["Penitência", "Virgindade consagrada", "Caridade com indígenas e enfermos"],
    frase: "Senhor, aumenta-me os sofrimentos e, com eles, o teu amor.",
  },
  {
    slug: "sao-martinho-de-porres",
    nome: "São Martinho de Porres",
    titulo: "Irmão dominicano, apóstolo da caridade em Lima",
    data: "03/11",
    seculo: "XVI–XVII",
    padroeiro: "Justiça social, harmonia racial, barbeiros, enfermeiros, animais",
    resumo:
      "Frade mulato limenho, enfermeiro dos pobres e taumaturgo, símbolo de fraternidade universal.",
    biografia:
      "Martinho nasceu em Lima em 9 de dezembro de 1579, filho ilegítimo do fidalgo espanhol Juan de Porres e de Ana Velázquez, negra liberta do Panamá. O pai o abandonou por causa da cor da pele e a mãe o criou na pobreza. Aos doze anos foi aprendiz de barbeiro e cirurgião, aprendendo a sangrar, extrair dentes e curar feridas, ofício que marcaria toda a sua vida. Aos quinze pediu para servir no convento dominicano do Santo Rosário como donado, isto é, servo voluntário, pois a legislação da época impedia mulatos de professar. Nove anos depois, vencendo os preconceitos pela evidência de sua virtude, o prior o admitiu à profissão como irmão cooperador. Foi enfermeiro do convento e da cidade, atendendo indistintamente frades, escravos, indígenas e espanhóis. Recolhia doentes da rua para a própria cela, distribuía diariamente comida a centenas de pobres, fundou com o auxílio de benfeitores o Colégio da Santa Cruz para crianças órfãs e obteve dotes para moças sem recursos. A tradição lhe atribui inúmeros prodígios: bilocação, curas instantâneas, conhecimento de consciências e uma singular amizade com os animais — dizia-se que fazia cão, gato e rato comerem da mesma tigela. Chamava a si mesmo de 'pobre mulato' e pedia para ser vendido como escravo quando o convento passava por dificuldades. Morreu em 3 de novembro de 1639. Canonizado por João XXIII em 1962.",
    virtudes: ["Humildade", "Caridade universal", "Paciência diante do preconceito"],
    frase: "Tudo o que faço é a misericórdia de Deus que o faz por mim.",
  },
  {
    slug: "sao-juan-diego",
    nome: "São Juan Diego",
    titulo: "Vidente de Nossa Senhora de Guadalupe",
    data: "09/12",
    seculo: "XV–XVI",
    padroeiro: "Povos indígenas, México, leigos evangelizadores",
    resumo:
      "Indígena chichimeca a quem a Virgem apareceu no Tepeyac, deixando sua imagem impressa na tilma.",
    biografia:
      "Cuauhtlatoatzin — 'águia que fala' — nasceu por volta de 1474 em Cuautitlán, no altiplano mexicano, membro do povo chichimeca. Camponês e tecelão de esteiras, converteu-se com a esposa Maria Lúcia por volta de 1524, sob a pregação dos primeiros franciscanos, recebendo no batismo o nome de Juan Diego. Viúvo em 1529, passou a percorrer a pé, várias vezes por semana, os quinze quilômetros que o separavam da capela de Tlatelolco para a catequese e a missa. Segundo o relato guadalupano (Nican Mopohua, séc. XVI), em 9 de dezembro de 1531, ao passar pelo cerro do Tepeyac, ouviu música e uma voz que o chamava 'Juanito, Juan Dieguito'. A Virgem Maria apresentou-se como 'a perfeita sempre Virgem Santa Maria, mãe do verdadeiro Deus por quem se vive' e pediu que se erguesse ali um templo. O bispo Juan de Zumárraga exigiu um sinal. No dia 12 de dezembro, a Senhora mandou que colhesse rosas de Castela florescidas fora de estação no morro árido; ao abrir a tilma diante do bispo, as flores caíram e apareceu impressa no tecido de fibra de agave a imagem da Virgem morena, grávida, vestida com símbolos legíveis pela cultura náuatle. O acontecimento provocou a conversão de milhões de indígenas em poucos anos. Juan Diego passou o resto da vida como guardião da ermida, atendendo peregrinos. Morreu em 30 de maio de 1548. Foi canonizado por João Paulo II na própria basílica de Guadalupe, em 31 de julho de 2002.",
    virtudes: ["Simplicidade", "Obediência à Virgem", "Perseverança diante da descrença"],
    frase: "Não estou eu aqui, que sou tua mãe?",
  },
  {
    slug: "sao-toribio-de-mogrovejo",
    nome: "São Toribio de Mogrovejo",
    titulo: "Arcebispo de Lima, organizador da Igreja na América",
    data: "23/03",
    seculo: "XVI–XVII",
    padroeiro: "Bispos latino-americanos, Peru, direitos dos indígenas",
    resumo:
      "Jurista leigo feito arcebispo, percorreu a pé os Andes catequizando indígenas em suas línguas.",
    biografia:
      "Toríbio Alfonso de Mogrovejo nasceu em Mayorga de Campos, Espanha, em 16 de novembro de 1538, de família nobre. Estudou direito em Valladolid e Salamanca e tornou-se professor e depois presidente do Tribunal da Inquisição de Granada, sendo ainda leigo. Em 1579 Filipe II o indicou para a sé de Lima; recebeu as ordens sacras em poucos meses e chegou ao Peru em 1581, aos quarenta e três anos. Encontrou uma Igreja desorganizada, colonos abusivos e indígenas explorados. Convocou o III Concílio Limense (1582-83), cujos decretos e catecismo trilíngue — em castelhano, quíchua e aimará — moldaram a evangelização de toda a América espanhola por dois séculos. Realizou três visitas pastorais que somaram cerca de quarenta mil quilômetros percorridos a pé e a lombo de mula, atravessando desertos, selvas e cordilheiras, ficando anos fora da capital. Aprendeu o quíchua para confirmar e catequizar pessoalmente; crismou cerca de meio milhão de pessoas, entre elas Santa Rosa de Lima e São Martinho de Porres. Fundou em 1591 o primeiro seminário do continente, além de hospitais, escolas e igrejas rurais. Defendeu com energia os indígenas contra encomenderos e autoridades civis, o que lhe custou processos e inimizades. Morreu em visita pastoral, em Saña, em 23 de março de 1606, e foi canonizado em 1726.",
    virtudes: ["Zelo pastoral", "Defesa dos indígenas", "Austeridade pessoal"],
    frase: "O tempo não é nosso; havemos de dar conta dele.",
  },
  {
    slug: "sao-pedro-claver",
    nome: "São Pedro Claver",
    titulo: "Jesuíta, 'escravo dos escravos negros'",
    data: "09/09",
    seculo: "XVI–XVII",
    padroeiro: "Missões entre afrodescendentes, Colômbia, direitos humanos",
    resumo:
      "Missionário catalão que socorreu por quarenta anos os africanos escravizados no porto de Cartagena.",
    biografia:
      "Pedro Claver nasceu em Verdú, na Catalunha, em 26 de junho de 1580, filho de camponeses. Entrou na Companhia de Jesus em 1602 e, durante os estudos em Maiorca, foi decisivamente influenciado pelo humilde porteiro São Afonso Rodríguez, que, segundo a tradição jesuíta, lhe profetizou a missão nas Índias. Chegou a Cartagena das Índias em 1610, então o maior mercado de escravos da América espanhola, onde desembarcavam cerca de dez mil africanos por ano em condições atrozes. Ordenado sacerdote em 1616, fez em 1622 um voto singular ao emitir a profissão solene, assinando-se 'Petrus Claver, aethiopum semper servus' — Pedro Claver, para sempre escravo dos negros. Ao avistar um navio negreiro, corria ao porto com intérpretes, remédios, água, frutas, tabaco e aguardente; entrava nos porões fétidos, curava chagas, alimentava os moribundos, defendia-os dos maus-tratos e só então, respeitando a pessoa antes da alma, ensinava-lhes o catecismo com imagens pintadas. Batizou pessoalmente, segundo os registros, mais de trezentos mil africanos em quarenta anos. Visitava também hospitais de leprosos, presídios e condenados à morte, enfrentando a hostilidade de traficantes e senhores. Atingido pela peste em 1650, ficou quatro anos paralítico e abandonado, cuidado por um servo negligente. Morreu em 8 de setembro de 1654. Canonizado em 1888 por Leão XIII, é padroeiro das missões entre afrodescendentes.",
    virtudes: ["Serviço aos escravizados", "Paciência", "Reverência pela dignidade humana"],
    frase: "Devemos falar com as mãos antes de falar com os lábios.",
  },
  {
    slug: "santa-catarina-laboure",
    nome: "Santa Catarina Labouré",
    titulo: "Filha da Caridade, vidente da Medalha Milagrosa",
    data: "31/12",
    seculo: "XIX",
    padroeiro: "Idosos, obras de caridade, devoção à Medalha Milagrosa",
    resumo:
      "Religiosa francesa que recebeu da Virgem, em 1830, o pedido de cunhar a Medalha Milagrosa.",
    biografia:
      "Catarina Labouré nasceu em Fain-lès-Moutiers, na Borgonha, em 2 de maio de 1806, nona de dez filhos de uma família camponesa. Órfã de mãe aos nove anos, disse então à imagem da Virgem: 'Agora sereis vós a minha mãe'. Analfabeta até tarde, cuidou da casa e dos irmãos e enfrentou a oposição do pai para seguir a vocação. Entrou nas Filhas da Caridade de São Vicente de Paulo em 1830, no seminário da rua du Bac, em Paris. Naquele mesmo ano recebeu três aparições marianas. Na noite de 18 para 19 de julho, uma criança luminosa a conduziu à capela, onde conversou longamente com a Virgem ajoelhada junto ao altar, que lhe anunciou provações para a França e a Igreja. Em 27 de novembro, viu Maria de pé sobre o globo, com raios de luz brotando de seus dedos, cercada pela invocação 'Ó Maria concebida sem pecado, rogai por nós que recorremos a vós'. O quadro girou e apresentou o monograma de Maria com uma cruz e os dois corações; a Senhora pediu que se cunhasse uma medalha com aquela imagem, prometendo graças abundantes a quem a usasse. Cunhada em 1832 durante uma epidemia de cólera, difundiu-se com tal rapidez de prodígios que o povo a chamou de 'Milagrosa'. Catarina guardou absoluto silêncio sobre sua identidade por quarenta e seis anos, vivendo obscura no hospício de Enghien, em Reuilly, cuidando de velhos e da criação de aves. Morreu em 31 de dezembro de 1876 e foi canonizada em 1947.",
    virtudes: ["Humildade escondida", "Obediência", "Confiança em Maria"],
    frase: "Ó Maria concebida sem pecado, rogai por nós que recorremos a vós.",
  },
  {
    slug: "santos-francisco-e-jacinta-marto",
    nome: "Santos Francisco e Jacinta Marto",
    titulo: "Pastorinhos videntes de Fátima",
    data: "20/02",
    seculo: "XX",
    padroeiro: "Crianças doentes, pastores, devoção reparadora",
    resumo:
      "Irmãos portugueses que viram a Virgem em 1917 e se ofereceram em reparação pelos pecadores.",
    biografia:
      "Francisco Marto nasceu em Aljustrel, freguesia de Fátima, em 11 de junho de 1908, e sua irmã Jacinta em 11 de março de 1910, filhos de Manuel Pedro Marto e Olímpia de Jesus. Guardavam ovelhas com a prima Lúcia dos Santos. Em 1916 receberam três visitas do Anjo de Portugal, que lhes ensinou orações de reparação e lhes deu a comunhão. Entre 13 de maio e 13 de outubro de 1917, na Cova da Iria, viram por seis vezes Nossa Senhora, que pediu a reza diária do terço pela paz, a penitência pela conversão dos pecadores e a devoção ao Imaculado Coração; a última aparição foi acompanhada pelo 'milagre do sol' diante de cerca de setenta mil pessoas. Interrogados com dureza pelas autoridades e até presos pelo administrador de Ourém, que os ameaçou de morte em óleo fervente, mantiveram firmemente o testemunho. Francisco, mais contemplativo, escolheu 'consolar o Senhor escondido', passando horas de joelhos na igreja diante do sacrário; Jacinta, tocada pela visão do inferno, oferecia sacrifícios contínuos pelos pecadores e pelo Santo Padre. Ambos adoeceram na pandemia de gripe espanhola. Francisco morreu em casa em 4 de abril de 1919, aos dez anos; Jacinta, após meses de sofrimento e duas cirurgias sem anestesia adequada, morreu sozinha num hospital de Lisboa em 20 de fevereiro de 1920, aos nove anos. Beatificados por João Paulo II em 2000 e canonizados por Francisco no centenário das aparições, em 13 de maio de 2017.",
    virtudes: ["Espírito de reparação", "Oração pelos pecadores", "Fortaleza infantil"],
    frase: "Ó Jesus, é por vosso amor, pela conversão dos pecadores.",
  },
  {
    slug: "bem-aventurado-miguel-agustin-pro",
    nome: "Bem-Aventurado Miguel Agustín Pro",
    titulo: "Jesuíta mártir da perseguição mexicana",
    data: "23/11",
    seculo: "XX",
    padroeiro: "Perseguidos pela fé, México, sacerdotes clandestinos",
    resumo:
      "Sacerdote jesuíta fuzilado em 1927 no México, morreu de braços abertos gritando 'Viva Cristo Rei!'.",
    biografia:
      "Miguel Agustín Pro Juárez nasceu em Guadalupe, Zacatecas, em 13 de janeiro de 1891, filho de um engenheiro de minas. Jovem alegre, brincalhão e exímio imitador, entrou na Companhia de Jesus em 1911. A revolução mexicana e as leis anticlericais obrigaram os jesuítas ao exílio: estudou na Califórnia, na Nicarágua, na Espanha e na Bélgica, onde foi ordenado sacerdote em 31 de agosto de 1925, já sofrendo de graves úlceras estomacais. Voltou ao México em julho de 1926, exatamente quando o governo de Plutarco Elías Calles fechava as igrejas e proibia o culto público, desencadeando a Guerra Cristera. Exerceu por dezesseis meses um ministério clandestino de audácia notável: disfarçado de mecânico, estudante, mendigo ou policial, celebrava missas em casas particulares, distribuía a comunhão a centenas de pessoas por dia, organizava socorro a famílias de operários desempregados e ouvia confissões em automóveis. Em novembro de 1927 foi preso com dois irmãos, acusado sem provas de participar de um atentado contra o general Obregón — o próprio autor do ato depois inocentou-o. Sem julgamento, Calles ordenou o fuzilamento e convocou fotógrafos para intimidar os católicos. Em 23 de novembro de 1927, no pátio da delegacia, Miguel Pro perdoou os algozes, recusou a venda nos olhos, ajoelhou-se em oração e ergueu os braços em cruz gritando 'Viva Cristo Rei!'. As fotografias produziram o efeito contrário ao desejado. Foi beatificado por João Paulo II em 1988.",
    virtudes: ["Coragem apostólica", "Alegria no perigo", "Perdão aos perseguidores"],
    frase: "Viva Cristo Rei!",
  },
  {
    slug: "santa-gianna-beretta-molla",
    nome: "Santa Gianna Beretta Molla",
    titulo: "Médica, esposa e mãe, mártir da vida",
    data: "28/04",
    seculo: "XX",
    padroeiro: "Mães, gestantes, médicos, nascituros, famílias",
    resumo:
      "Pediatra italiana que recusou o aborto terapêutico para salvar a filha e morreu poucos dias após o parto.",
    biografia:
      "Gianna Beretta nasceu em Magenta, perto de Milão, em 4 de outubro de 1922, décima de treze filhos de uma família profundamente cristã de terciários franciscanos. Formou-se em medicina em Pávia em 1949 e especializou-se em pediatria em 1952, abrindo consultório em Mesero, onde atendia com particular carinho mães, crianças, idosos e pobres, muitas vezes sem cobrar. Ativa na Ação Católica e no Movimento dos Universitários Católicos, praticava esqui e montanhismo e considerava seriamente a vocação missionária junto ao irmão sacerdote no Brasil, desistindo por razões de saúde. Discerniu então o matrimônio como sua vocação: casou-se em 24 de setembro de 1955 com o engenheiro Pietro Molla, e escreveu-lhe que queria formar 'uma família verdadeiramente cristã'. Tiveram Pierluigi, Maria Zita e Laura. Em setembro de 1961, no segundo mês da quarta gravidez, foi diagnosticado um volumoso fibroma no útero. Diante das três opções — histerectomia, aborto ou a arriscada remoção apenas do tumor —, escolheu a última, a única que preservava a criança, e disse claramente aos médicos: 'Se tiverem de decidir entre mim e a criança, nenhuma hesitação: escolham a criança'. A cirurgia foi bem-sucedida e a gestação prosseguiu. Em 21 de abril de 1962, Sábado Santo, nasceu por cesariana Gianna Emanuela, saudável. A mãe desenvolveu septicemia e morreu em casa em 28 de abril de 1962, aos trinta e nove anos, repetindo 'Jesus, eu te amo'. Foi canonizada por João Paulo II em 16 de maio de 2004, na presença do marido e dos filhos.",
    virtudes: ["Amor materno heroico", "Santidade no estado laical", "Serviço médico caritativo"],
    frase: "Se tiverem de decidir, escolham a criança.",
  },
  {
    slug: "sao-pio-x",
    nome: "São Pio X",
    titulo: "Papa, reformador da liturgia e da catequese",
    data: "21/08",
    seculo: "XIX–XX",
    padroeiro: "Catequistas, primeira comunhão, peregrinos",
    resumo:
      "Giuseppe Sarto, papa camponês que antecipou a comunhão frequente e combateu o modernismo.",
    biografia:
      "Giuseppe Melchiorre Sarto nasceu em Riese, no Vêneto, em 2 de junho de 1835, filho de um humilde carteiro. Ordenado sacerdote em 1858, exerceu ministério paroquial dedicado aos pobres antes de tornar-se bispo de Mântua e, depois, cardeal-patriarca de Veneza. Eleito papa em 1903 com o lema 'Instaurare omnia in Christo', manteve estilo simples e paternal, recusando pompas desnecessárias. Sua obra mais duradoura foi pastoral: antecipou a idade da primeira comunhão para os sete anos, incentivou a comunhão frequente e até diária dos fiéis, reformou o catecismo tornando-o acessível ao povo simples e promoveu a restauração do canto gregoriano na liturgia. Diante da crise do modernismo, que relativizava dogmas e Escrituras sob influência do racionalismo, publicou a encíclica Pascendi Dominici Gregis (1907) condenando os erros com firmeza doutrinal. Reorganizou também a Cúria Romana e iniciou a codificação do Direito Canônico. Viveu com angústia profética a iminência da Primeira Guerra Mundial, falecendo em 20 de agosto de 1914, dias após seu início, oferecendo a vida pela paz. Canonizado em 1954 por Pio XII, é o primeiro papa elevado aos altares desde São Pio V.",
    virtudes: [
      "Simplicidade evangélica",
      "Zelo pela Eucaristia",
      "Firmeza doutrinal",
      "Amor aos pequenos",
    ],
    frase: "Deixai vir a mim as criancinhas.",
  },
  {
    slug: "sao-leao-magno",
    nome: "São Leão Magno",
    titulo: "Papa e Doutor da Igreja",
    data: "10/11",
    seculo: "V",
    padroeiro: "Músicos, cantores, Roma",
    resumo:
      "Papa que deteve Átila diante de Roma e definiu a doutrina das duas naturezas de Cristo.",
    biografia:
      "Nascido na Toscana por volta de 400, Leão foi diácono de Roma antes de ser eleito papa em 440, num tempo de graves invasões bárbaras e disputas cristológicas no Oriente. Sua célebre carta dogmática, o 'Tomo a Flaviano', enviada ao Concílio de Calcedônia em 451, formulou com precisão luminosa a união das duas naturezas, divina e humana, numa só Pessoa de Cristo, sendo aclamada pelos padres conciliares com a frase: 'Pedro falou pela boca de Leão'. No plano temporal, encontrou-se pessoalmente com Átila, rei dos hunos, nas margens do Mincio em 452, persuadindo-o a poupar Roma da destruição; três anos depois intercedeu junto aos vândalos de Genserico, atenuando a violência do saque da cidade. Pregou numerosas homilias de grande beleza literária sobre o Natal, a Quaresma e a primazia romana, insistindo que o papa é sucessor de Pedro não por mérito próprio, mas por ofício confiado por Cristo. Reformou a disciplina eclesiástica e consolidou a autoridade do Bispo de Roma sobre toda a Igreja. Morreu em 461, sendo o primeiro papa sepultado na basílica de São Pedro, e recebeu o título de Doutor da Igreja por Bento XIV em 1754.",
    virtudes: [
      "Firmeza doutrinal",
      "Coragem diante do perigo",
      "Eloquência pastoral",
      "Fidelidade à Sé de Pedro",
    ],
    frase: "Reconhece, ó cristão, a tua dignidade!",
  },
  {
    slug: "santo-hilario-de-poitiers",
    nome: "Santo Hilário de Poitiers",
    titulo: "Bispo e Doutor da Igreja",
    data: "13/01",
    seculo: "IV",
    padroeiro: "Advogados, mães que amamentam",
    resumo:
      "'Atanásio do Ocidente', bispo exilado por defender a divindade de Cristo contra o arianismo.",
    biografia:
      "Nascido em Poitiers, na Gália, por volta de 315, em família pagã culta, converteu-se ao cristianismo pela leitura das Escrituras, especialmente do prólogo de João, e foi eleito bispo de sua cidade natal por volta de 353, já casado e pai de uma filha, Ábra. Empenhou-se com energia contra o arianismo, que negava a plena divindade do Filho, e por isso foi denunciado ao imperador ariano Constâncio II e exilado na Frígia por quatro anos, período em que aprofundou o estudo da teologia oriental e escreveu sua obra-prima, 'De Trinitate', síntese rigorosa da fé niceana em doze livros. Tão eficaz foi sua pregação no exílio que os bispos arianos pediram ao imperador que o devolvesse à Gália para 'não perturbar mais o Oriente'. De volta, presidiu sínodos que restauraram a ortodoxia nas Gálias e escreveu comentários bíblicos, hinos litúrgicos — sendo um dos primeiros hinógrafos latinos — e tratados sobre os Salmos. Faleceu por volta de 367. Pio IX o proclamou Doutor da Igreja em 1851, reconhecendo nele o principal defensor ocidental do dogma trinitário no século mais turbulento das controvérsias cristológicas.",
    virtudes: ["Firmeza na fé", "Amor à verdade revelada", "Perseverança no exílio"],
    frase: "Deus não pode ser conhecido senão por meio de si mesmo.",
  },
  {
    slug: "sao-cirilo-de-jerusalem",
    nome: "São Cirilo de Jerusalém",
    titulo: "Bispo e Doutor da Igreja",
    data: "18/03",
    seculo: "IV",
    padroeiro: "Catequistas, catecúmenos",
    resumo: "Bispo catequista, autor das célebres Catequeses aos candidatos ao Batismo.",
    biografia:
      "Nascido em Jerusalém ou arredores por volta de 315, foi ordenado sacerdote e encarregado da catequese dos candidatos ao batismo na cidade santa, tornando-se bispo por volta de 348. Suas dezenove 'Catequeses', pronunciadas durante a Quaresma e a semana pascal na basílica do Santo Sepulcro, constituem o testemunho mais completo da iniciação cristã do século IV: explicam o Credo artigo por artigo e, nas 'catequeses mistagógicas', os sacramentos do Batismo, da Crisma e da Eucaristia recém-recebidos pelos neófitos. Sua vida episcopal foi marcada por três exílios, impostos por adversários arianos e por disputas jurisdicionais sobre a sé de Jerusalém, totalizando cerca de dezesseis anos fora de sua cidade. Participou do primeiro Concílio de Constantinopla em 381, que confirmou sua ortodoxia. Testemunhou também, em 351, o aparecimento de uma luminosa cruz no céu de Jerusalém, relatado em carta ao imperador Constâncio. Morreu em 386, deixando uma catequese que continua modelo de clareza doutrinal unida à beleza mistagógica. Foi proclamado Doutor da Igreja por Leão XIII em 1883.",
    virtudes: ["Paciência nas provações", "Clareza no ensino da fé", "Amor aos catecúmenos"],
    frase: "Um só é o Batismo, uma só é a fé, um só é Deus e Pai.",
  },
  {
    slug: "sao-cirilo-de-alexandria",
    nome: "São Cirilo de Alexandria",
    titulo: "Bispo e Doutor da Igreja",
    data: "27/06",
    seculo: "IV–V",
    padroeiro: "Teólogos marianos, Egito",
    resumo: "Campeão da divina maternidade de Maria no Concílio de Éfeso contra Nestório.",
    biografia:
      "Sobrinho e sucessor do patriarca Teófilo, tornou-se bispo de Alexandria em 412, num contexto de tensões religiosas e sociais na grande metrópole egípcia. Sua atuação mais decisiva deu-se contra Nestório, patriarca de Constantinopla, que negava a Maria o título de Theotókos, Mãe de Deus, sustentando existirem em Cristo duas pessoas distintas. Cirilo, com rigor teológico e habilidade diplomática, obteve a condenação de Nestório no Concílio de Éfeso, em 431, definindo solenemente que Maria é verdadeiramente Mãe de Deus porque gerou a Pessoa única do Verbo encarnado. A proclamação foi recebida em Éfeso com aclamações populares e procissões de tochas em honra da Virgem. Escreveu numerosos comentários bíblicos, tratados cristológicos e cartas dogmáticas de grande precisão, sendo considerado o maior teólogo da escola alexandrina depois de Atanásio. Sua insistência na unidade da Pessoa de Cristo — 'uma só natureza encarnada do Verbo de Deus', em fórmula posteriormente reinterpretada com equilíbrio em Calcedônia — moldou definitivamente a cristologia católica e ortodoxa. Morreu em 444. Foi declarado Doutor da Igreja por Leão XIII em 1882.",
    virtudes: ["Zelo pela verdade mariana", "Rigor teológico", "Coragem pastoral"],
    frase: "A Virgem Santíssima é verdadeiramente Mãe de Deus.",
  },
  {
    slug: "sao-pedro-crisologo",
    nome: "São Pedro Crisólogo",
    titulo: "Bispo e Doutor da Igreja",
    data: "30/07",
    seculo: "V",
    padroeiro: "Pregadores",
    resumo: "'Palavra de ouro', bispo de Ravena célebre por homilias breves e luminosas.",
    biografia:
      "Nascido em Ímola por volta de 380, foi diácono sob o bispo Cornélio antes de ser escolhido, segundo a tradição de modo providencial, arcebispo de Ravena por volta de 433, então capital do Império Romano do Ocidente e sede imperial de Gala Placídia. Recebeu o sobrenome Crisólogo — 'palavra de ouro' — pela eloquência concisa e luminosa de suas homilias, das quais restam cerca de 176, quase todas breves, adaptadas à capacidade de assimilação do povo, mas densas de doutrina sobre o Credo, os sacramentos e a vida moral. Insistia que a pregação deveria ser 'curta, mas cheia', evitando tanto a superficialidade quanto o excesso retórico então em voga. Foi conselheiro espiritual de Eutiques antes de este cair no erro monofisita, exortando-o com carta prudente a submeter-se ao julgamento do papa Leão Magno. Promoveu a devoção à Virgem Maria e à Eucaristia, e zelou pela dignidade do culto na esplêndida Ravena bizantina, cujos mosaicos ainda hoje testemunham o esplendor litúrgico daquela época. Morreu por volta de 450 em Ímola, sua terra natal, e foi declarado Doutor da Igreja por Bento XIII em 1729.",
    virtudes: ["Clareza na pregação", "Prudência pastoral", "Devoção eucarística"],
    frase: "Quem quer ser ouvido por Deus, ouça primeiro Deus quando ele fala.",
  },
  {
    slug: "sao-pedro-damiao",
    nome: "São Pedro Damião",
    titulo: "Bispo, cardeal e Doutor da Igreja",
    data: "21/02",
    seculo: "XI",
    padroeiro: "Reformadores eclesiásticos",
    resumo: "Monge camaldulense, reformador implacável da disciplina do clero no século XI.",
    biografia:
      "Nascido em Ravena por volta de 1007, órfão e maltratado na infância, foi criado por um irmão que o fez estudar; tornou-se professor brilhante antes de renunciar a tudo para entrar no eremitério camaldulense de Fonte Avellana, do qual chegou a ser prior, imprimindo-lhe rigor ascético e vida de intensa oração e trabalho manual. Tornou-se o principal colaborador dos papas reformadores do século XI, sendo criado cardeal-bispo de Óstia em 1057, contra sua vontade de permanecer eremita. Combateu com veemência dois grandes males do clero de seu tempo: a simonia, compra e venda de cargos eclesiásticos, denunciada no 'Liber Gratissimus', e o nicolaísmo, a incontinência dos clérigos, atacado no áspero 'Liber Gomorrhianus'. Foi enviado como legado papal a Milão, à Alemanha e à França para resolver disputas episcopais e impor a reforma gregoriana antes mesmo de Gregório VII. Escreveu também tratados espirituais, cartas e uma vida de São Romualdo, fundador dos Camaldulenses. Homem de oração intensa e penitência severa, ansiava sempre voltar à solidão do eremitério. Morreu em Faenza em 1072, voltando de uma missão de paz. Foi proclamado Doutor da Igreja por Leão XII em 1828.",
    virtudes: ["Zelo reformador", "Amor à vida eremítica", "Coragem na correção fraterna"],
    frase: "Nada é mais precioso que a caridade, que sempre reina.",
  },
  {
    slug: "sao-joao-damasceno",
    nome: "São João Damasceno",
    titulo: "Monge, sacerdote e Doutor da Igreja",
    data: "04/12",
    seculo: "VII–VIII",
    padroeiro: "Ícones, artistas sacros, farmacêuticos",
    resumo: "Último dos Padres gregos, grande defensor da veneração das imagens sagradas.",
    biografia:
      "Nascido em Damasco por volta de 675, numa família cristã de alto funcionalismo na corte do califa omíada, sucedeu ao pai como administrador dos tributos cristãos antes de renunciar ao cargo e retirar-se ao mosteiro de São Sabas, perto de Jerusalém, onde foi ordenado sacerdote. Viveu no auge da crise iconoclasta, quando imperadores bizantinos ordenaram a destruição das imagens sagradas sob acusação de idolatria; protegido pelo domínio muçulmano, fora do alcance direto de Constantinopla, escreveu três tratados vigorosos 'Contra os que difamam as santas imagens', argumentando que, tendo Deus assumido carne visível na Encarnação, é legítimo representar visualmente o mistério e venerar, através da imagem, o modelo representado. Sua obra-prima, 'A Fonte do Conhecimento', sintetiza toda a teologia oriental precedente, tornando-se referência obrigatória inclusive para a escolástica latina, que a conheceu em tradução. Compôs também belíssimos hinos litúrgicos ainda cantados no rito bizantino, sobretudo os cânones pascais. Diz a tradição que teve a mão decepada por calúnia palaciana e milagrosamente restaurada pela Virgem, à qual ficou eternamente devoto. Morreu por volta de 749. Foi declarado Doutor da Igreja por Leão XIII em 1890.",
    virtudes: ["Defesa da tradição", "Devoção mariana", "Amor à beleza litúrgica"],
    frase: "Não adoro a matéria, mas o Criador da matéria.",
  },
  {
    slug: "sao-lourenco-de-brindisi",
    nome: "São Lourenço de Brindisi",
    titulo: "Sacerdote capuchinho e Doutor da Igreja",
    data: "21/07",
    seculo: "XVI–XVII",
    padroeiro: "Linguistas, diplomatas",
    resumo: "Capuchinho poliglota, pregador e diplomata, general da Ordem e Doutor Apostólico.",
    biografia:
      "Cesare de Rossi nasceu em Brindisi, no reino de Nápoles, em 1559, e entrou aos dezesseis anos entre os frades capuchinhos, recebendo o nome de Lourenço. Dotado de extraordinária facilidade para línguas, dominava latim, grego, hebraico, siríaco, francês, alemão, espanhol e sua língua materna, o que lhe permitiu pregar diretamente a públicos diversos e refutar controvérsias com protestantes e judeus a partir dos textos originais. Pregou incansavelmente pela Europa central, fundando conventos capuchinhos na Áustria, Boêmia e Alemanha, região então dilacerada pelas guerras de religião. Em 1601, a pedido do imperador Rodolfo II, atuou como capelão e conselheiro militar do exército cristão contra os turcos otomanos na Hungria, cavalgando à frente das tropas apenas com o crucifixo erguido, sem armas, na vitória de Székesfehérvár. Foi eleito ministro geral da Ordem Capuchinha em 1602 e, posteriormente, desempenhou delicadas missões diplomáticas a serviço da Santa Sé junto a príncipes católicos da Europa. Escreveu numerosos sermões e comentários bíblicos de refinada erudição, reunidos em quinze volumes. Morreu em Lisboa em 22 de julho de 1619, em missão diplomática. Foi proclamado Doutor da Igreja por João XXIII em 1959, com o título de 'Doutor Apostólico'.",
    virtudes: ["Zelo missionário", "Erudição a serviço da fé", "Coragem diplomática"],
    frase: "Quem confia em Deus não teme exército algum.",
  },
  {
    slug: "sao-roberto-belarmino",
    nome: "São Roberto Belarmino",
    titulo: "Cardeal jesuíta e Doutor da Igreja",
    data: "17/09",
    seculo: "XVI–XVII",
    padroeiro: "Catequistas, canonistas",
    resumo:
      "Teólogo jesuíta, principal controversista católico da Reforma, autor de catecismos populares.",
    biografia:
      "Nascido em Montepulciano, na Toscana, em 1542, sobrinho do futuro papa Marcelo II, entrou na Companhia de Jesus em 1560 e destacou-se como pregador e professor em Lovaina e depois na Universidade Gregoriana de Roma, onde ocupou a nova cátedra de controvérsias criada para responder sistematicamente ao protestantismo. Sua obra máxima, 'Disputationes de Controversiis Christianae Fidei', em três volumes, tratou com rigor histórico e bíblico os pontos disputados com luteranos e calvinistas — a autoridade do papa, os sacramentos, a justificação — tornando-se referência obrigatória de ambos os lados da polêmica. Criado cardeal em 1599 e depois arcebispo de Cápua, exerceu funções decisivas na Cúria Romana, participando do processo contra Giordano Bruno e das primeiras diligências relativas a Galileu Galilei, buscando sempre equilíbrio prudente entre ciência e fé. Escreveu ainda dois catecismos de extraordinária difusão popular, traduzidos em dezenas de línguas, e obras espirituais de grande unção, como 'A Arte de Bem Morrer'. Homem de vida austera, distribuía quase toda a renda aos pobres e recusou honrarias desnecessárias. Morreu em Roma em 1621. Foi canonizado e declarado Doutor da Igreja por Pio XI em 1931.",
    virtudes: ["Rigor doutrinal", "Simplicidade pessoal", "Caridade com os pobres"],
    frase: "Não há maior sabedoria que conhecer e amar a Deus.",
  },
  {
    slug: "sao-joao-de-avila",
    nome: "São João de Ávila",
    titulo: "Sacerdote e Doutor da Igreja",
    data: "10/05",
    seculo: "XVI",
    padroeiro: "Clero secular espanhol",
    resumo:
      "'Apóstolo da Andaluzia', mestre espiritual de Inácio de Loyola, Teresa de Ávila e João de Deus.",
    biografia:
      "Nascido em Almodóvar del Campo, na Espanha, em 1499, estudou direito em Salamanca e teologia em Alcalá, sendo ordenado sacerdote em 1526. Destinado a partir como missionário para as Índias, foi persuadido pelo arcebispo de Sevilha a permanecer na Espanha, onde havia campo vastíssimo para evangelizar. Pregou por toda a Andaluzia com tal eficácia que recebeu o título de 'Apóstolo da Andaluzia', convertendo multidões, entre elas nobres, universitários e futuros santos: aconselhou espiritualmente Inácio de Loyola, Francisco de Borja, João de Deus, Teresa de Ávila e Luís de Granada. Sofreu processo inquisitorial em 1531 por supostas afirmações imprudentes sobre a pobreza evangélica, sendo plenamente absolvido após um ano de prisão, período que aproveitou para aprofundar seus escritos espirituais. Fundou colégios e seminários que influenciaram decisivamente a formação sacerdotal tridentina, e sua obra-prima, 'Audi, Filia' ('Ouve, filha'), tratado de vida espiritual dedicado a uma jovem penitente, tornou-se clássico da mística castelhana. Enviou também memoriais aos padres do Concílio de Trento sobre a reforma do clero. Homem de saúde frágil e vida austera, morreu em Montilla em 1569. Foi declarado Doutor da Igreja por Bento XVI em 2012.",
    virtudes: ["Zelo pela reforma do clero", "Paciência na injustiça", "Direção espiritual sábia"],
    frase: "Mais vale um 'Pai Nosso' dito devagar que muitos ditos às pressas.",
  },
  {
    slug: "santo-efrem-o-sirio",
    nome: "Santo Efrém, o Sírio",
    titulo: "Diácono e Doutor da Igreja",
    data: "09/06",
    seculo: "IV",
    padroeiro: "Poetas cristãos, Síria",
    resumo: "'Harpa do Espírito Santo', diácono e poeta que evangelizou em hinos a fé trinitária.",
    biografia:
      "Nascido em Nísibis, na Mesopotâmia, por volta de 306, de família cristã, foi discípulo do bispo Jaime de Nísibis e recebeu o diaconato, ofício que exerceu toda a vida sem jamais aceitar o sacerdócio por humildade. Quando a cidade caiu sob domínio persa em 363, refugiou-se em Edessa, onde fundou uma escola catequética e dedicou-se à composição de hinos e sermões em versos siríacos de extraordinária beleza poética, valendo-lhe o título de 'Harpa do Espírito Santo'. Compôs milhares de estrofes contra as heresias arianas e gnósticas que então se espalhavam, mas sua maior originalidade está nos hinos marianos, eucarísticos e sobre o paraíso, que uniam doutrina sólida a imagens simbólicas de rara delicadeza, cantados por corais femininos que ele mesmo organizou nas igrejas de Edessa. Viveu com austeridade extrema, dormindo em caverna e alimentando-se de pão e ervas, mas em 373, durante uma fome que assolou a cidade, saiu de sua reclusão para organizar pessoalmente a distribuição de víveres aos famintos, exercendo intensa caridade prática até morrer, ainda naquele ano, contaminado pelas doenças que atendia. Sua influência marcou profundamente a liturgia siríaca oriental. Foi proclamado Doutor da Igreja por Bento XV em 1920.",
    virtudes: ["Humildade diaconal", "Poesia a serviço da fé", "Caridade com os famintos"],
    frase:
      "Senhor, encha meus lábios de louvores para que meu coração encontre alegria em cantar-te.",
  },
  {
    slug: "santo-ireneu-de-lyon",
    nome: "Santo Ireneu de Lyon",
    titulo: "Bispo e Doutor da Igreja",
    data: "28/06",
    seculo: "II",
    padroeiro: "Teólogos, unidade da Igreja",
    resumo:
      "Discípulo de Policarpo, o maior refutador do gnosticismo e teólogo da tradição apostólica.",
    biografia:
      "Nascido em Esmirna, na Ásia Menor, por volta de 140, ouviu ainda menino as pregações de Policarpo, que por sua vez fora discípulo do apóstolo João, formando assim elo vivo com a geração apostólica. Emigrou para as Gálias, onde se tornou sacerdote em Lyon e, após o martírio do bispo Potino durante a perseguição de 177, sucedeu-lhe na sé episcopal. Sua obra-prima, 'Contra as Heresias', em cinco livros, desmonta sistematicamente os sistemas gnósticos que then floresciam, opondo-lhes a regra da fé transmitida publicamente pelos apóstolos através da sucessão episcopal, com especial destaque à Igreja de Roma como referência de unidade: 'É necessário que toda Igreja concorde com esta Igreja, por causa de sua superior origem'. Desenvolveu a teologia da 'recapitulação', segundo a qual Cristo, novo Adão, refaz e eleva toda a história humana, e insistiu na unidade dos dois Testamentos contra o dualismo gnóstico. Interveio também na disputa sobre a data da Páscoa entre Roma e as Igrejas da Ásia, pedindo ao papa Vítor moderação e comunhão apesar da diversidade de costumes. Terá morrido mártir por volta de 202, durante perseguição em Lyon, embora a tradição não seja unânime. Foi proclamado Doutor da Igreja por Francisco em 2022, com o título de 'Doutor da Unidade'.",
    virtudes: ["Fidelidade à tradição apostólica", "Zelo pela unidade", "Rigor teológico"],
    frase: "A glória de Deus é o homem vivo, e a vida do homem é a visão de Deus.",
  },
  {
    slug: "sao-vicente-ferrer",
    nome: "São Vicente Ferrer",
    titulo: "Sacerdote dominicano, pregador apostólico",
    data: "05/04",
    seculo: "XIV–XV",
    padroeiro: "Pregadores, construtores, Valência",
    resumo:
      "Frade dominicano espanhol, pregador itinerante que percorreu a Europa anunciando a penitência.",
    biografia:
      "Nascido em Valência, na Espanha, em 1350, entrou na Ordem dos Pregadores aos dezoito anos, destacando-se rapidamente como teólogo e mestre em Toulouse. Durante o Grande Cisma do Ocidente, que dividiu a cristandade entre papas rivais, serviu como confessor e conselheiro do antipapa Bento XIII de Avinhão, esforçando-se por resolver o cisma, até finalmente romper com ele e contribuir para a solução unitária no Concílio de Constança. Dedicou os últimos vinte anos de vida a uma prodigiosa peregrinação de pregação a pé por toda a Espanha, França, Suíça e Itália, sempre acompanhado de penitentes cantando o 'Miserere', anunciando com veemência apocalíptica a urgência da conversão e do juízo final, o que lhe valeu fama de taumaturgo e converteu, segundo a tradição, milhares de judeus e mouros, além de reconciliar comunidades cristãs divididas. Falava apenas o valenciano, mas era compreendido, segundo relatos da época, por ouvintes de línguas diferentes — dom atribuído ao Espírito Santo. Contribuiu decisivamente para a resolução do compromisso de Caspe, que definiu a sucessão ao trono de Aragão. Morreu em Vannes, na Bretanha francesa, em 1419, durante uma de suas missões. Foi canonizado em 1455.",
    virtudes: ["Zelo pela conversão", "Espírito de penitência", "Trabalho pela unidade da Igreja"],
    frase: "Temei a Deus e dai-lhe glória, porque chegou a hora do seu juízo.",
  },
  {
    slug: "santa-escolastica",
    nome: "Santa Escolástica",
    titulo: "Virgem, irmã gêmea de São Bento",
    data: "10/02",
    seculo: "V–VI",
    padroeiro: "Freiras beneditinas, contra tempestades",
    resumo:
      "Irmã gêmea de São Bento, consagrada a Deus desde a infância, fundadora do monaquismo feminino beneditino.",
    biografia:
      "Nascida em Núrsia por volta de 480, irmã gêmea de São Bento, consagrou-se a Deus desde jovem e, segundo o testemunho de São Gregório Magno em seus 'Diálogos', fundou e dirigiu uma comunidade de virgens perto de Monte Cassino, em Plombariola, seguindo a Regra que o irmão redigira para os monges. Uma vez por ano os dois irmãos se encontravam numa casa a meio caminho entre os respectivos mosteiros para conversar sobre as coisas de Deus. No último encontro, sentindo aproximar-se a morte, Escolástica implorou ao irmão que prolongasse a conversa noturna sobre as alegrias do Paraíso; diante da recusa de Bento, que insistia em respeitar a regra do retorno ao mosteiro antes da noite, ela orou com lágrimas, e imediatamente desencadeou-se uma tempestade tão violenta que impediu qualquer saída, obrigando-o a permanecer até de madrugada. Bento então lhe disse: 'Deus te perdoe, irmã, que fizeste?', ao que ela respondeu terem sido atendidas suas preces porque o amor pode mais do que a norma. Três dias depois, Bento viu em visão a alma da irmã subindo ao céu em forma de pomba, e mandou sepultá-la no túmulo que preparara para si mesmo em Monte Cassino. Faleceu por volta de 547.",
    virtudes: ["Amor superior à norma", "Perseverança na oração", "Vida contemplativa"],
    frase: "Pedi a Deus, e ele fez o que eu não pude obter de ti.",
  },
  {
    slug: "sao-damiao-de-molokai",
    nome: "São Damião de Molokai",
    titulo: "Sacerdote missionário, apóstolo dos leprosos",
    data: "10/05",
    seculo: "XIX",
    padroeiro: "Leprosos, portadores de HIV, excluídos sociais",
    resumo:
      "Padre belga que se entregou aos hansenianos da ilha de Molokai até contrair a própria doença.",
    biografia:
      "Jozef De Veuster nasceu em Tremelo, na Bélgica, em 1840, e entrou na congregação dos Sagrados Corações, recebendo o nome de Damião. Partiu como missionário para o Havaí em 1864, onde foi ordenado sacerdote, e em 1873 ofereceu-se voluntariamente para servir a colônia de hansenianos isolada compulsoriamente na península de Kalaupapa, na ilha de Molokai, para onde eram deportados os doentes sem qualquer estrutura médica ou espiritual, em condições de abandono, violência e desespero. Ali construiu igrejas, casas, um sistema de água potável, organizou coral e banda musical, cuidou pessoalmente dos feridos, cavou sepulturas e devolveu dignidade humana e cristã a milhares de excluídos, insistindo em tratá-los como irmãos e não como parias. Em 1885 diagnosticou em si mesmo os primeiros sinais da lepra, contraída no convívio direto com os doentes, e passou a dirigir-se aos fiéis dizendo 'nós, os leprosos', identificando-se totalmente com seu rebanho sofredor. Continuou o trabalho por mais quatro anos apesar do avanço da doença, obtendo recursos internacionais e o apoio de outras congregações religiosas. Morreu em Kalaupapa em 15 de abril de 1889, aos 49 anos. Foi canonizado por Bento XVI em 2009.",
    virtudes: [
      "Doação total aos excluídos",
      "Identificação com os sofredores",
      "Coragem diante do contágio",
    ],
    frase: "Sem o Santíssimo Sacramento, uma posição como a minha seria insustentável.",
  },
  {
    slug: "santa-kateri-tekakwitha",
    nome: "Santa Kateri Tekakwitha",
    titulo: "Virgem leiga, 'Lírio dos Mohawks'",
    data: "17/04",
    seculo: "XVII",
    padroeiro: "Povos indígenas, ecologia, órfãos",
    resumo:
      "Primeira santa indígena da América do Norte, virgem consagrada apesar de tradições e perseguições tribais.",
    biografia:
      "Nascida em 1656 numa aldeia mohawk no atual estado de Nova York, filha de chefe mohawk pagão e de mãe cristã algonquina, ficou órfã aos quatro anos numa epidemia de varíola que também lhe deixou o rosto marcado e a visão enfraquecida — daí o nome Tekakwitha, 'a que avança tateando'. Criada por tios que se opunham ao cristianismo, foi instruída na fé por missionários jesuítas e batizada aos vinte anos, assumindo o nome de Catarina — Kateri, em mohawk. A conversão provocou hostilidade violenta da própria comunidade, que a via como traidora das tradições ancestrais; obrigada a fugir, refugiou-se numa missão cristã indígena junto ao rio São Lourenço, no atual Canadá. Ali viveu com austeridade extraordinária, fazendo voto perpétuo de virgindade — algo inédito entre as mulheres indígenas de sua nação —, dedicando-se à catequese das crianças, ao cuidado dos doentes e idosos e a rigorosas penitências corporais. Morreu em 1680, aos vinte e quatro anos; testemunhas relataram que, momentos após a morte, as marcas da varíola desapareceram de seu rosto, deixando-o de repente resplandecente. Foi canonizada por Bento XVI em 2012, tornando-se a primeira santa indígena norte-americana.",
    virtudes: [
      "Fidelidade apesar da perseguição familiar",
      "Virgindade consagrada",
      "Amor aos pobres e doentes",
    ],
    frase: "Jesus, eu te amo.",
  },
  {
    slug: "santo-andre-kim-taegon",
    nome: "Santo André Kim Taegon",
    titulo: "Primeiro sacerdote coreano, mártir",
    data: "20/09",
    seculo: "XIX",
    padroeiro: "Clero coreano, catequistas",
    resumo: "Primeiro padre nativo da Coreia, decapitado aos 25 anos na perseguição de 1846.",
    biografia:
      "Nascido em 1821 numa família de nobres convertidos ao catolicismo numa Coreia hermeticamente fechada ao cristianismo, teve o avô e o pai martirizados antes dele. Batizado ainda menino, foi enviado clandestinamente a Macau para estudar no seminário, cruzando a pé e de barco milhares de quilômetros através da Manchúria. Ordenado sacerdote em Xangai em 1845, o primeiro coreano a receber o presbiterato, voltou secretamente à pátria para organizar rotas de entrada de missionários estrangeiros e sustentar a Igreja subterrânea, que já contava dezenas de milhares de fiéis apesar de décadas de perseguições sangrentas. Foi preso pouco depois do retorno, ao tentar recolher um bispo francês que desembarcava clandestinamente, e submetido a torturas e interrogatórios em que confessou serenamente a fé, recusando renegar Cristo apesar das promessas de perdão em troca da apostasia. Escreveu, da prisão, uma última carta aos fiéis exortando-os à perseverança: 'Não tenhamos medo, meus queridos irmãos... apertai-vos ao Senhor como o filho se apega ao seio da mãe'. Foi decapitado às margens do rio Han, em Seul, em 16 de setembro de 1846, junto com outros companheiros. É venerado como o mais célebre dos 103 mártires coreanos canonizados por João Paulo II em 1984.",
    virtudes: [
      "Coragem apostólica",
      "Fidelidade filial à Igreja perseguida",
      "Serenidade no martírio",
    ],
    frase: "Apertai-vos ao Senhor como o filho se apega ao seio da mãe.",
  },
  {
    slug: "sao-paulo-miki",
    nome: "São Paulo Miki",
    titulo: "Jesuíta japonês, mártir de Nagasaki",
    data: "06/02",
    seculo: "XVI",
    padroeiro: "Japão, catequistas leigos",
    resumo:
      "Noviço jesuíta japonês, o mais célebre dos 26 mártires crucificados em Nagasaki em 1597.",
    biografia:
      "Nascido por volta de 1562 em família nobre japonesa, educado no colégio jesuíta de Azuchi, entrou na Companhia de Jesus e destacou-se como pregador eloquente, atraindo grande número de conversões entre samurais e comerciantes num Japão ainda parcialmente aberto ao Evangelho trazido por Francisco Xavier décadas antes. Quando o regente Toyotomi Hideyoshi, temendo a influência estrangeira e a fidelidade dividida dos convertidos, decretou a perseguição aos cristãos, Paulo Miki foi preso em Kyoto junto com outros vinte e cinco cristãos — sacerdotes, religiosos e leigos, entre eles crianças — e obrigado a marchar centenas de quilômetros até Nagasaki, mutilada uma orelha de cada um como sinal de infâmia pública. Ao longo da marcha de um mês, sob frio e escárnio, os prisioneiros seguiam cantando o Te Deum e rezando o terço. Chegados à colina de Nishizaka, em 5 de fevereiro de 1597, foram amarrados a vinte e seis cruzes; Paulo Miki, do alto da própria cruz, pregou ao povo reunido, perdoando os executores e proclamando não haver outro caminho de salvação senão o cristão, antes de ser traspassado por lanças junto com os companheiros. O episódio, narrado por testemunhas europeias presentes, tornou-se o relato fundador do martírio cristão japonês. Foram canonizados em 1862.",
    virtudes: ["Coragem juvenil", "Perdão aos algozes", "Testemunho até o fim"],
    frase: "Não tenho outro caminho a seguir senão o cristão.",
  },
  {
    slug: "sao-carlos-lwanga",
    nome: "São Carlos Lwanga",
    titulo: "Mártir de Uganda, padroeiro da juventude africana",
    data: "03/06",
    seculo: "XIX",
    padroeiro: "Jovens africanos, catequistas, escoteiros",
    resumo:
      "Chefe dos pajens do rei de Buganda, queimado vivo por defender a castidade dos jovens cristãos.",
    biografia:
      "Carlos Lwanga servia na corte do rei Mwanga II do reino de Buganda, na atual Uganda, como responsável pelos jovens pajens reais, muitos deles catecúmenos ou já cristãos, instruídos pelos Missionários de África conhecidos como Padres Brancos. O rei, de conduta dissoluta, exigia dos pajens submissão a práticas imorais, e os jovens cristãos, sob liderança e proteção de Carlos, resistiam firmemente, protegendo especialmente os mais novos da corrupção real. Irritado com a recusa e temendo a crescente influência cristã na corte, Mwanga decretou em 1886 a execução de todos os cristãos da casa real. Carlos foi batizado às pressas na própria noite anterior à prisão, sabendo do perigo iminente, e no dia seguinte, junto com outros doze pajens e companheiros de fé, foi condenado a morrer envolto em esteiras de palha e queimado vivo em Namugongo. Testemunhas relataram que os jovens mártires — ao todo vinte e dois católicos, além de mártires anglicanos executados na mesma perseguição — enfrentaram a fogueira cantando e perdoando os carrascos, alguns pedindo apenas para morrer mais lentamente a fim de ter mais tempo de oferecer o sofrimento a Deus. Foram canonizados por Paulo VI em 1964, primeiros santos oficialmente reconhecidos da África subsaariana moderna.",
    virtudes: [
      "Defesa da castidade alheia",
      "Coragem diante do poder tirânico",
      "Fé firme apesar da juventude",
    ],
    frase: "Deus há de me ajudar; morro por defender meus meninos.",
  },
  {
    slug: "sao-bento-jose-labre",
    nome: "São Bento José Labre",
    titulo: "Peregrino mendicante",
    data: "16/04",
    seculo: "XVIII",
    padroeiro: "Sem-teto, peregrinos, mendigos",
    resumo:
      "Peregrino francês que renunciou a tudo para viver mendicante pelos santuários da Europa.",
    biografia:
      "Nascido em Amettes, no norte da França, em 1748, primogênito de quinze irmãos, tentou por diversas vezes ingressar em ordens religiosas contemplativas — trapistas e cartuxos — sendo sempre recusado por fragilidade de saúde ou falta de vocação claramente monástica. Discerniu então uma vocação singular: viver como peregrino mendicante perpétuo, visitando a pé os principais santuários da cristandade europeia — Loreto, Assis, Nápoles, Santiago de Compostela e, repetidamente, Roma — vestido de trapos, dormindo ao relento ou em abrigos de caridade, alimentando-se do que lhe davam e repartindo com outros mendigos o pouco que recebia. Passava longas horas em adoração diante do Santíssimo Sacramento nas igrejas romanas, especialmente no Coliseu, então lugar de devoção aos mártires, e rezava o rosário incessantemente. Apesar da aparência miserável, transmitia paz e atraía a atenção de quem o observava em oração prolongada e imóvel. Recusou toda forma de conforto ou reconhecimento, vivendo radicalmente as bem-aventuranças da pobreza. Morreu em Roma, exausto, na escadaria da igreja de Santa Maria dei Monti, em 16 de abril de 1783, aos trinta e cinco anos; imediatamente o povo romano o aclamou como santo, gritando pelas ruas 'è morto il santo!'. Foi canonizado em 1881.",
    virtudes: ["Pobreza radical", "Perseverança na oração", "Humildade extrema"],
    frase: "É preciso amar a Deus sobre todas as coisas, sem medida.",
  },
  {
    slug: "santa-josefina-bakhita",
    nome: "Santa Josefina Bakhita",
    titulo: "Religiosa canossiana, ex-escrava sudanesa",
    data: "08/02",
    seculo: "XIX–XX",
    padroeiro: "Vítimas de tráfico humano, Sudão",
    resumo:
      "Sequestrada e escravizada na infância no Sudão, tornou-se religiosa e símbolo da dignidade humana.",
    biografia:
      "Nascida por volta de 1869 numa aldeia da região de Darfur, no Sudão, foi sequestrada aos sete anos por traficantes de escravos, que lhe deram o nome árabe 'Bakhita' — 'a afortunada' — em cruel ironia, apagando de propósito seu nome original, jamais recuperado. Vendida e revendida sucessivas vezes por mais de uma década, sofreu torturas físicas terríveis, entre elas mais de cem incisões rituais no corpo, marcas que carregou pela vida inteira. Comprada em 1883 pelo cônsul italiano Callisto Legnani em Cartum, foi finalmente tratada com humanidade e, levada à Itália, entregue aos cuidados da família Michieli, que a confiou às Irmãs Canossianas de Veneza para instrução enquanto viajavam. Ali ouviu falar de Deus pela primeira vez como Pai amoroso e não como senhor a ser temido, e decidiu permanecer com as irmãs quando a família quis levá-la de volta ao Sudão; um tribunal italiano reconheceu-lhe a liberdade, pois a escravidão era ilegal em solo italiano. Batizada em 1890, entrou na congregação em 1893 e viveu por décadas em Schio, dedicada à cozinha, à portaria e ao acolhimento carinhoso de todos, sempre repetindo: 'Se encontrasse os que me escravizaram, ajoelhar-me-ia para beijar-lhes as mãos, pois, se não fosse o que aconteceu, não seria hoje cristã e religiosa'. Morreu em 1947. Foi canonizada por João Paulo II em 2000.",
    virtudes: ["Perdão aos algozes", "Serenidade após o sofrimento", "Alegria simples no serviço"],
    frase: "Se encontrasse aqueles que me escravizaram, ajoelhar-me-ia para beijar-lhes as mãos.",
  },
  {
    slug: "sao-sebastiao",
    nome: "São Sebastião",
    titulo: "Mártir romano, capitão da guarda pretoriana",
    data: "20/01",
    seculo: "III",
    padroeiro: "Soldados, atletas, cidade do Rio de Janeiro, contra epidemias",
    resumo:
      "Oficial romano cristão, transpassado por flechas por ordem de Diocleciano e depois trucidado.",
    biografia:
      "Segundo a tradição consolidada por Santo Ambrósio e pela Paixão de São Sebastião do século V, nasceu em Milão ou na Gália Narbonense por volta de 256 e ingressou no exército romano, alcançando o posto de capitão da guarda pretoriana sob os imperadores Diocleciano e Maximiano, sem revelar publicamente sua fé cristã. Usava a posição privilegiada para assistir secretamente cristãos presos e condenados, fortalecendo-os na hora do martírio e, segundo relatos, operando conversões mesmo entre funcionários imperiais. Descoberta sua fé, foi condenado por Diocleciano a ser atado a um poste e alvejado por arqueiros mouros, sendo dado como morto e abandonado. Uma viúva cristã, Irene, ao recolher o corpo para sepultá-lo, encontrou-o ainda vivo e cuidou de seus ferimentos até a recuperação. Restabelecido, em vez de fugir, apresentou-se novamente diante do imperador para denunciar publicamente a crueldade das perseguições contra os cristãos; furioso, Diocleciano ordenou que fosse espancado até a morte e seu corpo lançado na Cloaca Máxima, de onde outra piedosa cristã o recolheu para sepultura digna nas catacumbas que hoje levam seu nome, na via Ápia. Desde a Idade Média é invocado contra pestes e epidemias, e sua iconografia — o corpo atravessado de flechas — tornou-se uma das mais representadas da arte cristã.",
    virtudes: [
      "Coragem militar a serviço da fé",
      "Fidelidade aos perseguidos",
      "Perseverança após o sofrimento",
    ],
    frase: "Nunca estive mais próximo de Cristo do que quando as flechas me atravessavam.",
  },
  {
    slug: "sao-roque",
    nome: "São Roque",
    titulo: "Peregrino taumaturgo, protetor contra pestes",
    data: "16/08",
    seculo: "XIV",
    padroeiro: "Enfermos contagiosos, cães, cirurgiões",
    resumo:
      "Peregrino francês que curou pestilentos na Itália e sofreu a própria peste em solidão exemplar.",
    biografia:
      "Nascido em Montpellier, no sul da França, por volta de 1295, filho de família nobre, ficou órfão ainda jovem e distribuiu toda a herança aos pobres antes de partir em peregrinação a Roma, vestido de peregrino com bastão, bornal e a concha característica. Atravessando a Itália durante uma violenta epidemia de peste, dedicou-se ao cuidado dos doentes em hospitais de Acquapendente, Cesena, Roma e outras cidades, curando muitos, segundo a tradição, apenas com o sinal da cruz e a imposição das mãos, recusando fugir do contágio como faziam a maioria dos que podiam. Contraiu ele mesmo a peste em Piacenza e, para não contaminar ninguém, retirou-se sozinho a uma cabana na floresta, onde teria sido sustentado milagrosamente por um cão que lhe trazia diariamente um pedaço de pão roubado da mesa de seu dono — motivo pelo qual é sempre representado com um cão a seu lado. Recuperado, voltou à cidade natal, mas, não sendo reconhecido devido à aparência desfigurada pela doença e pelos anos de peregrinação, foi preso como suposto espião e morreu na prisão, ainda jovem, por volta de 1327, sendo identificado postumamente por uma marca de nascença em forma de cruz. Sua devoção espalhou-se amplamente pela Europa e pelo Brasil durante surtos epidêmicos, sobretudo de cólera e varíola.",
    virtudes: [
      "Caridade com os contagiados",
      "Solidariedade autoisolada",
      "Confiança na Providência",
    ],
    frase: "Aqueles que me invocarem serão livres de toda peste.",
  },
  {
    slug: "santo-expedito",
    nome: "Santo Expedito",
    titulo: "Mártir, padroeiro das causas urgentes",
    data: "19/04",
    seculo: "III–IV",
    padroeiro: "Causas urgentes, comerciantes, estudantes em exames",
    resumo:
      "Soldado mártir de tradição antiga, invocado com grande popularidade para necessidades urgentes.",
    biografia:
      "A tradição hagiográfica mais difundida apresenta Expedito como oficial do exército romano estacionado em Melitene, na Armênia, martirizado durante as perseguições do século III ou início do IV por professar publicamente a fé cristã. Reza a piedade popular que, no momento de sua conversão, o demônio lhe teria aparecido sob a forma de um corvo grasnando 'cras, cras' — em latim, 'amanhã, amanhã' — tentando-o a adiar a decisão de abraçar o cristianismo; Expedito, pisando o corvo, teria respondido 'hodie', 'hoje', batizando-se imediatamente e selando pouco depois com o próprio sangue essa resposta pronta e sem demora. Embora os dados históricos precisos sobre sua vida sejam escassos, restando sobretudo a antiguíssima veneração litúrgica atestada em calendários romanos desde ao menos o século IX, seu culto floresceu extraordinariamente a partir do século XIX, primeiro na Europa e depois, com grande intensidade, no Brasil, onde se tornou um dos santos mais invocados popularmente para necessidades urgentes, exames, empregos e questões que não admitem demora — precisamente pelo simbolismo do 'hoje' contra o 'amanhã' da procrastinação. Sua imagem tradicional, um jovem soldado romano com uma cruz na mão erguendo a palavra 'HODIE' e pisando o corvo do 'CRAS', tornou-se ícone de devoção popular difundida em capelas, medalhas e estampas por todo o país.",
    virtudes: [
      "Presteza na conversão",
      "Coragem no martírio",
      "Confiança na Providência sem demora",
    ],
    frase: "Hoje, não amanhã.",
  },
  {
    slug: "sao-longuinho",
    nome: "São Longuinho",
    titulo: "Centurião convertido ao pé da cruz",
    data: "15/03",
    seculo: "I",
    padroeiro: "Objetos e causas perdidas (devoção popular brasileira)",
    resumo:
      "Soldado romano que, segundo a tradição, transpassou o lado de Cristo e se converteu ao ver os prodígios do Calvário.",
    biografia:
      "A tradição cristã antiga, recolhida em apócrifos como o Evangelho de Nicodemos e transmitida por diversos Padres da Igreja, identifica com o nome de Longuinho o soldado romano que, no Calvário, cravou a lança no lado de Cristo já morto, do qual, segundo o relato joanino, jorrou sangue e água — símbolo sacramental caro à tradição patrística. O mesmo relato popular funde essa figura com a do centurião que, testemunhando os prodígios que acompanharam a morte de Jesus — o véu do templo rasgado, o terremoto, o eclipse — exclamou: 'Verdadeiramente este homem era Filho de Deus', reconhecendo publicamente a divindade do Crucificado no instante em que os próprios discípulos haviam fugido apavorados. A tradição posterior acrescenta que, segundo alguns escritos apócrifos, o soldado sofria de fraqueza na visão e, ao ser tocado pelo sangue e água que jorraram da lançada, teria recuperado a vista, tornando-se ainda mais convicto testemunha do mistério que presenciara; convertido, teria deixado a vida militar para pregar o Evangelho, morrendo mártir na Capadócia. Embora os estudiosos apontem a natureza devocional e não estritamente histórica dessa tradição composta, a piedade popular, sobretudo no Brasil, consagrou amplamente a figura de São Longuinho como padroeiro invocado para encontrar objetos perdidos, unindo-se nessa devoção a Santo Antônio.",
    virtudes: ["Conversão diante da verdade", "Testemunho corajoso", "Fé nascida da graça"],
    frase: "Verdadeiramente este homem era o Filho de Deus.",
  },
  {
    slug: "santo-antao",
    nome: "Santo Antão",
    titulo: "Pai dos monges do deserto",
    data: "17/01",
    seculo: "III-IV",
    padroeiro: "Monges, criadores de animais, doentes de pele",
    resumo: "Eremita egípcio que abriu o caminho da vida monástica cristã no deserto da Tebaida.",
    biografia:
      "Nasceu por volta de 251 em Coma, no Egito, filho de camponeses cristãos acomodados. Órfão aos vinte anos, ouviu na assembleia litúrgica a passagem 'vai, vende o que tens e dá aos pobres' e a tomou literalmente: distribuiu as terras, confiou a irmã a uma comunidade de virgens e retirou-se para a periferia do povoado, depois para os túmulos e por fim para um forte abandonado junto ao Nilo, onde viveu vinte anos em solidão. A Vida de Antão, escrita por Santo Atanásio de Alexandria pouco depois de sua morte, é a principal fonte histórica e descreve suas lutas interiores, seus conselhos de discernimento e a multidão de discípulos que povoou o deserto. Antão desceu duas vezes a Alexandria: para consolar os mártires na perseguição de Diocleciano e para sustentar Atanásio contra o arianismo. Morreu por volta de 356, com mais de cem anos.",
    virtudes: ["Renúncia radical", "Discernimento dos espíritos", "Perseverança na oração"],
    frase: "Quem permanece no deserto tem paz de três guerras: do ouvir, do falar e do ver.",
  },
  {
    slug: "santa-genoveva",
    nome: "Santa Genoveva",
    titulo: "Padroeira de Paris",
    data: "03/01",
    seculo: "V-VI",
    padroeiro: "Paris, mulheres consagradas no mundo",
    resumo: "Virgem consagrada que sustentou a fé e a resistência de Paris no tempo das invasões.",
    biografia:
      "Nasceu por volta de 420 em Nanterre. Ainda menina, foi notada por São Germano de Auxerre, que a encorajou a consagrar-se a Deus; recebeu o véu das virgens da mão do bispo de Paris. Vivendo na cidade, uniu jejum prolongado, vigílias e uma ação pública incomum para uma mulher de seu tempo: quando Átila e os hunos avançaram sobre a Gália em 451, persuadiu a população parisiense a não abandonar a cidade e a organizar orações e penitência — os hunos desviaram-se para Orleans. Durante o cerco franco de Paris, conduziu barcos rio acima para buscar trigo e distribuir aos famintos. Foi devota do culto a São Dionísio, promovendo a construção de uma basílica sobre seu túmulo. Morreu por volta de 502 e foi sepultada em Paris, onde seu culto permaneceu vivo por toda a Idade Média.",
    virtudes: ["Coragem cívica", "Penitência", "Caridade organizada"],
    frase: "Não fujais: orai, e a cidade permanecerá.",
  },
  {
    slug: "santa-angela-de-merici",
    nome: "Santa Ângela de Mérici",
    titulo: "Fundadora das Ursulinas",
    data: "27/01",
    seculo: "XV-XVI",
    padroeiro: "Educadoras, doentes, órfãos",
    resumo:
      "Leiga franciscana que criou a primeira companhia de mulheres consagradas dedicada à educação.",
    biografia:
      "Nasceu em Desenzano, no lago de Garda, em 1474. Órfã cedo, tornou-se terciária franciscana e viveu como leiga consagrada, sem clausura, dedicando-se ao catecismo das meninas pobres e à reconciliação de famílias em conflito. Peregrinou à Terra Santa e a Roma no jubileu de 1525. Em Bréscia, em 1535, reuniu vinte e oito companheiras e fundou a Companhia de Santa Úrsula: mulheres que permaneciam nas próprias casas e no próprio trabalho, unidas por uma regra de oração, castidade e serviço — uma forma de vida inédita, aprovada pelo bispo em 1536 e depois pelo papa Paulo III. Deixou uma Regra e escritos de direção espiritual marcados pela suavidade e pela confiança na liberdade das pessoas. Morreu em 1540 e foi canonizada em 1807; as Ursulinas tornaram-se a primeira grande rede educativa feminina da Igreja.",
    virtudes: ["Zelo pela educação", "Doçura firme", "Iniciativa apostólica"],
    frase: "Agi, movei-vos, crede, esforçai-vos, esperai, clamai a Ele de coração.",
  },
  {
    slug: "sao-vicente-pallotti",
    nome: "São Vicente Pallotti",
    titulo: "Apóstolo de Roma",
    data: "22/01",
    seculo: "XIX",
    padroeiro: "Apostolado dos leigos, missões populares",
    resumo: "Sacerdote romano que antecipou em um século a teologia do apostolado dos leigos.",
    biografia:
      "Nasceu em Roma em 1795 e foi ordenado sacerdote em 1818. Doutor em teologia, deixou a carreira acadêmica para dedicar-se aos bairros pobres, aos presos, aos soldados e aos emigrantes, criando escolas noturnas de ofícios para jovens trabalhadores. Em 1835 fundou a Pia Sociedade do Apostolado Católico — hoje palotinos e irmãs palotinas — sobre a convicção de que todo batizado, e não apenas o clero, é chamado a ser apóstolo, ideia que o Concílio Vaticano II retomaria no decreto Apostolicam actuositatem. Promoveu a Oitava da Epifania com liturgias de vários ritos orientais em Roma, sinal de sua paixão pela unidade dos cristãos. Confessor incansável, morreu em 1850 após adoecer ao ceder o próprio manto a um pobre. Foi canonizado por João XXIII em 1963, durante o Concílio.",
    virtudes: ["Zelo missionário", "Caridade discreta", "Visão eclesial"],
    frase: "A caridade de Cristo nos impele: cada batizado é um apóstolo.",
  },
  {
    slug: "sao-bras",
    nome: "São Brás",
    titulo: "Bispo e mártir",
    data: "03/02",
    seculo: "IV",
    padroeiro: "Doenças da garganta, médicos, tecelões",
    resumo:
      "Bispo de Sebaste, na Armênia, mártir invocado desde a antiguidade contra os males da garganta.",
    biografia:
      "Os dados históricos seguros sobre Brás são poucos: foi bispo de Sebaste, na Armênia, e morreu mártir por volta de 316, no contexto das perseguições que se seguiram ao edito de Licínio no Oriente. Seu culto está atestado no Oriente já no século VI e difundiu-se largamente no Ocidente a partir do século VIII. A tradição hagiográfica, recolhida em atas tardias e de valor devocional, narra que exercia a medicina antes do episcopado, que se refugiou numa gruta durante a perseguição — onde os animais o procuravam — e que, sendo conduzido ao martírio, curou um menino que sufocava com uma espinha de peixe presa na garganta. Dessa narrativa nasceu a bênção das gargantas com duas velas cruzadas, celebrada em sua memória e ainda presente no Bento Ritual. A Igreja distingue claramente o fato do martírio, historicamente firme, dos episódios lendários que a piedade popular conservou.",
    virtudes: ["Fortaleza no martírio", "Compaixão pelos doentes", "Fidelidade pastoral"],
    frase: "Por intercessão de São Brás, livre-te Deus dos males da garganta.",
  },
  {
    slug: "santa-apolonia",
    nome: "Santa Apolônia",
    titulo: "Virgem e mártir de Alexandria",
    data: "09/02",
    seculo: "III",
    padroeiro: "Dentistas, doentes dos dentes",
    resumo:
      "Diaconisa idosa martirizada em Alexandria em 249, testemunha citada por Eusébio de Cesareia.",
    biografia:
      "Apolônia é uma das mártires de quem se possui testemunho antigo e direto: Eusébio de Cesareia, na História Eclesiástica, transcreve uma carta de Dionísio de Alexandria que descreve o tumulto anticristão de 249, sob Décio, e nomeia 'a admirável virgem idosa Apolônia', a quem os agressores quebraram os dentes e ameaçaram queimar viva se não blasfemasse. Segundo o relato, ela pediu um instante, rezou e lançou-se ela mesma ao fogo — gesto que os Padres interpretaram não como suicídio, mas como impulso do Espírito, discussão que Santo Agostinho retoma na Cidade de Deus. A iconografia a representa com uma tenaz e um dente, e daí nasceu sua invocação contra as dores dentárias, atestada na Europa desde a Idade Média. Seu culto é antigo em Alexandria e em Roma, onde recebeu igreja própria.",
    virtudes: ["Constância na velhice", "Coragem diante da tortura", "Confissão pública da fé"],
    frase: "Sou cristã: nada me fará blasfemar do meu Deus.",
  },
  {
    slug: "sao-jeronimo-emiliano",
    nome: "São Jerônimo Emiliano",
    titulo: "Pai dos órfãos",
    data: "08/02",
    seculo: "XVI",
    padroeiro: "Órfãos e crianças abandonadas",
    resumo:
      "Soldado veneziano convertido no cárcere, fundador dos Somascos e organizador dos primeiros orfanatos modernos.",
    biografia:
      "Nasceu em Veneza em 1486, de família patrícia, e seguiu a carreira militar da República. Feito prisioneiro na defesa do castelo de Castelnuovo, viveu no cárcere uma conversão profunda e, libertado, dedicou o resto da vida aos pobres. Durante a fome e a peste que assolaram o norte da Itália em 1528, vendeu bens e casa para alimentar famintos e sepultar mortos, recolhendo as crianças órfãs que vagavam pelas ruas. Abriu casas de acolhida em Veneza, Bréscia, Bérgamo, Como e Milão, e criou um método de assistência que unia teto, catequese, alfabetização e aprendizado de um ofício — inovação notável para a época. Em 1534 fundou em Somasca a Companhia dos Servos dos Pobres, hoje Clérigos Regulares Somascos. Morreu em 1537 ao contrair uma epidemia cuidando dos doentes; Pio XI o declarou padroeiro universal dos órfãos.",
    virtudes: ["Caridade heroica", "Humildade após a conversão", "Serviço às crianças"],
    frase: "Servir os órfãos é servir o próprio Cristo abandonado.",
  },
  {
    slug: "sao-cirilo-e-metodio",
    nome: "São Cirilo e Metódio",
    titulo: "Apóstolos dos eslavos e copadroeiros da Europa",
    data: "14/02",
    seculo: "IX",
    padroeiro: "Europa, povos eslavos, tradutores",
    resumo:
      "Irmãos de Tessalônica que criaram a escrita eslava e a liturgia na língua do povo evangelizado.",
    biografia:
      "Constantino — que tomou em religião o nome de Cirilo — e Miguel, chamado Metódio, nasceram em Tessalônica no início do século IX, filhos de um oficial bizantino. Cirilo foi bibliotecário e professor de filosofia em Constantinopla; Metódio, administrador e depois monge. Em 863, enviados pelo imperador ao pedido do príncipe Ratislau da Morávia, criaram um alfabeto para a língua eslava — o glagolítico, base do futuro cirílico — e traduziram os Evangelhos, o Saltério e os livros litúrgicos. A opção pela língua vernácula na liturgia provocou forte oposição do clero franco; os irmãos foram a Roma, onde o papa Adriano II aprovou os livros eslavos, ato de enorme alcance pastoral. Cirilo morreu em Roma em 869; Metódio, ordenado arcebispo, prosseguiu a missão até 885, enfrentando prisão e calúnias. João Paulo II os proclamou copadroeiros da Europa em 1980 na carta Egregiae virtutis.",
    virtudes: ["Inculturação do Evangelho", "Fidelidade a Roma", "Paciência nas perseguições"],
    frase: "Todo povo louve o Senhor na própria língua.",
  },
  {
    slug: "sao-patricio",
    nome: "São Patrício",
    titulo: "Apóstolo da Irlanda",
    data: "17/03",
    seculo: "V",
    padroeiro: "Irlanda, engenheiros, missionários",
    resumo: "Britano-romano escravizado na Irlanda que voltou como bispo e evangelizou a ilha.",
    biografia:
      "Nasceu na Britânia romana no fim do século IV, filho de um diácono. Aos dezesseis anos foi capturado por piratas e vendido como escravo na Irlanda, onde passou seis anos pastoreando e onde, como conta em sua Confissão — texto autobiográfico autêntico e comovente —, a oração se tornou o centro de sua vida. Fugiu, retornou à família, formou-se na Gália e foi ordenado; por volta de 432 voltou à ilha que o havia escravizado, agora como bispo. Evangelizou o norte e o centro da Irlanda, organizou dioceses e mosteiros, ordenou clero local, enfrentou reis e druidas e defendeu cristãos vítimas de tráfico, como na Carta a Coroticus. Sua obra deu à Irlanda uma Igreja monástica que, nos séculos seguintes, reevangelizaria boa parte da Europa. Morreu por volta de 461. A explicação do mistério trinitário pelo trevo é tradição posterior, não presente em seus escritos.",
    virtudes: ["Perdão dos algozes", "Zelo missionário", "Confiança na Providência"],
    frase: "Cristo à minha frente, Cristo atrás de mim, Cristo em cada coração que pensa em mim.",
  },
  {
    slug: "santa-maria-egipciaca",
    nome: "Santa Maria Egipcíaca",
    titulo: "Penitente do deserto",
    data: "01/04",
    seculo: "IV-V",
    padroeiro: "Penitentes, mulheres em conversão",
    resumo:
      "Antiga cortesã de Alexandria que viveu quase meio século de penitência no deserto da Transjordânia.",
    biografia:
      "A vida de Maria Egipcíaca chegou até nós pela biografia atribuída a São Sofrônio de Jerusalém, no século VII, texto de enorme influência na espiritualidade oriental e ocidental. Segundo esse relato — de caráter edificante, cujos detalhes os historiadores tratam com cautela —, Maria deixou a casa paterna aos doze anos e viveu de prostituição em Alexandria por dezessete anos. Embarcou com peregrinos para Jerusalém e, na porta da basílica do Santo Sepulcro, foi interiormente impedida de entrar; convertida diante de um ícone da Mãe de Deus, atravessou o Jordão e viveu quarenta e sete anos em solidão absoluta no deserto. Ali foi encontrada pelo monge Zósimo, que lhe deu a comunhão e recebeu sua história. A Igreja bizantina lhe consagra o quinto domingo da Quaresma, e a tradição latina a apresenta como ícone da misericórdia que alcança qualquer passado.",
    virtudes: ["Penitência perseverante", "Humildade", "Confiança na misericórdia"],
    frase: "Onde muito se pecou, ainda mais abundou a graça.",
  },
  {
    slug: "santa-zita",
    nome: "Santa Zita",
    titulo: "Serva doméstica de Lucca",
    data: "27/04",
    seculo: "XIII",
    padroeiro: "Empregadas domésticas, donas de casa, padeiros",
    resumo: "Empregada doméstica que santificou quarenta e oito anos de serviço na mesma casa.",
    biografia:
      "Nasceu em Monsagrati, perto de Lucca, em 1218, numa família pobre e devota. Aos doze anos entrou como criada na casa dos Fatinelli, mercadores de seda, e ali permaneceu até a morte, quarenta e oito anos depois. Enfrentou nos primeiros tempos a desconfiança dos patrões e a hostilidade dos outros criados, que a acusavam de perder tempo na igreja e de dar comida da casa aos pobres. Com constância silenciosa, transformou o trabalho em oração: levantava-se de madrugada para a Missa, cumpria integralmente as tarefas, jejuava e visitava presos e doentes de Lucca. Terminou os anos finais como governanta respeitada e conselheira da família. Morreu em 1272; o culto começou imediatamente entre o povo de Lucca e foi confirmado por Inocêncio XII em 1696. É a santa que a Igreja propõe como sinal de que nenhuma condição de trabalho é obstáculo à santidade.",
    virtudes: ["Fidelidade no trabalho", "Paciência", "Caridade escondida"],
    frase: "Uma serva não é devota se não é diligente.",
  },
  {
    slug: "sao-pacomio",
    nome: "São Pacômio",
    titulo: "Fundador do monaquismo cenobítico",
    data: "09/05",
    seculo: "IV",
    padroeiro: "Comunidades religiosas",
    resumo:
      "Soldado egípcio convertido pela caridade dos cristãos, criador da primeira regra monástica comunitária.",
    biografia:
      "Nasceu por volta de 292 na Tebaida, de família pagã. Recrutado à força para o exército imperial, foi encarcerado em Tebas e ficou marcado pela caridade dos cristãos que socorriam os presos; libertado, pediu o batismo. Depois de alguns anos junto ao eremita Palamon, teve a intuição decisiva que mudou a história da vida religiosa: em vez do isolamento absoluto dos anacoretas, reunir os monges em comunidade, sob um mesmo teto, com trabalho, refeições, oração comum e obediência a um superior. Em Tabennisi, por volta de 320, organizou o primeiro cenóbio e redigiu uma Regra prática — traduzida ao latim por São Jerônimo — que influenciou São Basílio e, através dele, São Bento. Ao morrer, em 346, seus mosteiros contavam milhares de monges e algumas comunidades femininas dirigidas por sua irmã. É reconhecido pelas Igrejas do Oriente e do Ocidente.",
    virtudes: ["Obediência", "Trabalho ordenado", "Sabedoria organizadora"],
    frase: "Ninguém se santifica sozinho: o irmão é o caminho.",
  },
  {
    slug: "sao-pancracio",
    nome: "São Pancrácio",
    titulo: "Mártir adolescente de Roma",
    data: "12/05",
    seculo: "IV",
    padroeiro: "Trabalho e saúde (devoção popular)",
    resumo: "Jovem mártir romano de culto antigo, sepultado na via Aurélia.",
    biografia:
      "O núcleo histórico do culto de Pancrácio é firme: um jovem mártir sepultado no cemitério da via Aurélia, em Roma, sobre cujo túmulo o papa Símaco ergueu, no início do século VI, a basílica de San Pancrazio, mencionada em documentos e itinerários antigos de peregrinos. Sua paixão, redigida séculos depois, é de gênero legendário: apresenta-o como adolescente frígio de família nobre, chegado a Roma com o tio, batizado pelo papa e decapitado aos catorze anos por recusar sacrificar aos deuses durante a perseguição de Diocleciano. A devoção popular, sobretudo na Itália, na Espanha e na América Latina, associou seu nome à saúde e ao trabalho, difundindo a imagem do santo com um ramo de salsa e uma moeda — costume folclórico sem fundamento litúrgico. A Igreja mantém a memória do mártir e não canoniza os elementos lendários acrescentados.",
    virtudes: ["Fé jovem", "Firmeza", "Desapego"],
    frase: "Adoro o Deus vivo e não sacrificarei a ídolos.",
  },
  {
    slug: "sao-leopoldo-mandic",
    nome: "São Leopoldo Mandić",
    titulo: "Confessor de Pádua",
    data: "12/05",
    seculo: "XIX-XX",
    padroeiro: "Confessores, doentes crônicos, unidade dos cristãos",
    resumo: "Capuchinho croata que passou quarenta anos no confessionário de Pádua.",
    biografia:
      "Bogdan Mandić nasceu em Castelnuovo, na Dalmácia, em 1866, o último de doze filhos. Entrou entre os capuchinhos e recebeu o nome de Leopoldo. Baixo, franzino, com dificuldades de fala e artrite severa, viu frustrado seu sonho de ser missionário no Oriente pela reunificação com os ortodoxos; compreendeu então que sua missão seria o confessionário. Em Pádua, de 1909 até a morte, atendeu penitentes de dez a treze horas por dia, célebre pela brevidade das penitências e pela paciência com quem voltava sempre com as mesmas fraquezas — quando censurado por excesso de benignidade, respondia que dava ao Senhor a satisfação que lhe faltasse. Morreu de câncer em 1942; a cela e o confessionário ficaram intactos após os bombardeios que destruíram parte do convento. Foi canonizado por João Paulo II em 1983 e sua urna foi levada a Roma no Jubileu da Misericórdia.",
    virtudes: ["Misericórdia paciente", "Humildade", "Desejo da unidade dos cristãos"],
    frase: "Sê generoso: o Senhor conhece o que falta e eu me encarrego do resto.",
  },
  {
    slug: "sao-joao-nepomuceno",
    nome: "São João Nepomuceno",
    titulo: "Mártir do sigilo da confissão",
    data: "16/05",
    seculo: "XIV",
    padroeiro: "Confessores, boa fama, Boêmia",
    resumo:
      "Vigário-geral de Praga afogado no Vltava em 1393, venerado como mártir do segredo sacramental.",
    biografia:
      "Nasceu em Nepomuk, na Boêmia, por volta de 1345, estudou direito canônico em Praga e Pádua e chegou a vigário-geral do arcebispo de Praga. Envolveu-se no violento conflito entre o arcebispo e o rei Venceslau IV a propósito da autonomia da Igreja e da nomeação de abades; preso, torturado e morto, foi lançado da ponte Carlos ao rio Vltava em 20 de março de 1393. O motivo político da execução é documentado; a tradição, difundida já no século XV, acrescenta que a hostilidade do rei nasceu da recusa de João em revelar o que a rainha lhe havia confessado. Foi canonizado em 1729 e tornou-se um dos santos mais representados da Europa central — sua estátua na ponte Carlos deu origem à difusão universal da imagem do santo com dedo nos lábios e coroa de cinco estrelas. Sua memória sustenta a inviolabilidade absoluta do sigilo confessional.",
    virtudes: ["Fidelidade ao sigilo", "Coragem diante do poder", "Zelo pela liberdade da Igreja"],
    frase: "O que se diz a Deus no confessionário jamais volta aos homens.",
  },
  {
    slug: "santa-maria-madalena-de-pazzi",
    nome: "Santa Maria Madalena de Pazzi",
    titulo: "Mística carmelita de Florença",
    data: "25/05",
    seculo: "XVI-XVII",
    padroeiro: "Doentes, reforma do clero",
    resumo:
      "Carmelita florentina de grandes êxtases e de intensa intercessão pela reforma da Igreja.",
    biografia:
      "Caterina de Pazzi nasceu em Florença em 1566, de família patrícia. Entrou no Carmelo de Santa Maria degli Angeli em 1582, tomando o nome de Maria Madalena. Sua vida religiosa foi marcada por longos períodos de êxtase, cujas palavras as irmãs anotavam — daí os volumes de seus Colóquios, que registram uma doutrina espiritual centrada no Verbo encarnado, no Espírito Santo e na Igreja. Alternou anos de grande consolação com cinco anos de prova interior severa, que ela chamava 'o leão', vividos em obediência e serviço comum: foi mestra de noviças, sacristã e enfermeira. Escreveu cartas à Cúria e a bispos pedindo a reforma dos costumes do clero, no espírito de Trento. Sofreu os últimos três anos com doença dolorosa, morrendo em 1607. Foi canonizada em 1669 e é venerada especialmente por sua devoção ao Espírito Santo.",
    virtudes: ["Amor à Igreja", "Obediência nas provações", "Devoção ao Espírito Santo"],
    frase: "Vem, Espírito Santo: une-te a nós, que sem ti nada podemos.",
  },
  {
    slug: "santo-agostinho-de-cantuaria",
    nome: "Santo Agostinho de Cantuária",
    titulo: "Apóstolo dos ingleses",
    data: "27/05",
    seculo: "VI-VII",
    padroeiro: "Inglaterra, missionários",
    resumo: "Monge romano enviado por São Gregório Magno para evangelizar os anglo-saxões.",
    biografia:
      "Era prior do mosteiro de Santo André, no monte Célio, em Roma, quando o papa São Gregório Magno o enviou, em 596, à frente de quarenta monges, à ilha da Britânia habitada pelos anglo-saxões pagãos. Desembarcou em Kent em 597 e foi recebido pelo rei Etelberto, casado com a princesa cristã Berta; o rei recebeu o batismo e concedeu liberdade de pregação. Agostinho estabeleceu a sé em Cantuária, recebeu o pálio de Gregório e organizou as primeiras dioceses inglesas, incluindo Londres e Rochester. A correspondência entre ele e Gregório, conservada por São Beda na História Eclesiástica do Povo Inglês, é um documento notável de prudência missionária: Gregório recomenda purificar e reaproveitar templos e costumes locais em vez de destruí-los. As negociações com os bispos britânicos autóctones sobre a data da Páscoa fracassaram. Morreu por volta de 604 em Cantuária.",
    virtudes: ["Obediência missionária", "Prudência pastoral", "Perseverança"],
    frase: "Não destruais os costumes bons: elevai-os a Deus.",
  },
  {
    slug: "santa-clotilde",
    nome: "Santa Clotilde",
    titulo: "Rainha dos francos",
    data: "04/06",
    seculo: "V-VI",
    padroeiro: "Esposas, mães, França",
    resumo: "Rainha católica cuja perseverança levou Clóvis e o povo franco ao batismo.",
    biografia:
      "Nasceu por volta de 474, princesa burgúndia educada na fé católica em meio a uma corte majoritariamente ariana. Casou-se cerca de 492 com Clóvis, rei dos francos, ainda pagão, e obteve dele que os filhos fossem batizados — a morte do primeiro logo após o batismo tornou-se motivo de censura, mas ela persistiu. Gregório de Tours narra que Clóvis, na batalha de Tolbiac, invocou o Deus de Clotilde e, vencedor, recebeu o batismo em Reims das mãos de São Remígio, com milhares de guerreiros: acontecimento decisivo, pois deu à Gália uma monarquia católica e não ariana. Viúva em 511, Clotilde retirou-se para Tours, junto ao túmulo de São Martinho, onde viveu em oração, penitência e amparo aos pobres, sofrendo com as guerras fratricidas entre os próprios filhos. Morreu em 545 e foi sepultada em Paris.",
    virtudes: ["Perseverança na oração", "Paciência no matrimônio", "Caridade régia"],
    frase: "O Deus dos cristãos é o único que salva; nele confia, e vencerás.",
  },
  {
    slug: "sao-norberto",
    nome: "São Norberto",
    titulo: "Fundador dos premonstratenses",
    data: "06/06",
    seculo: "XI-XII",
    padroeiro: "Reforma do clero, paz nas cidades",
    resumo: "Cônego renano convertido, reformador do clero e apóstolo da Eucaristia.",
    biografia:
      "Nasceu em Xanten, na Renânia, por volta de 1080, de família nobre; recebeu benefícios eclesiásticos e viveu na corte imperial sem verdadeira conversão. Um raio que o derrubou do cavalo, por volta de 1115, marcou a virada: ordenou-se sacerdote, renunciou às rendas e tornou-se pregador itinerante, autorizado pelo papa Gelásio II. Em 1121, no vale de Prémontré, fundou com treze companheiros uma comunidade de cônegos regulares sob a Regra de Santo Agostinho, unindo vida contemplativa e ministério pastoral — a Ordem Premonstratense. Combateu com energia o cisma provocado pelas ordenações simoníacas e as heresias que negavam a presença real, tornando-se conhecido pela sua defesa da Eucaristia, motivo pelo qual é representado com o ostensório. Arcebispo de Magdeburgo a partir de 1126, reformou o clero em meio a forte resistência e trabalhou pela paz no Império. Morreu em 1134.",
    virtudes: ["Zelo pela Eucaristia", "Pobreza voluntária", "Firmeza reformadora"],
    frase: "Ó sacramento admirável, no qual Cristo verdadeiramente permanece.",
  },
  {
    slug: "sao-marcelino-champagnat",
    nome: "São Marcelino Champagnat",
    titulo: "Fundador dos Irmãos Maristas",
    data: "06/06",
    seculo: "XIX",
    padroeiro: "Educadores, professores de crianças pobres",
    resumo:
      "Sacerdote francês que criou uma congregação de irmãos para educar as crianças do campo.",
    biografia:
      "Nasceu em Marlhes, na França, em 1789, filho de camponeses, num tempo em que a Revolução havia desmantelado o ensino religioso. Entrou tarde no seminário e teve dificuldades escolares, o que marcaria sua sensibilidade pedagógica. Ordenado em 1816, foi vigário em La Valla, região rural pobre; chamado a atender um adolescente moribundo que ignorava o essencial da fé, decidiu fundar naquele mesmo ano uma congregação de irmãos leigos dedicada à educação dos meninos do campo: os Pequenos Irmãos de Maria, ou Maristas. Formou os primeiros irmãos ele próprio, construiu com as próprias mãos a casa de L'Hermitage e desenvolveu um método educativo baseado na presença simples entre os alunos, no afeto e na formação da consciência. Morreu em 1840, deixando quase trezentos irmãos; foi canonizado por João Paulo II em 1999.",
    virtudes: ["Amor educativo", "Confiança em Maria", "Trabalho perseverante"],
    frase: "Para educar bem as crianças, é preciso primeiro amá-las, e amá-las todas igualmente.",
  },
  {
    slug: "sao-romualdo",
    nome: "São Romualdo",
    titulo: "Fundador dos camaldulenses",
    data: "19/06",
    seculo: "X-XI",
    padroeiro: "Eremitas, vida contemplativa",
    resumo: "Nobre de Ravena que uniu vida eremítica e cenobítica na Congregação de Camáldoli.",
    biografia:
      "Nasceu em Ravena por volta de 951, de família ducal. Testemunha de um duelo mortal em que seu pai matou um parente, retirou-se por penitência ao mosteiro de Sant'Apollinare in Classe e ali se fez monge. Buscando maior rigor, viveu como eremita junto ao mestre Marino e no mosteiro catalão de Cuxá. Voltou à Itália e passou quarenta anos percorrendo os Apeninos, fundando e reformando mosteiros, atendendo ao pedido do imperador Otão III e de bispos que buscavam corrigir a decadência monástica. Sua obra mais duradoura nasceu por volta de 1023 em Camáldoli, na Toscana: um conjunto de celas eremíticas ligado a um mosteiro comunitário, permitindo que a mesma família religiosa viva a solidão e a fraternidade. O único texto que dele se conserva, a Breve Regra, resume seu ensino: 'senta-te na cela como no paraíso'. Morreu em 1027.",
    virtudes: ["Silêncio", "Penitência", "Discernimento"],
    frase: "Senta-te na cela como no paraíso e vive só para Deus.",
  },
  {
    slug: "sao-joao-fisher",
    nome: "São João Fisher",
    titulo: "Bispo e mártir da unidade da Igreja",
    data: "22/06",
    seculo: "XV-XVI",
    padroeiro: "Bispos, teólogos",
    resumo: "Único bispo inglês a resistir ao cisma de Henrique VIII, decapitado em 1535.",
    biografia:
      "Nasceu em Beverley, Inglaterra, em 1469. Teólogo de primeira ordem, foi chanceler da Universidade de Cambridge, promotor dos estudos bíblicos e do humanismo cristão, amigo de Erasmo e de Thomas More, e bispo de Rochester, a mais pobre das dioceses inglesas, que nunca quis trocar por outra mais rica. Escreveu contra Lutero e pela reforma real dos costumes do clero. Quando Henrique VIII quis anular seu matrimônio com Catarina de Aragão e declarar-se chefe supremo da Igreja na Inglaterra, Fisher foi o único bispo do reino a opor-se publicamente, defendendo a validade do matrimônio e o primado do Papa. Preso na Torre de Londres, foi criado cardeal por Paulo III durante o cárcere — o que apressou sua condenação — e decapitado em 22 de junho de 1535, quinze dias antes de Thomas More. Foi canonizado em 1935.",
    virtudes: ["Fidelidade doutrinal", "Pobreza episcopal", "Coragem até a morte"],
    frase: "Morro pela fé da santa Igreja Católica de Cristo.",
  },
  {
    slug: "santa-veronica",
    nome: "Santa Verônica",
    titulo: "A mulher do véu, na tradição da Via-Sacra",
    data: "12/07",
    seculo: "I (tradição)",
    padroeiro: "Lavadeiras, fotógrafos, tecelões",
    resumo: "Figura da tradição piedosa que enxugou o rosto de Cristo no caminho do Calvário.",
    biografia:
      "Verônica não aparece nos Evangelhos canônicos. A figura nasce da tradição piedosa medieval ligada à relíquia romana do Sudário — a imagem do rosto de Cristo venerada em São Pedro — e o próprio nome parece derivar da expressão vera icon, 'verdadeira imagem'. Textos apócrifos tardios, como os Atos de Pilatos em algumas recensões, identificam-na com a hemorroíssa curada por Jesus (Mc 5, 25-34), e escritos medievais desenvolveram a narrativa de que, no caminho para o Calvário, uma mulher rompeu o cordão dos soldados para enxugar o rosto do Senhor, recebendo nele impressa a sua face. A cena é a sexta estação da Via-Sacra, exercício de piedade aprovado pela Igreja, embora sem valor histórico assegurado. A tradição a conservou como imagem da compaixão corajosa e do rosto de Cristo reconhecido no sofredor.",
    virtudes: ["Compaixão corajosa", "Ternura", "Fidelidade nos momentos de abandono"],
    frase: "Mostra-nos, Senhor, o teu rosto, e seremos salvos.",
  },
  {
    slug: "sao-luis-martin",
    nome: "São Luís Martin",
    titulo: "Pai de Santa Teresinha",
    data: "12/07",
    seculo: "XIX",
    padroeiro: "Pais de família, doentes mentais",
    resumo: "Relojoeiro de Alençon, canonizado com sua esposa Zélia como casal santo.",
    biografia:
      "Nasceu em Bordéus em 1823 e estabeleceu-se como relojoeiro em Alençon. Tentara a vida monástica no Grande São Bernardo, mas foi impedido pela falta de latim; casou-se em 1858 com Zélia Guérin, rendeira de Alençon, e tiveram nove filhos, quatro dos quais morreram na infância — as cinco filhas sobreviventes tornaram-se religiosas, entre elas Santa Teresa do Menino Jesus. A vida da família era marcada pela Missa diária, pelo respeito ao domingo, pela justiça no comércio e pela caridade discreta com pobres e trabalhadores. Viúvo em 1877, mudou-se para Lisieux e acompanhou uma a uma as vocações das filhas. Nos últimos anos sofreu arteriosclerose cerebral com crises psíquicas e passou três anos internado em Caen, humilhação que aceitou em silêncio. Morreu em 1894. Ele e Zélia foram canonizados juntos por Francisco em 2015, durante o Sínodo sobre a família.",
    virtudes: ["Paternidade generosa", "Honestidade profissional", "Aceitação da provação"],
    frase: "Meu Deus, eu vos agradeço: sois vós que me dais esta prova.",
  },
  {
    slug: "santa-joana-francisca-de-chantal",
    nome: "Santa Joana Francisca de Chantal",
    titulo: "Cofundadora da Visitação",
    data: "12/08",
    seculo: "XVI-XVII",
    padroeiro: "Viúvas, mães, esquecidos",
    resumo: "Baronesa viúva que, com São Francisco de Sales, criou a Ordem da Visitação.",
    biografia:
      "Jeanne-Françoise Frémyot nasceu em Dijon em 1572. Casou-se com o barão de Chantal, teve seis filhos e ficou viúva aos vinte e oito anos, quando o marido morreu num acidente de caça; perdoou publicamente o autor do tiro e chegou a ser madrinha de seu filho. Em 1604 conheceu Francisco de Sales, que se tornou seu diretor espiritual e com quem manteve uma das correspondências espirituais mais importantes da história cristã. Tendo assegurado a educação e a herança dos filhos, fundou com ele em Annecy, em 1610, a Ordem da Visitação de Santa Maria, pensada para acolher também viúvas e mulheres de saúde frágil, excluídas das ordens de rigor severo — inovação que a resistência da época obrigou a adaptar à clausura. Governou a expansão de oitenta e sete mosteiros, atravessou lutos sucessivos e longas noites interiores. Morreu em 1641 e foi canonizada em 1767.",
    virtudes: ["Perdão", "Fortaleza na viuvez", "Docilidade espiritual"],
    frase: "Nada peçais, nada recuseis, e vivereis em paz.",
  },
  {
    slug: "sao-estevao-da-hungria",
    nome: "São Estêvão da Hungria",
    titulo: "Primeiro rei cristão dos húngaros",
    data: "16/08",
    seculo: "X-XI",
    padroeiro: "Hungria, reis, canteiros",
    resumo: "Rei que organizou a Igreja e o Estado húngaros e consagrou o reino à Virgem.",
    biografia:
      "Vajk, batizado Estêvão, nasceu por volta de 969, filho do príncipe magiar Géza. Sucedeu ao pai em 997 e recebeu do papa Silvestre II a coroa real, sendo coroado no Natal de 1000 ou no início de 1001 — ato que integrou a Hungria ao cristianismo latino. Fundou dez dioceses, entre elas a arquidiocese de Esztergom, ergueu mosteiros beneditinos, legislou em favor da liberdade da Igreja e do amparo aos pobres e estrangeiros, e enviou hospedarias para peregrinos em Roma, Jerusalém e Constantinopla. Suas Exortações ao filho Emérico são um dos primeiros documentos políticos cristãos da Europa central. Sofreu revoltas pagãs e a morte prematura do filho e herdeiro. Segundo a tradição húngara, ao morrer, em 1038, ofereceu a coroa à Virgem Maria, de onde nasceu o título de Regnum Marianum. Foi canonizado em 1083.",
    virtudes: ["Justiça no governo", "Zelo pela Igreja", "Hospitalidade"],
    frase: "Sê hospitaleiro com os estrangeiros: eles enriquecem o reino.",
  },
  {
    slug: "santa-rosalia",
    nome: "Santa Rosália",
    titulo: "Eremita e padroeira de Palermo",
    data: "04/09",
    seculo: "XII",
    padroeiro: "Palermo, doentes em epidemias",
    resumo: "Nobre normanda que viveu como eremita no monte Pellegrino, invocada na peste de 1624.",
    biografia:
      "Rosália viveu no século XII em Palermo, na corte normanda de Rogério II, segundo a tradição descendente de família nobre. Renunciou ao casamento e retirou-se para uma gruta, primeiro no monte Quisquina e depois no monte Pellegrino, onde viveu em solidão e penitência até a morte, por volta de 1166; inscrições encontradas nas grutas atestam sua presença. Seu culto permaneceu local e discreto até 1624, quando a peste devastou Palermo: relíquias identificadas como suas foram encontradas no monte Pellegrino e levadas em procissão pela cidade, e o fim da epidemia, que se seguiu, foi atribuído à sua intercessão. Desde então é padroeira principal de Palermo, celebrada com o festino de julho e a peregrinação de setembro. Sua história une o testemunho documentado de uma vida eremítica ao vínculo com a memória coletiva de uma cidade socorrida na desgraça.",
    virtudes: ["Vida escondida", "Penitência", "Intercessão pelo povo"],
    frase: "Por amor de meu Senhor Jesus Cristo, habitei nesta gruta.",
  },
  {
    slug: "sao-cosme-e-damiao",
    nome: "São Cosme e Damião",
    titulo: "Mártires médicos",
    data: "26/09",
    seculo: "III-IV",
    padroeiro: "Médicos, farmacêuticos, cirurgiões",
    resumo: "Irmãos médicos da Cilícia, martirizados por atender gratuitamente os doentes.",
    biografia:
      "Cosme e Damião foram irmãos médicos na Ásia Menor, martirizados provavelmente em Ciro, na Síria, durante a perseguição de Diocleciano, por volta de 303. Seu culto é antigo e sólido: a basílica erguida em Ciro sobre seu túmulo era grande centro de peregrinação no século V, e seus nomes entraram no Canon Romano da Missa, sinal de veneração romana precoce; em Roma, o papa Félix IV dedicou-lhes no século VI a basílica dos Santos Cosme e Damião no Fórum. A tradição lhes deu o epíteto grego de anárgiros, 'os que não cobram', porque exerciam a medicina sem receber pagamento, e narrativas hagiográficas tardias acrescentam curas prodigiosas. No Brasil, a devoção adquiriu forte caráter popular, associada à proteção das crianças e à distribuição de doces em 27 de setembro — costume cultural que a Igreja não confunde com o conteúdo litúrgico da memória dos mártires.",
    virtudes: ["Gratuidade no serviço", "Fraternidade", "Fidelidade até o martírio"],
    frase: "Recebemos de graça a arte de curar: de graça a damos.",
  },
  {
    slug: "sao-venceslau",
    nome: "São Venceslau",
    titulo: "Duque e mártir da Boêmia",
    data: "28/09",
    seculo: "X",
    padroeiro: "Chéquia, governantes cristãos",
    resumo: "Duque cristão da Boêmia assassinado pelo próprio irmão em 935.",
    biografia:
      "Nasceu por volta de 907, neto de Santa Ludmila, que o educou na fé cristã em uma Boêmia ainda dividida entre cristãos e pagãos. Assumiu o governo por volta de 922 e conduziu uma política de consolidação do cristianismo: fez construir a igreja de São Vito em Praga, favoreceu o clero e a liturgia, aplicou a justiça com clemência conhecida e buscou o entendimento com o rei Henrique I da Germânia, evitando guerras — o que a facção nobiliárquica pagã interpretou como submissão. Em 28 de setembro de 935, foi assassinado à porta da igreja de Stará Boleslav por partidários de seu irmão Boleslau, que lhe sucedeu. O povo o venerou imediatamente como mártir; seu túmulo na catedral de Praga tornou-se o centro simbólico da nação tcheca, e a coroa de São Venceslau, o sinal da soberania do reino. É considerado padroeiro celeste do povo tcheco.",
    virtudes: ["Justiça clemente", "Amor à paz", "Caridade com os pobres"],
    frase: "Antes servir a Cristo que reinar sem ele.",
  },
  {
    slug: "sao-rafael-arcanjo",
    nome: "São Rafael Arcanjo",
    titulo: "Medicina de Deus",
    data: "29/09",
    seculo: "—",
    padroeiro: "Viajantes, enfermos, noivos, farmacêuticos",
    resumo: "Um dos três arcanjos nomeados na Escritura, guia de Tobias e curador de Tobit.",
    biografia:
      "Rafael é nomeado no livro de Tobias, escrito deuterocanônico recebido pela Igreja Católica no canon das Escrituras. Seu nome significa 'Deus cura'. No relato, apresenta-se sob a aparência humana de Azarias e acompanha o jovem Tobias na viagem à Média, protege-o, negocia seu casamento com Sara, liberta-a do mal que a afligia e ensina a Tobias o modo de devolver a vista ao pai cego. Ao fim, revela sua identidade: 'Eu sou Rafael, um dos sete anjos que estão diante do Senhor' (Tb 12, 15), e ensina que ofereceu as orações da família a Deus, recomendando louvor, esmola e verdade. A tradição cristã ligou a ele os cuidados da saúde, o acompanhamento dos viajantes e o noivado cristão. Sua memória litúrgica é celebrada em 29 de setembro, junto com Miguel e Gabriel, na festa dos Santos Arcanjos.",
    virtudes: ["Companhia providente", "Cura", "Louvor a Deus"],
    frase: "Eu sou Rafael, um dos sete anjos que estão diante do Senhor.",
  },
  {
    slug: "sao-gabriel-arcanjo",
    nome: "São Gabriel Arcanjo",
    titulo: "Mensageiro da Encarnação",
    data: "29/09",
    seculo: "—",
    padroeiro: "Mensageiros, comunicações, rádio e imprensa",
    resumo: "Arcanjo que anunciou a Zacarias o Precursor e a Maria a Encarnação do Verbo.",
    biografia:
      "Gabriel — 'força de Deus' — é nomeado no livro de Daniel, onde interpreta visões e anuncia o tempo das setenta semanas (Dn 8-9), e no Evangelho de São Lucas, onde ocupa lugar central na história da salvação: anuncia a Zacarias, no templo, o nascimento de João Batista, e depois, em Nazaré, dirige-se a Maria com a saudação 'Ave, cheia de graça, o Senhor é contigo', propondo-lhe a maternidade divina e recebendo o consentimento que abre a Encarnação (Lc 1, 26-38). A tradição patrística viu nesse diálogo o momento em que a obediência de Maria desfaz a desobediência de Eva. A Igreja o celebra na festa dos Santos Arcanjos, em 29 de setembro, e retoma suas palavras diariamente na Ave-Maria e no Angelus. Pio XII o declarou padroeiro dos que trabalham nas telecomunicações, por ser o portador da mensagem por excelência.",
    virtudes: ["Fidelidade à missão", "Reverência", "Serviço à Palavra"],
    frase: "Ave, cheia de graça, o Senhor é contigo.",
  },
  {
    slug: "sao-bruno-de-colonia",
    nome: "São Bruno de Colônia",
    titulo: "Fundador dos cartuxos",
    data: "06/10",
    seculo: "XI",
    padroeiro: "Contemplativos, possessos, Calábria",
    resumo: "Mestre de teologia que deixou a fama acadêmica para fundar a Grande Cartuxa.",
    biografia:
      "Nasceu em Colônia por volta de 1030 e estudou em Reims, onde se tornou o mais célebre mestre de teologia de seu tempo — entre seus discípulos esteve o futuro papa Urbano II. Chanceler da diocese de Reims, opôs-se ao arcebispo simoníaco Manasses e teve de fugir. Essa crise amadureceu a decisão de deixar tudo: em 1084, com seis companheiros, recebeu do bispo São Hugo de Grenoble um vale alpino deserto, a Chartreuse, e ali estabeleceu uma vida de solidão em celas individuais, silêncio, ofício divino e trabalho manual, com breve convivência comunitária — o modelo cartusiano, jamais reformado porque, segundo o dito tradicional, nunca deformado. Chamado a Roma por Urbano II como conselheiro, recusou o arcebispado de Reggio e obteve permissão para retirar-se à Calábria, onde fundou nova cartuxa e morreu em 1101. Nunca foi canonizado por processo solene: seu culto foi reconhecido pela Igreja em 1514 e estendido em 1623.",
    virtudes: ["Silêncio", "Desapego da fama", "Amor à solidão com Deus"],
    frase: "Ó bondade, que só a solidão ensina a saborear.",
  },
  {
    slug: "sao-eduardo-o-confessor",
    nome: "São Eduardo, o Confessor",
    titulo: "Rei da Inglaterra",
    data: "13/10",
    seculo: "XI",
    padroeiro: "Reis, casamentos difíceis, Inglaterra",
    resumo: "Último rei anglo-saxão da dinastia de Wessex, construtor da abadia de Westminster.",
    biografia:
      "Nasceu por volta de 1003, filho do rei Etelredo, e passou a juventude exilado na Normandia durante o domínio dinamarquês. Coroado rei da Inglaterra em 1043, governou por vinte e três anos marcados por relativa paz interna, redução de tributos pesados, generosidade com os pobres e grande deferência à Igreja. Fundou e reconstruiu a abadia de Westminster, consagrada dias antes de sua morte, que se tornou o lugar de coroação e sepultura dos reis ingleses. A tradição lhe atribuiu o título de Confessor por sua piedade e por uma vida de continência no matrimônio com Edite — tema debatido pelos historiadores, que também discutem se a ausência de herdeiros resultou de motivos políticos. Sua morte, em janeiro de 1066, abriu a crise sucessória que levou à conquista normanda. Foi canonizado em 1161 por Alexandre III e permanece um dos padroeiros históricos da Inglaterra.",
    virtudes: ["Mansidão no governo", "Generosidade com os pobres", "Piedade litúrgica"],
    frase: "O rei é servo dos pobres de Cristo.",
  },
  {
    slug: "santa-edwiges",
    nome: "Santa Edwiges",
    titulo: "Duquesa da Silésia",
    data: "16/10",
    seculo: "XII-XIII",
    padroeiro: "Endividados, sem-teto, reconciliação familiar",
    resumo: "Duquesa que empregou os bens do ducado em hospitais e no socorro aos endividados.",
    biografia:
      "Nasceu por volta de 1174 na Baviera, filha do conde de Andechs, e casou-se muito jovem com Henrique I, duque da Silésia. Teve sete filhos, dos quais poucos sobreviveram. Como duquesa, exerceu influência decisiva na fundação de hospitais — entre eles o primeiro hospital para leprosos da região —, de mosteiros como a abadia cisterciense de Trzebnica, de escolas e de albergues, e ficou conhecida pelo atendimento pessoal a doentes e presos por dívidas, cuja libertação frequentemente pagava. Viveu com austeridade pessoal notável para sua posição, sustentou o marido no governo e mediou conflitos entre príncipes vizinhos. Viúva em 1238, retirou-se para Trzebnica sem tomar o hábito, servindo as religiosas. Sofreu a morte do filho Henrique II na batalha de Legnica contra os mongóis, em 1241. Morreu em 1243 e foi canonizada em 1267. No Brasil, é largamente invocada pelos endividados.",
    virtudes: ["Misericórdia com os devedores", "Austeridade", "Fortaleza no luto"],
    frase: "Quem nada dá aos pobres nada terá guardado no céu.",
  },
  {
    slug: "sao-isaac-jogues",
    nome: "São Isaac Jogues",
    titulo: "Mártir jesuíta da América do Norte",
    data: "19/10",
    seculo: "XVII",
    padroeiro: "América do Norte, missionários",
    resumo:
      "Jesuíta francês martirizado entre os iroqueses em 1646, após anos de cativeiro e mutilação.",
    biografia:
      "Nasceu em Orléans, França, em 1607, entrou na Companhia de Jesus e partiu em 1636 para a Nova França, onde trabalhou entre os hurons na região dos Grandes Lagos, aprendendo suas línguas. Em 1642 foi capturado por uma partida mohawk, torturado com mutilação de dedos e mantido como escravo por mais de um ano; escapou com ajuda de comerciantes holandeses e regressou à Europa, onde obteve de Urbano VIII dispensa para celebrar a Missa apesar das mãos mutiladas. Voltou voluntariamente à missão e, em 1646, participou de uma embaixada de paz junto aos mohawks; acusado de trazer doença e má colheita, foi morto a golpes de machado em Ossernenon, na atual Nova York, com seu companheiro leigo João de La Lande. Seus escritos, publicados nas Relações dos Jesuítas, são fonte histórica importante. Foi canonizado em 1930 com os demais Mártires Canadenses.",
    virtudes: ["Coragem missionária", "Perdão aos torturadores", "Perseverança"],
    frase: "Meu Deus, não vou embora enquanto houver almas a evangelizar.",
  },
  {
    slug: "santa-ursula",
    nome: "Santa Úrsula",
    titulo: "Virgem e mártir de Colônia",
    data: "21/10",
    seculo: "IV (tradição)",
    padroeiro: "Educadoras, estudantes, Colônia",
    resumo:
      "Mártir venerada em Colônia desde a antiguidade, cuja lenda medieval difundiu o culto pela Europa.",
    biografia:
      "O núcleo verificável do culto de Úrsula é uma inscrição do século IV ou V, na igreja de Santa Úrsula em Colônia, que registra a restauração de uma basílica erguida no lugar do martírio de virgens cristãs, sem citar nomes nem número. A partir do século IX, textos hagiográficos desenvolveram a narrativa de Úrsula, princesa britânica que, viajando em peregrinação com um cortejo de companheiras, foi massacrada com elas em Colônia pelos hunos; um erro de leitura de abreviaturas medievais transformou o número em onze mil virgens, cifra que a crítica histórica há muito reconhece como lendária. A Igreja manteve a memória das mártires de Colônia e retirou a festa do calendário universal em 1969, conservando-a como memória local e nas famílias religiosas. Santa Ângela de Mérici escolheu Úrsula como patrona das Ursulinas, ligando seu nome à educação cristã.",
    virtudes: ["Virgindade consagrada", "Fidelidade", "Testemunho comunitário"],
    frase: "Antes morrer que negar a Cristo.",
  },
  {
    slug: "santa-margarida-da-escocia",
    nome: "Santa Margarida da Escócia",
    titulo: "Rainha e mãe dos pobres",
    data: "16/11",
    seculo: "XI",
    padroeiro: "Escócia, mães, viúvas",
    resumo: "Rainha que reformou a Igreja escocesa e serviu pessoalmente os pobres de Dunfermline.",
    biografia:
      "Nasceu por volta de 1045 na Hungria, onde sua família anglo-saxã vivia exilada, e foi educada na piedade e nas letras. Após a conquista normanda de 1066, refugiou-se na Escócia e casou-se com o rei Malcolm III, homem rude que ela influenciou profundamente. Teve oito filhos, entre eles David I e a rainha Matilde da Inglaterra, e cuidou pessoalmente de sua educação cristã. Como rainha, promoveu sínodos que corrigiram abusos e uniformizaram a disciplina da Igreja escocesa — observância da Quaresma, comunhão pascal, descanso dominical —, restaurou a abadia de Iona, fundou a abadia de Dunfermline e criou albergues para peregrinos. Servia diariamente órfãos e pobres à mesa, resgatava cativos ingleses e jejuava com rigor. Morreu em 1093, dias depois do marido e do filho mortos em batalha. Foi canonizada em 1250 e proclamada padroeira da Escócia.",
    virtudes: [
      "Caridade pessoal com os pobres",
      "Zelo pela reforma da Igreja",
      "Educação dos filhos",
    ],
    frase: "Servi a Cristo nos pobres, que estão sempre à nossa porta.",
  },
  {
    slug: "santa-catarina-de-alexandria",
    nome: "Santa Catarina de Alexandria",
    titulo: "Virgem e mártir, patrona dos filósofos",
    data: "25/11",
    seculo: "IV (tradição)",
    padroeiro: "Filósofos, estudantes, professores, advogados",
    resumo: "Mártir alexandrina da tradição antiga, símbolo da fé que dialoga com a razão.",
    biografia:
      "A veneração de Catarina é antiga e ampla no Oriente e no Ocidente, ligada ao mosteiro do Sinai, que desde o século IX guarda suas relíquias; os relatos de sua paixão, porém, são tardios e de gênero hagiográfico, e a crítica histórica não pode reconstituir os fatos com segurança. A tradição a apresenta como jovem alexandrina de família nobre e formação filosófica que, na perseguição de Maximino, enfrentou publicamente o imperador, refutou cinquenta filósofos convocados para convencê-la — vários dos quais se converteram — e foi condenada à roda dentada, que se partiu, sendo por fim decapitada. Foi uma das santas mais populares da Idade Média, incluída entre os catorze santos auxiliares e nomeada entre as vozes ouvidas por Santa Joana d'Arc. Sua festa saiu do calendário universal em 1969 e foi reintroduzida como memória facultativa em 2002. Permanece patrona dos que estudam e ensinam.",
    virtudes: ["Inteligência ao serviço da fé", "Firmeza", "Pureza"],
    frase: "A razão bem usada não afasta de Cristo: conduz a ele.",
  },
  {
    slug: "sao-columbano",
    nome: "São Columbano",
    titulo: "Monge irlandês, missionário da Europa",
    data: "23/11",
    seculo: "VI-VII",
    padroeiro: "Missionários itinerantes, motociclistas, Irlanda",
    resumo: "Abade irlandês que fundou mosteiros na Gália e na Itália e formou a Europa cristã.",
    biografia:
      "Nasceu na Irlanda por volta de 543 e formou-se no mosteiro de Bangor. Por volta de 590, com doze companheiros, partiu em peregrinatio pro Christo — o exílio voluntário próprio do monaquismo irlandês — e chegou à Gália merovíngia, então em decadência religiosa. Fundou os mosteiros de Annegray, Luxeuil e Fontaines, que atraíram multidões de vocações e formaram gerações de monges e bispos. Sua Regra, austera, e a prática irlandesa da confissão privada e reiterada tiveram influência duradoura na disciplina penitencial da Igreja. Entrou em conflito com bispos francos pela data da Páscoa e com a rainha Brunilda ao censurar a moral da corte; expulso, atravessou a Europa pregando, passou por Bregenz e pelos Alpes e fundou em 614 a abadia de Bobbio, na Itália, onde morreu em 615. Bento XVI o chamou um dos pais da Europa; deixou cartas, sermões e poemas.",
    virtudes: ["Austeridade", "Liberdade profética", "Zelo evangelizador"],
    frase: "Somos peregrinos do mundo: nossa pátria é o céu.",
  },
  {
    slug: "sao-edmundo-campion",
    nome: "São Edmundo Campion",
    titulo: "Jesuíta mártir da Inglaterra",
    data: "01/12",
    seculo: "XVI",
    padroeiro: "Perseguidos pela fé, professores",
    resumo: "Orador de Oxford convertido ao catolicismo, executado em Tyburn em 1581.",
    biografia:
      "Nasceu em Londres em 1540 e tornou-se um dos mais brilhantes oradores de Oxford, admirado pela rainha Isabel I. A consciência levou-o a romper com a Igreja da Inglaterra: passou à Irlanda, depois a Douai e a Roma, onde entrou na Companhia de Jesus em 1573. Em 1580 voltou clandestinamente à Inglaterra, onde o ministério católico era crime de traição, e percorreu o país celebrando a Missa em casas particulares e confortando famílias perseguidas. Escreveu o Decem rationes, dez argumentos em defesa da fé católica, impresso em prensa oculta e distribuído dentro da própria universidade de Oxford, o que provocou enorme escândalo. Preso em 1581, foi torturado no cavalete, submetido a disputas públicas e condenado por conspiração — acusação que sempre negou, declarando-se leal à rainha e fiel ao Papa em matéria de fé. Foi enforcado e esquartejado em Tyburn; canonizado em 1970 entre os Quarenta Mártires.",
    virtudes: ["Coragem intelectual", "Lealdade à fé", "Serenidade no suplício"],
    frase: "Morremos rezando pela rainha e pela Inglaterra.",
  },
  {
    slug: "santa-barbara",
    nome: "Santa Bárbara",
    titulo: "Virgem e mártir",
    data: "04/12",
    seculo: "III-IV (tradição)",
    padroeiro: "Artilheiros, mineiros, bombeiros; invocada nas tempestades",
    resumo: "Mártir de culto antigo no Oriente, invocada contra tempestades e mortes súbitas.",
    biografia:
      "O culto de Bárbara está documentado no Oriente desde o século VII e difundiu-se amplamente no Ocidente medieval, mas sua paixão é um texto tardio, de valor devocional e não histórico; por isso a reforma litúrgica de 1969 retirou sua festa do calendário romano geral, conservando-se a memória em calendários locais e na piedade popular. A tradição a apresenta como filha de um pagão rico de Nicomédia que a manteve numa torre para preservá-la do cristianismo; batizada em segredo, teria mandado abrir uma terceira janela na torre em honra da Trindade, sendo por isso denunciada e por fim decapitada pelo próprio pai, morto em seguida por um raio. Desse último detalhe nasceu sua invocação contra tempestades, explosões e morte súbita, que a tornou patrona de mineiros, artilheiros e bombeiros. A Igreja venera a mártir sem endossar os episódios lendários.",
    virtudes: ["Fé firme na provação", "Coragem", "Confissão da Trindade"],
    frase: "Creio no Pai, no Filho e no Espírito Santo: um só Deus.",
  },
  {
    slug: "santa-filomena",
    nome: "Santa Filomena",
    titulo: "Virgem e mártir das catacumbas",
    data: "11/08",
    seculo: "III-IV (tradição)",
    padroeiro: "Jovens, causas desesperadas, sacerdotes",
    resumo: "Mártir descoberta nas catacumbas de Priscila em 1802, muito venerada no século XIX.",
    biografia:
      "Em 1802 foi encontrado nas catacumbas de Priscila, em Roma, um sepulcro com a inscrição fragmentada 'LUMENA PAXTE CUM FI', lida como Filumena pax tecum, e vestígios interpretados como sinais de martírio. As relíquias foram transferidas em 1805 para Mugnano del Cardinale, na Itália, onde o culto se difundiu com rapidez extraordinária. Não há documento antigo sobre sua vida, e os relatos difundidos no século XIX provêm de revelações privadas, que a Igreja nunca declarou históricas; estudos epigráficos posteriores levantaram sérias dúvidas sobre a leitura da inscrição e até sobre a atribuição do túmulo. A devoção, porém, foi acolhida por santos de peso — São João Maria Vianney, o Cura d'Ars, atribuiu-lhe curas e ergueu-lhe capela; Santa Madalena Sofia Barat e o beato Pio IX a promoveram. Em 1961 a Santa Sé retirou sua festa do calendário, mantendo-se o culto local e privado.",
    virtudes: ["Pureza", "Confiança", "Intercessão pelos desanimados"],
    frase: "Pequena santa, obtém-nos o que os homens julgam impossível.",
  },
  {
    slug: "sao-tomas-becket",
    nome: "São Tomás Becket",
    titulo: "Arcebispo de Cantuária e mártir",
    data: "29/12",
    seculo: "XII",
    padroeiro: "Clero secular, liberdade da Igreja",
    resumo:
      "Chanceler de Henrique II que, feito arcebispo, morreu defendendo a liberdade da Igreja.",
    biografia:
      "Nasceu em Londres em 1118 ou 1120, filho de mercadores normandos. Estudou direito, entrou no serviço do arcebispo Teobaldo e tornou-se chanceler do rei Henrique II, com quem manteve uma amizade estreita e um estilo de vida mundano. Nomeado arcebispo de Cantuária em 1162 por vontade do rei, que esperava um aliado, converteu-se profundamente à responsabilidade do ofício: adotou vida austera e passou a defender os direitos da Igreja contra as Constituições de Clarendon, que submetiam o clero à jurisdição régia. O conflito o levou a seis anos de exílio na França. De volta a Cantuária, foi assassinado na própria catedral, em 29 de dezembro de 1170, por quatro cavaleiros que interpretaram como ordem as palavras iradas do rei. O choque foi imenso na Europa; foi canonizado em 1173, e Henrique II fez penitência pública em seu túmulo, que se tornou o maior santuário de peregrinação da Inglaterra medieval.",
    virtudes: ["Conversão profunda", "Fortaleza episcopal", "Fidelidade à Igreja"],
    frase: "Estou pronto a morrer pelo meu Senhor, para que na Igreja haja liberdade e paz.",
  },
  {
    slug: "sao-simeao-estilita",
    nome: "São Simeão Estilita",
    titulo: "Asceta da coluna",
    data: "05/01",
    seculo: "V",
    padroeiro: "Penitentes, pastores",
    resumo: "Monge sírio que viveu décadas sobre uma coluna, conselheiro de povos e imperadores.",
    biografia:
      "Nasceu por volta de 390 na Síria, filho de pastores, e entrou muito jovem na vida monástica, onde seu rigor extremo levou a comunidade a dispensá-lo. Buscando maior solidão sem escapar às multidões que o procuravam, adotou por volta de 423, perto de Antioquia, uma forma inédita de ascese: viver permanentemente sobre uma coluna, cuja altura foi aumentando ao longo dos anos. Ali passou cerca de trinta e sete anos, exposto ao sol e ao frio, rezando, pregando duas vezes ao dia, arbitrando disputas, convertendo beduínos e correspondendo-se com bispos e com os imperadores Teodósio II e Leão I. Sua vida é atestada por fontes contemporâneas de primeira ordem, entre elas Teodoreto de Ciro, que o visitou. Sustentou a fé calcedoniana. Morreu em 459, e sobre sua coluna ergueu-se o grande santuário de Qalaat Semaan. Inaugurou a linhagem dos estilitas do Oriente cristão.",
    virtudes: ["Penitência extrema", "Oração contínua", "Serviço aos que o procuravam"],
    frase: "Sede pacientes uns com os outros e temei o Senhor.",
  },
  {
    slug: "sao-jose-de-cupertino",
    nome: "São José de Cupertino",
    titulo: "O santo dos êxtases",
    data: "18/09",
    seculo: "XVII",
    padroeiro: "Estudantes em exames, aviadores, pessoas com dificuldades de aprendizagem",
    resumo: "Franciscano de origem pobríssima, célebre pelos êxtases e invocado por quem estuda.",
    biografia:
      "Giuseppe Desa nasceu em 1603 em Cupertino, no reino de Nápoles, numa família tão pobre que sua mãe deu à luz num estábulo. Menino distraído e de aprendizado difícil, foi recusado por conventos e admitido primeiro como irmão leigo entre os conventuais; acolhido depois pelos frades menores, chegou com muito esforço ao sacerdócio em 1628 — segundo o relato tradicional, o bispo examinador interrompeu o exame satisfeito com a primeira resposta, e daí nasceu sua invocação pelos estudantes. Sua vida religiosa foi marcada por êxtases frequentes e por fenômenos de levitação, relatados por numerosas testemunhas e examinados pela Inquisição, que preferiu transferi-lo repetidas vezes e mantê-lo afastado do público. Viveu em obediência silenciosa a esses isolamentos, dedicado à confissão e à humildade mais simples. Morreu em Osimo em 1663 e foi canonizado em 1767.",
    virtudes: ["Humildade", "Obediência", "Simplicidade de coração"],
    frase: "Deixai que Deus vos leve: basta não resistir.",
  },
];

export const SANTOS: Santo[] = [...SANTOS_NUCLEO, ...SANTOS_ADICIONAIS];


export function getSanto(slug: string) {
  return SANTOS.find((s) => s.slug === slug);
}
