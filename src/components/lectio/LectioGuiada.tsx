...
import { Check, Clock3, Download, Pause, Play, Printer, RotateCcw, ShieldCheck } from "lucide-react";
...
const ETAPAS: { id: EtapaId; titulo: string; pergunta: string; instrucao: string }[] = [
  {
    ...
    instrucao: "Leia devagar, mais de uma vez. Observe pessoas, verbos, repetições e o contexto; evite começar por suas impressões.",
  },
  {
    id: "meditatio",
    titulo: "Meditatio — meditação",
    pergunta: "O que Deus me diz por este texto?",
    instrucao: "Confronte a passagem com sua vida à luz da fé da Igreja. Registre uma palavra, um chamado ou uma verdade a acolher.",
  },
  {
    id: "oratio",
    titulo: "Oratio — oração",
    pergunta: "O que respondo a Deus?",
    instrucao: "Transforme a Palavra em louvor, súplica, contrição ou ação de graças. Fale com simplicidade e verdade.",
  },
  {
    id: "contemplatio",
    titulo: "Contemplatio — contemplação",
    pergunta: "Como permaneço com Deus e levo a Palavra à vida?",
    instrucao: "Silencie. Não procure produzir ideias. Termine escolhendo uma resolução concreta, pequena e possível.",
  },
];
...
export function LectioGuiada() {
  ...
  const salvar = useCallback((novo: Registro) => {
    setRegistro(novo);
    try { window.localStorage.setItem(CHAVE, JSON.stringify(novo)); } catch { /* privado e local */ }
  }, []);
    ...
    <div className="mt-10 space-y-8">
      <section className="surface-card p-5 sm:p-7" data-sem-sumario>
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="kicker flex items-center gap-2"><Clock3 className="size-3.5" /> Cronômetro opcional</p>
            <p className="mt-2 font-mono text-3xl tabular-nums text-foreground" aria-live="polite">{tempo}</p>
          </div>
          <div className="action-tray">
            <Botao type="button" tamanho="md" variante="contorno" onClick={() => setRodando((valor) => !valor)}>
              {rodando ? <Pause className="size-4" /> : <Play className="size-4" />}
              {rodando ? "Pausar" : "Iniciar"}
            </Botao>
            <Botao type="button" tamanho="md" variante="discreto" onClick={() => { setRodando(false); setSegundos(0); }}>
      ...
      <nav aria-label="Etapas da Lectio Divina" className="grid gap-2 sm:grid-cols-4 print:hidden">
        {ETAPAS.map((item, indice) => (
          ...
          >
            <span className="mr-2 text-gold/60">0{indice + 1}</span>{item.titulo.split(" — ")[0]}
            {registro[item.id].trim() ? <Check className="ml-2 inline size-3.5" aria-label="Preenchida" /> : null}
          </button>
        ))}
      </nav>
      ...
      <div className="space-y-7">
        {ETAPAS.map((item, indice) => (
          ...
          >
            ...
            <label htmlFor={`nota-${item.id}`} className="mt-6 block text-step--1 font-medium text-foreground">{item.pergunta}</label>
            <textarea
              id={`nota-${item.id}`}
              value={registro[item.id]}
              onChange={(evento) => salvar({ ...registro, [item.id]: evento.target.value.slice(0, 6000) })}
              rows={8}
            ...
            <div className="mt-5 flex justify-between gap-3 print:hidden">
              <Botao type="button" variante="discreto" tamanho="sm" disabled={indice === 0} onClick={() => setEtapa(indice - 1)}>Anterior</Botao>
              <Botao type="button" variante="ouro" tamanho="sm" disabled={indice === ETAPAS.length - 1} onClick={() => setEtapa(indice + 1)}>Próxima etapa</Botao>
            </div>
          </section>
        ))}
      </div>
      ...
      <aside className="surface-card border-gold/25 p-5 sm:p-7" data-sem-sumario>
        <p className="kicker flex items-center gap-2"><ShieldCheck className="size-4" /> Privacidade espiritual</p>
        <p className="body-sm mt-3 text-muted-foreground">Estas anotações permanecem somente no armazenamento deste aparelho. Não são enviadas à Sophia, ao fórum ou ao banco do Portal.</p>
        <div className="action-tray mt-5 print:hidden">
          <Botao type="button" tamanho="md" variante="contorno" onClick={exportarMarkdown}><Download className="size-4" /> Baixar Markdown</Botao>
          <Botao type="button" tamanho="md" variante="contorno" onClick={() => window.print()}><Printer className="size-4" /> Imprimir ou PDF</Botao>
          <Botao type="button" tamanho="md" variante="discreto" onClick={limpar}>Apagar anotações</Botao>
        </div>
      </aside>
    </div>
  );
