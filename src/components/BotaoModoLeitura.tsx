...
export function BotaoModoLeitura({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={
        "fixed right-4 bottom-[calc(5.25rem+env(safe-area-inset-bottom))] z-[60] inline-flex items-center gap-2 min-h-11 rounded-full px-4 py-3 border label-btn shadow-[var(--shadow-card)] transition-premium print:hidden " +
        (ativo
