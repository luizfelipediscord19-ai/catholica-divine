CREATE OR REPLACE FUNCTION public.reconciliar_identidade_conta(_token_anonimo uuid, _email text DEFAULT NULL)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT * INTO v_conta
  FROM public.identidades
  WHERE user_id = v_user_id
  FOR UPDATE;

  IF _token_anonimo IS NOT NULL THEN
    SELECT * INTO v_anonima
    FROM public.identidades
    WHERE token = _token_anonimo
  ...
  IF v_anonima.id IS NULL OR v_anonima.id = v_conta.id THEN
    RETURN v_conta.token;
  END IF;
  ...
  SELECT v_conta.id, conquista_slug, created_at
  FROM public.conquistas_usuario
  WHERE identidade_id = v_anonima.id
  ON CONFLICT (identidade_id, conquista_slug) DO NOTHING;
  ...
  SELECT v_conta.id, livro, capitulo, created_at
  FROM public.leituras_biblia
  WHERE identidade_id = v_anonima.id
  ON CONFLICT (identidade_id, livro, capitulo) DO NOTHING;
  ...
  SELECT v_conta.id, livro, capitulo, versiculo, texto, created_at
  FROM public.favoritos
  WHERE identidade_id = v_anonima.id
  ON CONFLICT (identidade_id, livro, capitulo, versiculo) DO UPDATE SET
    texto = COALESCE(public.favoritos.texto, EXCLUDED.texto);
  ...
  FROM public.forum_reacoes r
  WHERE r.identidade_id = v_anonima.id
    AND NOT EXISTS (
      SELECT 1 FROM public.forum_reacoes d
      WHERE d.identidade_id = v_conta.id
        AND d.tipo = r.tipo
  ...
  UPDATE public.identidades
  SET xp = v_conta.xp + v_anonima.xp,
...
REVOKE ALL ON FUNCTION public.reconciliar_identidade_conta(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.reconciliar_identidade_conta(uuid, text) TO authenticated, service_role;